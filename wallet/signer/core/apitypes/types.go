...
var (
	IntendedValidator = SigFormat{
		wallet.MimetypeDataWithValidator,
		0x00,
	}
	DataTyped = SigFormat{
		wallet.MimetypeTypedData,
		0x01,
	}
	ApplicationClique = SigFormat{
		wallet.MimetypeClique,
		0x02,
	}
	TextPlain = SigFormat{
...
func (typedData *TypedData) EncodePrimitiveValue(encType string, encValue any, depth int) ([]byte, error) {
	...
	case "address":
		...
		if !ok || !util.IsHexAddress(stringValue) {
			return nil, dataMismatchError(encType, encValue)
		}
		retval := make([]byte, 32)
		copy(retval[12:], util.HexToAddress(stringValue).Bytes())
		return retval, nil
	case "bool":
		...
		if boolValue {
			return math.PaddedBigBytes(util.Big1, 32), nil
		}
		return math.PaddedBigBytes(util.Big0, 32), nil
	case "string":
...
func formatPrimitiveValue(encType string, encValue any) (string, error) {
	...
	case "address":
		if stringValue, ok := encValue.(string); !ok {
			...
			return util.HexToAddress(stringValue).String(), nil
		}
	case "bool":
