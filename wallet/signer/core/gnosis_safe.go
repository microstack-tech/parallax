...
type GnosisSafeTx struct {
	...
	SafeTxHash util.Hash             `json:"contractTransactionHash"`
	Sender     util.MixedcaseAddress `json:"sender"`
	// These fields are used both on input and output
	Safe           util.MixedcaseAddress `json:"safe"`
	To             util.MixedcaseAddress `json:"to"`
	Value          math.Decimal256       `json:"value"`
	GasPrice       math.Decimal256       `json:"gasPrice"`
	Data           *hexutil.Bytes        `json:"data"`
	Operation      uint8                 `json:"operation"`
	GasToken       util.Address          `json:"gasToken"`
	RefundReceiver util.Address          `json:"refundReceiver"`
	BaseGas        big.Int               `json:"baseGas"`
	SafeTxGas      big.Int               `json:"safeTxGas"`
	Nonce          big.Int               `json:"nonce"`
	InputExpHash   util.Hash             `json:"safeTxHash"`
