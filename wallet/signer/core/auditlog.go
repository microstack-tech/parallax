...
func (l *AuditLogger) List(ctx context.Context) ([]util.Address, error) {
...
func (l *AuditLogger) New(ctx context.Context) (util.Address, error) {
	return l.api.New(ctx)
}

func (l *AuditLogger) SignTransaction(ctx context.Context, args apitypes.SendTxArgs, methodSelector *string) (*api.SignTransactionResult, error) {
	sel := "<nil>"
	...
	if res != nil {
		l.log.Info("SignTransaction", "type", "response", "data", util.Bytes2Hex(res.Raw), "error", e)
	} else {
...
func (l *AuditLogger) SignData(ctx context.Context, contentType string, addr util.MixedcaseAddress, data any) (hexutil.Bytes, error) {
	...
	l.log.Info("SignData", "type", "response", "data", util.Bytes2Hex(b), "error", e)
	return b, e
}

func (l *AuditLogger) SignGnosisSafeTx(ctx context.Context, addr util.MixedcaseAddress, gnosisTx GnosisSafeTx, methodSelector *string) (*GnosisSafeTx, error) {
	sel := "<nil>"
	if methodSelector != nil {
		sel = *methodSelector
	}
...
func (l *AuditLogger) SignTypedData(ctx context.Context, addr util.MixedcaseAddress, data apitypes.TypedData) (hexutil.Bytes, error) {
	...
	l.log.Info("SignTypedData", "type", "response", "data", util.Bytes2Hex(b), "error", e)
	return b, e
}

func (l *AuditLogger) EcRecover(ctx context.Context, data hexutil.Bytes, sig hexutil.Bytes) (util.Address, error) {
	l.log.Info("EcRecover", "type", "request", "metadata", MetadataFromContext(ctx).String(),
		"data", util.Bytes2Hex(data), "sig", util.Bytes2Hex(sig))
...
func NewAuditLogger(path string, api ExternalAPI) (*AuditLogger, error) {
	l := logging.New("api", "signer")
	handler, err := logging.FileHandler(path, logging.LogfmtFormat())
	if err != nil {
		return nil, err
	}
