...
func NewStdIOUI() *StdIOUI {
	...
	if err != nil {
		logging.Crit("Could not create stdio client", "err", err)
	}
...
func (ui *StdIOUI) dispatch(serviceMethod string, args any, reply any) error {
	...
	if err != nil {
		logging.Info("Error", "exc", err.Error())
	}
	return err
}
...
func (ui *StdIOUI) ShowError(message string) {
	...
	if err != nil {
		logging.Info("Error calling 'ui_showError'", "exc", err.Error(), "msg", message)
	}
}
...
func (ui *StdIOUI) ShowInfo(message string) {
	...
	if err != nil {
		logging.Info("Error calling 'ui_showInfo'", "exc", err.Error(), "msg", message)
	}
}
...
func (ui *StdIOUI) OnSignerStartup(info StartupInfo) {
	...
	if err != nil {
		logging.Info("Error calling 'ui_onSignerStartup'", "exc", err.Error(), "info", info)
	}
}
...
func (ui *StdIOUI) OnInputRequired(info UserInputRequest) (UserInputResponse, error) {
	...
	if err != nil {
		logging.Info("Error calling 'ui_onInputRequired'", "exc", err.Error(), "info", info)
	}
	return result, err
}
