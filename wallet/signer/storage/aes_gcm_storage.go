...
func (s *AESEncryptedStorage) Put(key, value string) {
	...
	if err != nil {
		logging.Warn("Failed to read encrypted storage", "err", err, "file", s.filename)
		return
	}
	ciphertext, iv, err := encrypt(s.key, []byte(value), []byte(key))
	if err != nil {
		logging.Warn("Failed to encrypt entry", "err", err)
		return
	}
	encrypted := storedCredential{Iv: iv, CipherText: ciphertext}
	data[key] = encrypted
	if err = s.writeEncryptedStorage(data); err != nil {
		logging.Warn("Failed to write entry", "err", err)
	}
}
...
func (s *AESEncryptedStorage) Get(key string) (string, error) {
	...
	if !exist {
		logging.Warn("Key does not exist", "key", key)
		return "", ErrNotFound
	}
	entry, err := decrypt(s.key, encrypted.Iv, encrypted.CipherText, []byte(key))
	if err != nil {
		logging.Warn("Failed to decrypt key", "key", key)
		return "", err
	}
...
func (s *AESEncryptedStorage) readEncryptedStorage() (map[string]storedCredential, error) {
	...
	if err = json.Unmarshal(raw, &creds); err != nil {
		logging.Warn("Failed to unmarshal encrypted storage", "err", err, "file", s.filename)
		return nil, err
	}
	return creds, nil
}
