...
func (r *rulesetUI) execute(jsfunc string, jsarg any) (goja.Value, error) {
	...
	if err != nil {
		logging.Warn("Failed loading libraries", "err", err)
	...
	if err != nil {
		logging.Warn("Execution failed", "err", err)
		return goja.Undefined(), err
	}
	...
	if err != nil {
		logging.Warn("failed marshalling data", "data", jsarg)
		return goja.Undefined(), err
	}
...
func (r *rulesetUI) checkApproval(jsfunc string, jsarg []byte, err error) (bool, error) {
	...
	if err != nil {
		logging.Info("error occurred during execution", "error", err)
		return false, err
	}
	result := v.ToString().String()
	if result == "Approve" {
		logging.Info("Op approved")
		return true, nil
	} else if result == "Reject" {
		logging.Info("Op rejected")
		return false, nil
	}
...
func (r *rulesetUI) ApproveTx(request *core.SignTxRequest) (core.SignTxResponse, error) {
	...
	if err != nil {
		logging.Info("Rule-based approval error, going to manual", "error", err)
...
func (r *rulesetUI) OnSignerStartup(info core.StartupInfo) {
	...
	if err != nil {
		logging.Warn("failed marshalling data", "data", info)
		return
	}
