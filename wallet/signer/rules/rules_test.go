...
func mixAddr(a string) (*util.MixedcaseAddress, error) {
	return util.NewMixedcaseAddressFromString(a)
}
...
func TestListRequest(t *testing.T) {
	...
	for i := range accs {
		...
		acc := wallet.Account{
			Address: util.BytesToAddress(util.Hex2Bytes(addr)),
			URL:     wallet.URL{Scheme: "test", Path: fmt.Sprintf("acc-%d", i)},
		}
		accs[i] = acc
	}
...
func dummySigned(value *big.Int) *types.Transaction {
	to := util.HexToAddress("000000000000000000000000000000000000dead")
	gas := uint64(21000)
...
func TestLimitWindow(t *testing.T) {
	...
	v := big.NewInt(0).SetBytes(util.Hex2Bytes("0429D069189E0000"))
	h := hexutil.Big(*v)
...
function ApproveSignData(r){
	...
	hash, rawdata := wallet.TextAndHash([]byte(message))
