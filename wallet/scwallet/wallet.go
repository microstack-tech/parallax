...
var (
	...
	DerivationSignatureHash = sha256.Sum256(util.Hash{}.Bytes())
)
...
func (w *Wallet) Accounts() []wallet.Account {
	...
	if pairing := w.Hub.pairing(w); pairing != nil {
		ret := make([]wallet.Account, 0, len(pairing.Accounts))
		for address, path := range pairing.Accounts {
			ret = append(ret, w.makeAccount(address, path))
		}
		sort.Sort(wallet.AccountsByURL(ret))
		return ret
	}
	return nil
}

func (w *Wallet) makeAccount(address util.Address, path wallet.DerivationPath) wallet.Account {
...
func (w *Wallet) Contains(account wallet.Account) bool {
	if pairing := w.Hub.pairing(w); pairing != nil {
...
func (w *Wallet) Derive(path wallet.DerivationPath, pin bool) (wallet.Account, error) {
	w.lock.Lock()
	defer w.lock.Unlock()
...
// chain state reader.
func (w *Wallet) SelfDerive(bases []wallet.DerivationPath, chain parallax.ChainStateReader) {
	w.lock.Lock()
	defer w.lock.Unlock()
...
func (w *Wallet) SignData(account wallet.Account, mimeType string, data []byte) ([]byte, error) {
	return w.signHash(account, crypto.Keccak256(data))
}

func (w *Wallet) signHash(account wallet.Account, hash []byte) ([]byte, error) {
	w.lock.Lock()
	defer w.lock.Unlock()
...
// the account in a keystore).
func (w *Wallet) SignTx(account wallet.Account, tx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
...
// It looks up the account specified either solely via its address contained within,
// or optionally with the aid of any location metadata from the embedded URL field.
func (w *Wallet) SignDataWithPassphrase(account wallet.Account, passphrase, mimeType string, data []byte) ([]byte, error) {
	return w.signHashWithPassphrase(account, passphrase, crypto.Keccak256(data))
}

func (w *Wallet) signHashWithPassphrase(account wallet.Account, passphrase string, hash []byte) ([]byte, error) {
	if !w.session.verified {
...
// the account in a keystore).
func (w *Wallet) SignText(account wallet.Account, text []byte) ([]byte, error) {
	return w.signHash(account, wallet.TextHash(text))
}

// SignTextWithPassphrase implements accounts.Wallet, attempting to sign the
// given hash with the given account using passphrase as extra authentication
func (w *Wallet) SignTextWithPassphrase(account wallet.Account, passphrase string, text []byte) ([]byte, error) {
	return w.signHashWithPassphrase(account, passphrase, crypto.Keccak256(wallet.TextHash(text)))
}
...
// It looks up the account specified either solely via its address contained within,
// or optionally with the aid of any location metadata from the embedded URL field.
func (w *Wallet) SignTxWithPassphrase(account wallet.Account, passphrase string, tx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
	if !w.session.verified {
		if err := w.Open(passphrase); err != nil {
			return nil, err
		}
...
func (w *Wallet) findAccountPath(account wallet.Account) (wallet.DerivationPath, error) {
	pairing := w.Hub.pairing(w)
	...
	return wallet.ParseDerivationPath(parts[1])
}
...
func (s *Session) pair(secret []byte) (smartcardPairing, error) {
	...
	return smartcardPairing{
		...
		Accounts:     make(map[util.Address]wallet.DerivationPath),
	}, nil
}
...
func (s *Session) derivationPath() (wallet.DerivationPath, error) {
	response, err := s.Channel.transmitEncrypted(claSCWallet, insStatus, statusP1Path, 0, nil)
	if err != nil {
		return nil, err
	}
	buf := bytes.NewReader(response.Data)
	path := make(wallet.DerivationPath, len(response.Data)/4)
...
func (s *Session) derive(path wallet.DerivationPath) (wallet.Account, error) {
	...
	default:
		return wallet.Account{}, fmt.Errorf("invalid startingPoint %d", startingPoint)
	}
...
func (s *Session) sign(path wallet.DerivationPath, hash []byte) ([]byte, error) {
	startTime := time.Now()
	_, err := s.derive(path)
	if err != nil {
		return nil, err
	...
	logging.Debug("Signed using smartcard", "deriveTime", deriveTime.Sub(startTime), "signingTime", time.Since(deriveTime))
