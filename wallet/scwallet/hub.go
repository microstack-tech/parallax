...
type smartcardPairing struct {
	...
	Accounts     map[util.Address]wallet.DerivationPath `json:"accounts"`
}
...
func (hub *Hub) Wallets() []wallet.Wallet {
	...
	cpy := make([]wallet.Wallet, 0, len(hub.wallets))
	for _, w := range hub.wallets {
		cpy = append(cpy, w)
	}
	sort.Sort(wallet.WalletsByURL(cpy))
	return cpy
}
...
func (hub *Hub) refreshWallets() {
	...
	if err != nil {
		...
		if err.Error() != "scard: Cannot find a smart card reader." {
			logging.Error("Failed to enumerate smart card readers", "err", err)
			return
		}
	}
	...
	for _, reader := range readers {
		...
		if err != nil {
			logging.Debug("Failed to open smart card", "reader", reader, "err", err)
			continue
		}
		w := NewWallet(hub, card)
		if err = w.connect(); err != nil {
			logging.Debug("Failed to connect to smart card", "reader", reader, "err", err)
			card.Disconnect(pcsc.LeaveCard)
			continue
		}
		// Card connected, start tracking in amongs the wallets
		hub.wallets[reader] = w
		events = append(events, wallet.WalletEvent{Wallet: w, Kind: wallet.WalletArrived})
	}
	// Remove any wallets no longer present
	for reader, stale := range hub.wallets {
