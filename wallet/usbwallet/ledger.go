...
type ledgerDriver struct {
	...
	log     logging.Logger // Contextual logger to tag the ledger with its id
}

// newLedgerDriver creates a new instance of a Ledger USB protocol driver.
func newLedgerDriver(logger logging.Logger) driver {
	return &ledgerDriver{
		log: logger,
	}
}
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	_, err := w.ledgerDerive(wallet.DefaultBaseDerivationPath)
	if err != nil {
...
func (w *ledgerDriver) Derive(path wallet.DerivationPath) (util.Address, error) {
...
func (w *ledgerDriver) SignTx(path wallet.DerivationPath, tx *types.Transaction, chainID *big.Int) (util.Address, *types.Transaction, error) {
	// If the Parallax app doesn't run, abort
	if w.offline() {
		return util.Address{}, nil, wallet.ErrWalletClosed
	}
	// Ensure the wallet is capable of signing the given transaction
	if chainID != nil && w.version[0] <= 1 && w.version[1] <= 0 && w.version[2] <= 2 {
		//lint:ignore ST1005 brand name displayed on the console
		return util.Address{}, nil, fmt.Errorf("Ledger v%d.%d.%d doesn't support signing this transaction, please update to v1.0.3 at least", w.version[0], w.version[1], w.version[2])
	}
...
func (w *ledgerDriver) SignTypedMessage(path wallet.DerivationPath, domainHash []byte, messageHash []byte) ([]byte, error) {
	// If the Parallax app doesn't run, abort
	if w.offline() {
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (util.Address, error) {
	...
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return util.Address{}, errors.New("reply lacks public key entry")
	}
	reply = reply[1+int(reply[0]):]

	// Extract the Parallax hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return util.Address{}, errors.New("reply lacks address entry")
	}
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (util.Address, *types.Transaction, error) {
	// Flatten the derivation path into the Ledger request
	path := make([]byte, 1+4*len(derivationPath))
	path[0] = byte(len(derivationPath))
	for i, component := range derivationPath {
	...
	if len(reply) != crypto.SignatureLength {
		return util.Address{}, nil, errors.New("reply lacks signature")
	}
