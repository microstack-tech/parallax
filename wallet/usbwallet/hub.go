...
type Hub struct {
	...
	makeDriver func(logging.Logger) driver // Factory method to construct a vendor specific driver
...
func newHub(scheme string, vendorID uint16, productIDs []uint16, usageID uint16, endpointID int, makeDriver func(logging.Logger) driver) (*Hub, error) {
	if !usb.Supported() {
...
func (hub *Hub) Wallets() []wallet.Wallet {
	...
	cpy := make([]wallet.Wallet, len(hub.wallets))
...
func (hub *Hub) refreshWallets() {
	...
	if err != nil {
		...
		logging.Error("Failed to enumerate USB devices", "hub", hub.scheme,
	...
	var (
		wallets = make([]wallet.Wallet, 0, len(devices))
		events  []wallet.WalletEvent
	)

	for _, device := range devices {
		url := wallet.URL{Scheme: hub.scheme, Path: device.Path}
		...
		for len(hub.wallets) > 0 {
			...
			events = append(events, wallet.WalletEvent{Wallet: hub.wallets[0], Kind: wallet.WalletDropped})
			hub.wallets = hub.wallets[1:]
		}
		// If there are no more wallets or the device is before the next, wrap new wallet
		if len(hub.wallets) == 0 || hub.wallets[0].URL().Cmp(url) > 0 {
			logger := logging.New("url", url)
			w := &usbWallet{hub: hub, driver: hub.makeDriver(logger), url: &url, info: device, log: logger}
	...
	for _, stale := range hub.wallets {
		events = append(events, wallet.WalletEvent{Wallet: stale, Kind: wallet.WalletDropped})
	}
...
func (hub *Hub) Subscribe(sink chan<- wallet.WalletEvent) event.Subscription {
