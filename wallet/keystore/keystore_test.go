...
func TestSign(t *testing.T) {
	...
	if _, err := ks.SignHash(wallet.Account{Address: a1.Address}, testSigData); err != nil {
		t.Fatal(err)
	}
}
...
func TestTimedUnlock(t *testing.T) {
	...
	_, err = ks.SignHash(wallet.Account{Address: a1.Address}, testSigData)
	if err != ErrLocked {
...
func TestSignRace(t *testing.T) {
	...
	for time.Now().Before(end) {
		if _, err := ks.SignHash(wallet.Account{Address: a1.Address}, testSigData); err == ErrLocked {
			return
		} else if err != nil {
...
func TestWalletNotifierLifecycle(t *testing.T) {
	...
	updates := make(chan wallet.WalletEvent)
...
func TestWalletNotifications(t *testing.T) {
	...
	var (
		live       = make(map[util.Address]wallet.Account)
	...
	for i := 0; i < 1024; i++ {
		if create := len(live) == 0 || rand.Int()%4 > 0; create {
			...
			wantEvents = append(wantEvents, walletEvent{wallet.WalletEvent{Kind: wallet.WalletArrived}, account})
		} else {
			...
			wantEvents = append(wantEvents, walletEvent{wallet.WalletEvent{Kind: wallet.WalletDropped}, account})
		}
	}
...
func checkAccounts(t *testing.T, live map[util.Address]wallet.Account, wallets []wallet.Wallet) {
	if len(live) != len(wallets) {
		t.Errorf("wallet list doesn't match required accounts: have %d, want %d", len(wallets), len(live))
		return
	}
	liveList := make([]wallet.Account, 0, len(live))
