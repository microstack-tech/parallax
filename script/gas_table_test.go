...
func TestEIP2200(t *testing.T) {
	for i, tt := range eip2200Tests {
		...
		statedb.SetState(address, util.Hash{}, util.BytesToHash([]byte{tt.original}))
		statedb.Finalise(true) // Push the state into the "original" slot

		vmctx := BlockContext{
			CanTransfer: func(StateDB, util.Address, *big.Int) bool { return true },
			Transfer:    func(StateDB, util.Address, util.Address, *big.Int) {},
		}
		vmenv := NewPVM(vmctx, TxContext{}, statedb, chainparams.AllXHashProtocolChanges, Config{ExtraEips: []int{2200}})

		_, gas, err := vmenv.Call(AccountRef(util.Address{}), address, nil, tt.gaspool, new(big.Int))
		if err != tt.failure {
