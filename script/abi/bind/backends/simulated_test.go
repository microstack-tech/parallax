...
func TestSimulatedBackend(t *testing.T) {
	...
	genAlloc := make(validation.GenesisAlloc)
	genAlloc[auth.From] = validation.GenesisAccount{Balance: big.NewInt(9223372036854775807)}

	sim := NewSimulatedBackend(genAlloc, gasLimit)
	defer sim.Close()

	// should return an error if the tx is not found
	txHash := util.HexToHash("2")
	...
	tx := types.NewContractCreation(0, big.NewInt(0), gas, gasPrice, util.FromHex(code))
...
func simTestBackend(testAddr util.Address) *SimulatedBackend {
...
func TestNewSimulatedBackend(t *testing.T) {
	...
	if sim.config != chainparams.AllXHashProtocolChanges {
		t.Errorf("expected sim config to equal params.AllXHashProtocolChanges, got %v", sim.config)
	}

	if sim.blockchain.Config() != chainparams.AllXHashProtocolChanges {
...
func TestAdjustTime(t *testing.T) {
	sim := NewSimulatedBackend(
		validation.GenesisAlloc{}, 10000000,
	)
	defer sim.Close()
...
func TestNonceAt(t *testing.T) {
	...
	tx := types.NewTransaction(nonce, testAddr, big.NewInt(1000), chainparams.TxGas, gasPrice, nil)
...
func TestSendTransaction(t *testing.T) {
	...
	tx := types.NewTransaction(uint64(0), testAddr, big.NewInt(1000), chainparams.TxGas, gasPrice, nil)
	signedTx, err := types.SignTx(tx, types.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
...
func TestEstimateGas(t *testing.T) {
	...
	sim := NewSimulatedBackend(validation.GenesisAlloc{addr: {Balance: big.NewInt(chainparams.Ether)}}, 10000000)
	defer sim.Close()

	parsed, _ := abi.JSON(strings.NewReader(contractAbi))
	contractAddr, _, _, _ := bind.DeployContract(opts, parsed, util.FromHex(contractBin), sim)
	sim.Commit()
...
func TestEstimateGasWithPrice(t *testing.T) {
	...
	sim := NewSimulatedBackend(validation.GenesisAlloc{addr: {Balance: big.NewInt(chainparams.Ether*2 + 2e17)}}, 10000000)
	defer sim.Close()

	recipient := util.HexToAddress("deadbeef")
	cases := []struct {
		name        string
		message     parallax.CallMsg
		expect      uint64
		...
		{"EstimateEIP1559WithSuperHighFees", parallax.CallMsg{
			...
			Data:      nil,
		}, chainparams.TxGas, errors.New("gas required exceeds allowance (20999)")}, // 20999=(2.2ether-0.1ether-1wei)/(1e14)
	}
	for i, c := range cases {
		got, err := sim.EstimateGas(context.Background(), c.message)
		if c.expectError != nil {
...
func TestPendingNonceAt(t *testing.T) {
	...
	tx = types.NewTransaction(uint64(1), testAddr, big.NewInt(1000), chainparams.TxGas, gasPrice, nil)
	signedTx, err = types.SignTx(tx, types.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
...
func TestPendingCodeAt(t *testing.T) {
	...
	contractAddr, tx, contract, err := bind.DeployContract(auth, parsed, util.FromHex(abiBin), sim)
	if err != nil {
	...
	if !bytes.Equal(code, util.FromHex(deployedCode)) {
		t.Errorf("code received did not match expected deployed code:\n expected %v\n actual %v", util.FromHex(deployedCode), code)
	}
}
...
func TestPendingAndCallContract(t *testing.T) {
	...
	addr, _, _, err := bind.DeployContract(contractAuth, parsed, util.FromHex(abiBin), sim)
	if err != nil {
...
func TestCallContractRevert(t *testing.T) {
	...
	contractAuth, _ := bind.NewKeyedTransactorWithChainID(testKey, big.NewInt(1337))
	addr, _, _, err := bind.DeployContract(contractAuth, parsed, util.FromHex(reverterBin), sim)
	if err != nil {
		t.Errorf("could not deploy contract: %v", err)
	}
...
func TestForkLogsReborn(t *testing.T) {
	...
	auth, _ := bind.NewKeyedTransactorWithChainID(testKey, big.NewInt(1337))
	_, _, contract, err := bind.DeployContract(auth, parsed, util.FromHex(callableBin), sim)
	if err != nil {
...
func TestForkResendTx(t *testing.T) {
	...
	_tx := types.NewTransaction(0, testAddr, big.NewInt(1000), chainparams.TxGas, gasPrice, nil)
