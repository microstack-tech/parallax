...
import "github.com/ParallaxProtocol/parallax/v2/script/abi"
...
{{range $contract := .Contracts}}
	...
	{{if .InputBin}}
		...
		func Deploy{{.Type}}(auth *bind.TransactOpts, backend bind.ContractBackend {{range .Constructor.Inputs}}, {{.Name}} {{bindtype .Type $structs}}{{end}}) (util.Address, *types.Transaction, *{{.Type}}, error) {
		  parsed, err := {{.Type}}MetaData.GetAbi()
		  if err != nil {
		    return util.Address{}, nil, nil, err
		  }
		  if parsed == nil {
			return util.Address{}, nil, nil, errors.New("GetABI returned nil")
		  }
		  {{range $pattern, $name := .Libraries}}
			{{decapitalise $name}}Addr, _, _, _ := Deploy{{capitalise $name}}(auth, backend)
			{{$contract.Type}}Bin = strings.ReplaceAll({{$contract.Type}}Bin, "__${{$pattern}}$__", {{decapitalise $name}}Addr.String()[2:])
		  {{end}}
		  address, tx, contract, err := bind.DeployContract(auth, *parsed, util.FromHex({{.Type}}Bin), backend {{range .Constructor.Inputs}}, {{.Name}}{{end}})
		  if err != nil {
		    return util.Address{}, nil, nil, err
		  }
	...
	func New{{.Type}}(address util.Address, backend bind.ContractBackend) (*{{.Type}}, error) {
	...
	func New{{.Type}}Caller(address util.Address, caller bind.ContractCaller) (*{{.Type}}Caller, error) {
	...
	func New{{.Type}}Transactor(address util.Address, transactor bind.ContractTransactor) (*{{.Type}}Transactor, error) {
	...
	// New{{.Type}}Filterer creates a new log filterer instance of {{.Type}}, bound to a specific deployed contract.
 	func New{{.Type}}Filterer(address util.Address, filterer bind.ContractFilterer) (*{{.Type}}Filterer, error) {
	...
	func bind{{.Type}}(address util.Address, caller bind.ContractCaller, transactor bind.ContractTransactor, filterer bind.ContractFilterer) (*bind.BoundContract, error) {
