...
func TestMethodPack(t *testing.T) {
	...
	var addrA, addrB = util.Address{1}, util.Address{2}
	...
	packed, err = abi.Pack("sliceAddress", []util.Address{addrA, addrB})
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(packed, sig) {
	...
	var addrC, addrD = util.Address{3}, util.Address{4}
	...
	packed, err = abi.Pack("sliceMultiAddress", []util.Address{addrA, addrB}, []util.Address{addrC, addrD})
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(packed, sig) {
	...
	sig = append(sig, util.LeftPadBytes(addrC[:], 32)...)
	sig = append(sig, util.LeftPadBytes(addrD[:], 32)...)
	packed, err = abi.Pack("nestedArray", a, []util.Address{addrC, addrD})
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(packed, sig) {
