...
var allPrecompiles = map[util.Address]PrecompiledContract{
	...
	util.BytesToAddress([]byte{0xf5}): &bigModExp{eip2565: true},
...
func testPrecompiled(addr string, test precompiledTest, t *testing.T) {
	p := allPrecompiles[util.HexToAddress(addr)]
...
func benchmarkPrecompiled(addr string, test precompiledTest, bench *testing.B) {
	...
	bench.Run(fmt.Sprintf("%s-Gas=%d", test.Name, reqGas), func(bench *testing.B) {
		...
		if util.Bytes2Hex(res) != test.Expected {
			bench.Errorf("Expected %v, got %v", test.Expected, util.Bytes2Hex(res))
			return
		}
	})
}
