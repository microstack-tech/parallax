...
type ChainReader interface {
	...
	TransactionCount(ctx context.Context, blockHash util.Hash) (uint, error)
	TransactionInBlock(ctx context.Context, blockHash util.Hash, index uint) (*types.Transaction, error)
...
type TransactionReader interface {
	...
	// it's not pending.
	TransactionByHash(ctx context.Context, txHash util.Hash) (tx *types.Transaction, isPending bool, err error)
	// TransactionReceipt returns the receipt of a mined transaction. Note that the
	// transaction may not be included in the current canonical chain even if a receipt
	// exists.
	TransactionReceipt(ctx context.Context, txHash util.Hash) (*types.Receipt, error)
}
...
type ChainStateReader interface {
	BalanceAt(ctx context.Context, account util.Address, blockNumber *big.Int) (*big.Int, error)
	StorageAt(ctx context.Context, account util.Address, key util.Hash, blockNumber *big.Int) ([]byte, error)
	CodeAt(ctx context.Context, account util.Address, blockNumber *big.Int) ([]byte, error)
	NonceAt(ctx context.Context, account util.Address, blockNumber *big.Int) (uint64, error)
}
...
type SyncProgress struct {
	...
	// "fast sync" fields. These used to be sent by parallax, but are no longer used
	// since version v1.10.
...
type CallMsg struct {
	From      util.Address  // the sender of the 'transaction'
	To        *util.Address // the destination contract (nil for contract creation)
...
type FilterQuery struct {
	BlockHash *util.Hash     // used by eth_getLogs, return logs only from block with this hash
	FromBlock *big.Int       // beginning of the queried range, nil means genesis block
	ToBlock   *big.Int       // end of the range, nil means latest block
	Addresses []util.Address // restricts matches to events created by specific contracts
...
type PendingStateReader interface {
	PendingBalanceAt(ctx context.Context, account util.Address) (*big.Int, error)
	PendingStorageAt(ctx context.Context, account util.Address, key util.Hash) ([]byte, error)
	PendingCodeAt(ctx context.Context, account util.Address) ([]byte, error)
	PendingNonceAt(ctx context.Context, account util.Address) (uint64, error)
