...
func newTestBackendWithGenerator(blocks int, generator func(int, *validation.BlockGen)) *testBackend {
	// Create a database pre-initialize with a genesis block
	db := rawdb.NewMemoryDatabase()
	(&validation.Genesis{
		Config: chainparams.TestChainConfig,
		Alloc:  validation.GenesisAlloc{testAddr: {Balance: big.NewInt(100_000_000_000_000_000)}},
	}).MustCommit(db)

	chain, _ := validation.NewBlockChain(db, nil, chainparams.TestChainConfig, xhash.NewFaker(), script.Config{}, nil, nil)

	bs, _ := validation.GenerateChain(chainparams.TestChainConfig, chain.Genesis(), xhash.NewFaker(), db, blocks, generator)
	if _, err := chain.InsertChain(bs); err != nil {
		panic(err)
	}
	txconfig := validation.DefaultTxPoolConfig
	txconfig.Journal = "" // Don't litter the disk with test journals

	return &testBackend{
		db:     db,
		chain:  chain,
		txpool: validation.NewTxPool(txconfig, chainparams.TestChainConfig, chain),
	}
}
...
func (b *testBackend) Chain() *validation.BlockChain { return b.chain }
...
func testGetBlockHeaders(t *testing.T, protocol uint) {
	...
	getHashes := func(from, limit uint64) (hashes []util.Hash) {
	...
	tests := []struct {
		...
		// A single random block should be retrievable by hash
		{
			...
			[]util.Hash{backend.chain.GetBlockByNumber(limit / 2).Hash()},
		},
	...
	for i, tt := range tests {
		...
		if tt.query.Origin.Hash == (util.Hash{}) {
...
func testGetBlockBodies(t *testing.T, protocol uint) {
	...
	tests := []struct {
		...
		{0, []util.Hash{backend.chain.Genesis().Hash()}, []bool{true}, 1},      // The genesis block should be retrievable
		{0, []util.Hash{backend.chain.CurrentBlock().Hash()}, []bool{true}, 1}, // The chains head block should be retrievable
...
func testGetNodeData(t *testing.T, protocol uint) {
	...
	generator := func(i int, block *validation.BlockGen) {
		switch i {
		case 0:
			// In block 1, the test bank sends account #1 some laxes.
			tx, _ := types.SignTx(types.NewTransaction(block.TxNonce(testAddr), acc1Addr, big.NewInt(10_000_000_000_000_000), chainparams.TxGas, block.BaseFee(), nil), signer, testKey)
			block.AddTx(tx)
		case 1:
			// In block 2, the test bank sends some more laxes to account #1.
			// acc1Addr passes it on to account #2.
			tx1, _ := types.SignTx(types.NewTransaction(block.TxNonce(testAddr), acc1Addr, big.NewInt(1_000_000_000_000_000), chainparams.TxGas, block.BaseFee(), nil), signer, testKey)
			tx2, _ := types.SignTx(types.NewTransaction(block.TxNonce(acc1Addr), acc2Addr, big.NewInt(1_000_000_000_000_000), chainparams.TxGas, block.BaseFee(), nil), signer, acc1Key)
			block.AddTx(tx1)
			block.AddTx(tx2)
		case 2:
	...
	for it.Next() {
		if key := it.Key(); len(key) == util.HashLength {
			hashes = append(hashes, util.BytesToHash(key))
		}
	}
	it.Release()
	...
	accounts := []util.Address{testAddr, acc1Addr, acc2Addr}
