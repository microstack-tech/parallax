...
type GetAccountRangePacket struct {
	...
	Origin util.Hash // Hash of the first account to retrieve
	Limit  util.Hash // Hash of the last account to retrieve
...
type AccountData struct {
	Hash util.Hash    // Hash of the account
...
func (p *AccountRangePacket) Unpack() ([]util.Hash, [][]byte, error) {
	var (
		hashes   = make([]util.Hash, len(p.Accounts))
...
type GetStorageRangesPacket struct {
	ID       uint64      // Request ID to match up responses with
	Root     util.Hash   // Root hash of the account trie to serve
	Accounts []util.Hash // Account hashes of the storage tries to serve
...
type StorageData struct {
	Hash util.Hash // Hash of the storage slot
...
func (p *StorageRangesPacket) Unpack() ([][]util.Hash, [][][]byte) {
	var (
		hashset = make([][]util.Hash, len(p.Slots))
		slotset = make([][][]byte, len(p.Slots))
	)
	for i, slots := range p.Slots {
		hashset[i] = make([]util.Hash, len(slots))
...
type GetByteCodesPacket struct {
	ID     uint64      // Request ID to match up responses with
	Hashes []util.Hash // Code hashes to retrieve the code for
	Bytes  uint64      // Soft limit at which to stop returning data
}
