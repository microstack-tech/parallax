...
import (
	...
	"github.com/ParallaxProtocol/parallax/v2/validation/state/snapshot"
...
type accountRequest struct {
	...
	origin util.Hash // First account requested to allow continuation checks
	limit  util.Hash // Last account requested to allow non-overlapping chunking
...
type accountResponse struct {
	...
	hashes   []util.Hash           // Account hashes in the returned range
...
type bytecodeRequest struct {
	...
	hashes []util.Hash  // Bytecode hashes to validate responses
	task   *accountTask // Task which this request is filling (only access fields through the runloop!!)
}
...
type bytecodeResponse struct {
	...
	hashes []util.Hash // Hashes of the bytecode to avoid double hashing
...
type storageRequest struct {
	...
	accounts []util.Hash // Account hashes to validate responses
	roots    []util.Hash // Storage roots to validate responses

	origin util.Hash // First storage slot requested to allow continuation checks
	limit  util.Hash // Last storage slot requested to allow non-overlapping chunking
...
type storageResponse struct {
	...
	accounts []util.Hash // Account hashes requested, may be only partially filled
	roots    []util.Hash // Storage roots requested, may be only partially filled

	hashes [][]util.Hash // Storage slot hashes in the returned range
...
type trienodeHealRequest struct {
	...
	hashes []util.Hash     // Trie node hashes to validate responses
...
type trienodeHealResponse struct {
	...
	hashes []util.Hash     // Hashes of the trie nodes to avoid double hashing
...
type accountTask struct {
	...
	Next     util.Hash                    // Next account to sync in this interval
	Last     util.Hash                    // Last account to sync in this interval
	SubTasks map[util.Hash][]*storageTask // Storage intervals needing fetching for large contracts
	...
	codeTasks  map[util.Hash]struct{}  // Code hashes that need retrieval
	stateTasks map[util.Hash]util.Hash // Account hashes->roots that need full state retrieval

	genBatch dbstore.Batch   // Batch used by the node generator
...
type storageTask struct {
	...
	// These fields are internals used during runtime
	root util.Hash       // Storage root hash for this instance
...
type healTask struct {
	...
	trieTasks map[util.Hash]trie.SyncPath // Set of trie node tasks currently queued for retrieval
	codeTasks map[util.Hash]struct{}      // Set of byte code tasks currently queued for retrieval
}
...
type SyncProgress struct {
	...
	AccountBytes   util.StorageSize // Number of account trie bytes persisted to disk
	BytecodeSynced uint64           // Number of bytecodes downloaded
	BytecodeBytes  util.StorageSize // Number of bytecode bytes downloaded
	StorageSynced  uint64           // Number of storage slots downloaded
	StorageBytes   util.StorageSize // Number of storage trie bytes persisted to disk

	// Status report during healing phase
	TrienodeHealSynced uint64           // Number of state trie nodes downloaded
	TrienodeHealBytes  util.StorageSize // Number of state trie bytes persisted to disk
	BytecodeHealSynced uint64           // Number of bytecodes downloaded
	BytecodeHealBytes  util.StorageSize // Number of bytecodes persisted to disk
}
...
type SyncPeer interface {
	...
	RequestAccountRange(id uint64, root, origin, limit util.Hash, bytes uint64) error

	// RequestStorageRanges fetches a batch of storage slots belonging to one or
	// more accounts. If slots from only one accout is requested, an origin marker
	// may also be used to retrieve from there.
	RequestStorageRanges(id uint64, root util.Hash, accounts []util.Hash, origin, limit []byte, bytes uint64) error

	// RequestByteCodes fetches a batch of bytecodes by hash.
	RequestByteCodes(id uint64, hashes []util.Hash, bytes uint64) error

	// RequestTrieNodes fetches a batch of account or storage trie nodes rooted in
	// a specificstate trie.
	RequestTrieNodes(id uint64, root util.Hash, paths []TrieNodePathSet, bytes uint64) error
...
type Syncer struct {
	db dbstore.KeyValueStore // Database to store the trie nodes into (and dedup)

	root    util.Hash      // Current state trie root being synced
	...
	stateWriter        dbstore.Batch    // Shared batch writer used for persisting raw states
	accountHealed      uint64           // Number of accounts downloaded during the healing stage
	accountHealedBytes util.StorageSize // Number of raw account bytes persisted to disk during the healing stage
	storageHealed      uint64           // Number of storage slots downloaded during the healing stage
	storageHealedBytes util.StorageSize // Number of raw storage bytes persisted to disk during the healing stage
...
// snap protocol.
func NewSyncer(db dbstore.KeyValueStore) *Syncer {
	return &Syncer{
		db: db,
		...
		rates:    msgrate.NewTrackers(logging.New("proto", "snap")),
...
func (s *Syncer) Register(peer SyncPeer) error {
	...
	if _, ok := s.peers[id]; ok {
		logging.Error("Snap peer already registered", "id", id)
...
func (s *Syncer) Unregister(id string) error {
	...
	s.lock.Lock()
	if _, ok := s.peers[id]; !ok {
		logging.Error("Snap peer not registered", "id", id)
...
func (s *Syncer) Sync(root util.Hash, cancel chan struct{}) error {
	...
	s.healer = &healTask{
		...
		trieTasks: make(map[util.Hash]trie.SyncPath),
		codeTasks: make(map[util.Hash]struct{}),
	}
	...
	if len(s.tasks) == 0 && s.healer.scheduler.Pending() == 0 {
		logging.Debug("Snapshot sync already completed")
		return nil
	}
	...
	logging.Debug("Starting snapshot sync cycle", "root", root)
	...
	defer func() {
		logging.Debug("Terminating snapshot sync cycle", "root", root)
		s.lock.Lock()
...
func (s *Syncer) loadSyncStatus() {
	...
	if status := rawdb.ReadSnapshotSyncStatus(s.db); status != nil {
		if err := json.Unmarshal(status, &progress); err != nil {
			logging.Error("Failed to decode snap sync status", "err", err)
		} else {
			for _, task := range progress.Tasks {
				logging.Debug("Scheduled account sync task", "from", task.Next, "last", task.Last)
			}
			s.tasks = progress.Tasks
			for _, task := range s.tasks {
				task.genBatch = dbstore.HookedBatch{
					Batch: s.db.NewBatch(),
					OnPut: func(key []byte, value []byte) {
						s.accountBytes += util.StorageSize(len(key) + len(value))
					},
				}
				...
				for _, subtasks := range task.SubTasks {
					for _, subtask := range subtasks {
						subtask.genBatch = dbstore.HookedBatch{
							Batch: s.db.NewBatch(),
							OnPut: func(key []byte, value []byte) {
								s.storageBytes += util.StorageSize(len(key) + len(value))
							},
						}
	...
	step := new(big.Int).Sub(
		new(big.Int).Div(
			new(big.Int).Exp(util.Big2, util.Big256, nil),
			big.NewInt(int64(accountConcurrency)),
		), util.Big1,
	)
	for i := 0; i < accountConcurrency; i++ {
		last := util.BigToHash(new(big.Int).Add(next.Big(), step))
		if i == accountConcurrency-1 {
			// Make sure we don't overflow if the step is not a proper divisor
			last = util.HexToHash("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff")
		}
		batch := dbstore.HookedBatch{
			Batch: s.db.NewBatch(),
			OnPut: func(key []byte, value []byte) {
				s.accountBytes += util.StorageSize(len(key) + len(value))
			},
		...
		s.tasks = append(s.tasks, &accountTask{
			Next:     next,
			Last:     last,
			SubTasks: make(map[util.Hash][]*storageTask),
			genBatch: batch,
			genTrie:  trie.NewStackTrie(batch),
		})
		logging.Debug("Created account sync task", "from", next, "last", last)
		next = util.BigToHash(new(big.Int).Add(last.Big(), util.Big1))
	}
}
...
func (s *Syncer) saveSyncStatus() {
	...
	for _, task := range s.tasks {
		if err := task.genBatch.Write(); err != nil {
			logging.Error("Failed to persist account slots", "err", err)
		}
		for _, subtasks := range task.SubTasks {
			for _, subtask := range subtasks {
				if err := subtask.genBatch.Write(); err != nil {
					logging.Error("Failed to persist storage slots", "err", err)
				}
			}
		}
	}
	...
	for _, task := range s.tasks {
		...
		go func() {
			...
			if err := peer.RequestByteCodes(reqid, hashes, maxRequestSize); err != nil {
				logging.Debug("Failed to request bytecodes", "err", err)
				s.scheduleRevertBytecodeRequest(req)
			}
		}()
	}
...
func (s *Syncer) assignStorageTasks(success chan *storageResponse, fail chan *storageRequest, cancel chan struct{}) {
	...
	for _, task := range s.tasks {
		...
		var (
			accounts = make([]util.Hash, 0, storageSets)
			roots    = make([]util.Hash, 0, storageSets)
			subtask  *storageTask
		)
		for account, subtasks := range task.SubTasks {
		...
		go func(root util.Hash) {
			...
			if err := peer.RequestStorageRanges(reqid, root, accounts, origin, limit, uint64(cap)); err != nil {
				logging.Debug("Failed to request storage", "err", err)
				s.scheduleRevertStorageRequest(req)
			}
		}(s.root)
	...
	for len(s.healer.trieTasks) > 0 || s.healer.scheduler.Pending() > 0 {
		...
		go func(root util.Hash) {
			...
			if err := peer.RequestTrieNodes(reqid, root, pathsets, maxRequestSize); err != nil {
				logging.Debug("Failed to request trienode healers", "err", err)
				s.scheduleRevertTrienodeHealRequest(req)
			}
		}(s.root)
	}
...
func (s *Syncer) assignBytecodeHealTasks(success chan *bytecodeHealResponse, fail chan *bytecodeHealRequest, cancel chan struct{}) {
	...
	for len(s.healer.codeTasks) > 0 || s.healer.scheduler.Pending() > 0 {
		...
		go func() {
			...
			// Attempt to send the remote request and revert if it fails
			if err := peer.RequestByteCodes(reqid, hashes, maxRequestSize); err != nil {
				logging.Debug("Failed to request bytecode healers", "err", err)
				s.scheduleRevertBytecodeHealRequest(req)
			}
		}()
	}
...
func (s *Syncer) revertAccountRequest(req *accountRequest) {
	logging.Debug("Reverting account request", "peer", req.peer, "reqid", req.id)
	select {
	case <-req.stale:
		logging.Trace("Account request already reverted", "peer", req.peer, "reqid", req.id)
		return
	default:
	}
	close(req.stale)
...
func (s *Syncer) revertBytecodeRequest(req *bytecodeRequest) {
	logging.Debug("Reverting bytecode request", "peer", req.peer)
	select {
	case <-req.stale:
		logging.Trace("Bytecode request already reverted", "peer", req.peer, "reqid", req.id)
		return
	default:
	}
	close(req.stale)
...
func (s *Syncer) revertStorageRequest(req *storageRequest) {
	logging.Debug("Reverting storage request", "peer", req.peer)
	select {
	case <-req.stale:
		logging.Trace("Storage request already reverted", "peer", req.peer, "reqid", req.id)
		return
	default:
	}
	close(req.stale)
...
func (s *Syncer) revertTrienodeHealRequest(req *trienodeHealRequest) {
	logging.Debug("Reverting trienode heal request", "peer", req.peer)
	select {
	case <-req.stale:
		logging.Trace("Trienode heal request already reverted", "peer", req.peer, "reqid", req.id)
		return
	default:
	}
	close(req.stale)
...
func (s *Syncer) revertBytecodeHealRequest(req *bytecodeHealRequest) {
	logging.Debug("Reverting bytecode heal request", "peer", req.peer)
	select {
	case <-req.stale:
		logging.Trace("Bytecode heal request already reverted", "peer", req.peer, "reqid", req.id)
		return
	default:
	}
	close(req.stale)
...
func (s *Syncer) processAccountResponse(res *accountResponse) {
	...
	res.task.codeTasks = make(map[util.Hash]struct{})
	res.task.stateTasks = make(map[util.Hash]util.Hash)

	resumed := make(map[util.Hash]struct{})

	res.task.pend = 0
	for i, account := range res.accounts {
		// Check if the account is a contract with an unknown code
		if !bytes.Equal(account.CodeHash, emptyCode[:]) {
			if !rawdb.HasCodeWithPrefix(s.db, util.BytesToHash(account.CodeHash)) {
				res.task.codeTasks[util.BytesToHash(account.CodeHash)] = struct{}{}
		...
		if account.Root != emptyRoot {
			if ok, err := s.db.Has(account.Root[:]); err != nil || !ok {
				...
				if subtasks, ok := res.task.SubTasks[res.hashes[i]]; ok {
					logging.Debug("Resuming large storage retrieval", "account", res.hashes[i], "root", account.Root)
					for _, subtask := range subtasks {
	...
	for hash := range res.task.SubTasks {
		if _, ok := resumed[hash]; !ok {
			logging.Debug("Aborting suspended storage retrieval", "account", hash)
...
func (s *Syncer) processBytecodeResponse(res *bytecodeResponse) {
	...
	for i, hash := range res.hashes {
		...
		for j, account := range res.task.res.accounts {
			if res.task.needCode[j] && hash == util.BytesToHash(account.CodeHash) {
	...
	bytes := util.StorageSize(batch.ValueSize())
	if err := batch.Write(); err != nil {
		logging.Crit("Failed to persist bytecodes", "err", err)
	}
	s.bytecodeSynced += codes
	s.bytecodeBytes += bytes

	logging.Debug("Persisted set of bytecodes", "count", codes, "bytes", bytes)
...
func (s *Syncer) processStorageResponse(res *storageResponse) {
			...
			if res.subTask == nil && i == len(res.hashes)-1 && res.cont {
				...
				if tasks, ok := res.mainTask.SubTasks[account]; !ok {
					...
					if estimate, err := estimateRemainingSlots(len(keys), lastKey); err == nil {
						...
						logging.Debug("Chunked large contract", "initiators", len(keys), "tail", lastKey, "remaining", estimate, "chunks", chunks)
					} else {
						logging.Debug("Chunked large contract", "initiators", len(keys), "tail", lastKey, "chunks", chunks)
					}
					...
					for _, task := range tasks {
						logging.Debug("Created storage sync task", "account", account, "root", acc.Root, "from", task.Next, "last", task.Last)
					}
	...
	if res.subTask != nil {
		if res.subTask.done {
			if root, err := res.subTask.genTrie.Commit(); err != nil {
				logging.Error("Failed to commit stack slots", "err", err)
		...
		if res.subTask.genBatch.ValueSize() > dbstore.IdealBatchSize || res.subTask.done {
			if err := res.subTask.genBatch.Write(); err != nil {
				logging.Error("Failed to persist stack slots", "err", err)
			}
	...
	if err := batch.Write(); err != nil {
		logging.Crit("Failed to persist storage slots", "err", err)
	}
	s.storageSynced += uint64(slots)

	logging.Debug("Persisted set of storage slots", "accounts", len(res.hashes), "slots", slots, "bytes", s.storageBytes-oldStorageBytes)
...
func (s *Syncer) processTrienodeHealResponse(res *trienodeHealResponse) {
	for i, hash := range res.hashes {
		...
		s.trienodeHealBytes += util.StorageSize(len(node))
		...
		case trie.ErrNotRequested:
			s.trienodeHealNops++
		default:
			logging.Error("Invalid trienode processed", "hash", hash, "err", err)
		}
	}
	batch := s.db.NewBatch()
	if err := s.healer.scheduler.Commit(batch); err != nil {
		logging.Error("Failed to commit healing data", "err", err)
	}
	if err := batch.Write(); err != nil {
		logging.Crit("Failed to persist healing data", "err", err)
	}
	logging.Debug("Persisted set of healing data", "type", "trienodes", "bytes", util.StorageSize(batch.ValueSize()))
}
...
func (s *Syncer) processBytecodeHealResponse(res *bytecodeHealResponse) {
	for i, hash := range res.hashes {
		...
		s.bytecodeHealBytes += util.StorageSize(len(node))
		...
		case trie.ErrNotRequested:
			s.bytecodeHealNops++
		default:
			logging.Error("Invalid bytecode processed", "hash", hash, "err", err)
		}
	}
	batch := s.db.NewBatch()
	if err := s.healer.scheduler.Commit(batch); err != nil {
	...
	logging.Debug("Persisted set of healing data", "type", "bytecode", "bytes", util.StorageSize(batch.ValueSize()))
}
...
func (s *Syncer) forwardAccountTask(task *accountTask) {
	...
	// Flush anything written just now and update the stats
	if err := batch.Write(); err != nil {
		logging.Crit("Failed to persist accounts", "err", err)
	}
	...
	if task.done {
		if _, err := task.genTrie.Commit(); err != nil {
			logging.Error("Failed to commit stack account", "err", err)
		}
	}
	if task.genBatch.ValueSize() > dbstore.IdealBatchSize || task.done {
		if err := task.genBatch.Write(); err != nil {
			logging.Error("Failed to persist stack account", "err", err)
		}
		task.genBatch.Reset()
	}
	logging.Debug("Persisted range of accounts", "accounts", len(res.accounts), "bytes", s.accountBytes-oldAccountBytes)
}

// OnAccounts is a callback method to invoke when a range of accounts are
// received from a remote peer.
func (s *Syncer) OnAccounts(peer SyncPeer, id uint64, hashes []util.Hash, accounts [][]byte, proof [][]byte) error {
	size := util.StorageSize(len(hashes) * util.HashLength)
	for _, account := range accounts {
		size += util.StorageSize(len(account))
	}
	for _, node := range proof {
		size += util.StorageSize(len(node))
	}
...
func (s *Syncer) onByteCodes(peer SyncPeer, id uint64, bytecodes [][]byte) error {
	var size util.StorageSize
	for _, code := range bytecodes {
		size += util.StorageSize(len(code))
	}
	logger := peer.Log().New("reqid", id)
...
func (s *Syncer) OnStorage(peer SyncPeer, id uint64, hashes [][]util.Hash, slots [][][]byte, proof [][]byte) error {
	// Gather some trace stats to aid in debugging issues
	var (
		hashCount int
		slotCount int
		size      util.StorageSize
	)
	for _, hashset := range hashes {
		size += util.StorageSize(util.HashLength * len(hashset))
		hashCount += len(hashset)
	}
	for _, slotset := range slots {
		for _, slot := range slotset {
			size += util.StorageSize(len(slot))
		}
...
func (s *Syncer) onHealState(paths [][]byte, value []byte) error {
	if len(paths) == 1 {
		...
		rawdb.WriteAccountSnapshot(s.stateWriter, util.BytesToHash(paths[0]), blob)
		s.accountHealed += 1
		s.accountHealedBytes += util.StorageSize(1 + util.HashLength + len(blob))
	}
	if len(paths) == 2 {
		rawdb.WriteStorageSnapshot(s.stateWriter, util.BytesToHash(paths[0]), util.BytesToHash(paths[1]), value)
		s.storageHealed += 1
		s.storageHealedBytes += util.StorageSize(1 + 2*util.HashLength + len(value))
	}
	if s.stateWriter.ValueSize() > dbstore.IdealBatchSize {
...
var hashSpace = new(big.Int).Exp(util.Big2, util.Big256, nil)
...
func (s *Syncer) reportSyncProgress(force bool) {
	...
	var (
		...
		accounts = fmt.Sprintf("%v@%v", logging.FormatLogfmtUint64(s.accountSynced), s.accountBytes.TerminalString())
		storage  = fmt.Sprintf("%v@%v", logging.FormatLogfmtUint64(s.storageSynced), s.storageBytes.TerminalString())
		bytecode = fmt.Sprintf("%v@%v", logging.FormatLogfmtUint64(s.bytecodeSynced), s.bytecodeBytes.TerminalString())
	)
	logging.Info("State sync in progress", "synced", progress, "state", synced,
		"accounts", accounts, "slots", storage, "codes", bytecode, "eta", util.PrettyDuration(estTime-elapsed))
}
...
func (s *Syncer) reportHealProgress(force bool) {
	...
	// Create a mega progress report
	var (
		trienode = fmt.Sprintf("%v@%v", logging.FormatLogfmtUint64(s.trienodeHealSynced), s.trienodeHealBytes.TerminalString())
		bytecode = fmt.Sprintf("%v@%v", logging.FormatLogfmtUint64(s.bytecodeHealSynced), s.bytecodeHealBytes.TerminalString())
		accounts = fmt.Sprintf("%v@%v", logging.FormatLogfmtUint64(s.accountHealed), s.accountHealedBytes.TerminalString())
		storage  = fmt.Sprintf("%v@%v", logging.FormatLogfmtUint64(s.storageHealed), s.storageHealedBytes.TerminalString())
	)
	logging.Info("State heal in progress", "accounts", accounts, "slots", storage,
...
func estimateRemainingSlots(hashes int, last util.Hash) (uint64, error) {
	if last == (util.Hash{}) {
...
func sortByAccountPath(hashes []util.Hash, paths []trie.SyncPath) ([]util.Hash, []trie.SyncPath, []TrieNodePathSet) {
