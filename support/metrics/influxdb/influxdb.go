...
func InfluxDBWithTags(r metrics.Registry, d time.Duration, url, database, username, password, namespace string, tags map[string]string) {
	...
	if err != nil {
		logging.Warn("Unable to parse InfluxDB", "url", url, "err", err)
		return
	}
	...
	if err := rep.makeClient(); err != nil {
		logging.Warn("Unable to make InfluxDB client", "err", err)
		return
	}
	...
	for {
		...
		case <-intervalTicker:
			if err := r.send(); err != nil {
				logging.Warn("Unable to send to InfluxDB", "err", err)
			}
		case <-pingTicker:
			_, _, err := r.client.Ping()
			if err != nil {
				logging.Warn("Got error while sending a ping to InfluxDB, trying to recreate client", "err", err)
