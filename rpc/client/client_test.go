...
func TestToFilterArg(t *testing.T) {
	...
	addresses := []util.Address{
		util.HexToAddress("0xD36722ADeC3EdCB29c8e7b5a47f352D701393462"),
	}
	blockHash := util.HexToHash(
...
var genesis = &validation.Genesis{
	...
	Alloc:     validation.GenesisAlloc{testAddr: {Balance: testBalance}},
...
func generateTestChain() []*types.Block {
	...
	blocks, _ := validation.GenerateChain(genesis.Config, gblock, engine, db, 2, generate)
...
func testChainID(t *testing.T, client *rpc.Client) {
	...
	if id == nil || id.Cmp(chainparams.AllXHashProtocolChanges.ChainID) != 0 {
...
func testAtFunctions(t *testing.T, client *rpc.Client) {
	...
	// StorageAt
	storage, err := ec.StorageAt(context.Background(), testAddr, util.Hash{}, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	penStorage, err := ec.PendingStorageAt(context.Background(), testAddr, util.Hash{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
