...
func createGQLService(t *testing.T, stack *node.Node) {
	...
	parallaxBackend, err := protocol.New(stack, parallaxConf)
	if err != nil {
		t.Fatalf("could not create parallax backend: %v", err)
	}
	// Create some blocks and import them
	chain, _ := validation.GenerateChain(chainparams.AllXHashProtocolChanges, parallaxBackend.BlockChain().Genesis(),
		xhash.NewFaker(), parallaxBackend.ChainDb(), 10, func(i int, gen *validation.BlockGen) {})
...
func createGQLServiceWithTransactions(t *testing.T, stack *node.Node) {
	...
	// Create some blocks and import them
	chain, _ := validation.GenerateChain(chainparams.AllXHashProtocolChanges, parallaxBackend.BlockChain().Genesis(),
		xhash.NewFaker(), parallaxBackend.ChainDb(), 1, func(i int, b *validation.BlockGen) {
