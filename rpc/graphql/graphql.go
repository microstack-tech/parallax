...
func (a *Account) Address(ctx context.Context) (util.Address, error) {
...
func (a *Account) Storage(ctx context.Context, args struct{ Slot util.Hash }) (util.Hash, error) {
	state, err := a.getState(ctx)
	if err != nil {
...
func (l *Log) Topics(ctx context.Context) []util.Hash {
	return l.log.Topics
}
...
func (at *AccessTuple) Address(ctx context.Context) util.Address {
	return at.address
}

func (at *AccessTuple) StorageKeys(ctx context.Context) []util.Hash {
...
func (t *Transaction) CreatedContract(ctx context.Context, args BlockNumberArgs) (*Account, error) {
	receipt, err := t.getReceipt(ctx)
	if err != nil || receipt == nil || receipt.ContractAddress == (util.Address{}) {
		return nil, err
	}
	return &Account{
		backend:       t.backend,
...
func (b *Block) NextBaseFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	...
	if header.BaseFee == nil {
		...
		if !chaincfg.IsLondon(new(big.Int).Add(header.Number, util.Big1)) {
			return nil, nil
		}
	}
	nextBaseFee := kernel.CalcBaseFee(chaincfg, header)
...
func (b *Block) MixHash(ctx context.Context) (util.Hash, error) {
	header, err := b.resolveHeader(ctx)
	if err != nil {
		return util.Hash{}, err
	}
...
func (b *Block) TransactionsRoot(ctx context.Context) (util.Hash, error) {
	header, err := b.resolveHeader(ctx)
	if err != nil {
		return util.Hash{}, err
	}
...
func (b *Block) StateRoot(ctx context.Context) (util.Hash, error) {
	header, err := b.resolveHeader(ctx)
	if err != nil {
		return util.Hash{}, err
	}
...
func (b *Block) ReceiptsRoot(ctx context.Context) (util.Hash, error) {
	header, err := b.resolveHeader(ctx)
	if err != nil {
		return util.Hash{}, err
	}
...
func (b *Block) OmmerHash(ctx context.Context) (util.Hash, error) {
...
type CallData struct {
	From                 *util.Address   // The Parallax address the call is from.
	To                   *util.Address   // The Parallax address the call is to.
...
func (r *Resolver) Transaction(ctx context.Context, args struct{ Hash util.Hash }) (*Transaction, error) {
	tx := &Transaction{
		backend: r.backend,
		hash:    args.Hash,
	}
...
func (r *Resolver) SendRawTransaction(ctx context.Context, args struct{ Data hexutil.Bytes }) (util.Hash, error) {
