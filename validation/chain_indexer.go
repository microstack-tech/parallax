...
type ChainIndexerBackend interface {
	...
	Reset(ctx context.Context, section uint64, prevHead util.Hash) error
...
type ChainIndexer struct {
	chainDb  dbstore.Database    // Chain database to index the data from
	indexDb  dbstore.Database    // Prefixed table-view of the db to write index metadata into
	...
	checkpointHead     util.Hash // Section head belonging to the checkpoint
...
func NewChainIndexer(chainDb dbstore.Database, indexDb dbstore.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
...
func (c *ChainIndexer) AddCheckpoint(section uint64, shead util.Hash) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *ChainIndexer) processSection(section uint64, lastHead util.Hash) (util.Hash, error) {
	...
	for number := section * c.sectionSize; number < (section+1)*c.sectionSize; number++ {
		...
		if hash == (util.Hash{}) {
			return util.Hash{}, fmt.Errorf("canonical block #%d unknown", number)
		}
		header := rawdb.ReadHeader(c.chainDb, hash, number)
		if header == nil {
			return util.Hash{}, fmt.Errorf("block #%d [%x..] not found", number, hash[:4])
		} else if header.ParentHash != lastHead {
			return util.Hash{}, fmt.Errorf("chain reorged during section processing")
		}
...
// verifications.
func (c *ChainIndexer) Sections() (uint64, uint64, util.Hash) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
// index database.
func (c *ChainIndexer) SectionHead(section uint64) util.Hash {
	var data [8]byte
	binary.BigEndian.PutUint64(data[:], section)

	hash, _ := c.indexDb.Get(append([]byte("shead"), data[:]...))
	if len(hash) == len(util.Hash{}) {
...
// database.
func (c *ChainIndexer) setSectionHead(section uint64, hash util.Hash) {
	var data [8]byte
	binary.BigEndian.PutUint64(data[:], section)
