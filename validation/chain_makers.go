...
func (b *BlockGen) SetCoinbase(addr util.Address) {
	if b.gasPool != nil {
		if len(b.txs) > 0 {
...
func (b *BlockGen) AddTxWithChain(bc *BlockChain, tx *types.Transaction) {
	if b.gasPool == nil {
		b.SetCoinbase(util.Address{})
	}
	b.statedb.Prepare(tx.Hash(), len(b.txs))
	receipt, err := ApplyTransaction(b.config, bc, &b.header.Coinbase, b.gasPool, b.statedb, b.header, tx, &b.header.GasUsed, script.Config{})
	if err != nil {
		panic(err)
	}
...
func (b *BlockGen) GetBalance(addr util.Address) *big.Int {
...
func (b *BlockGen) TxNonce(addr util.Address) uint64 {
...
// OffsetTime modifies the time instance of a block and recomputes its
// difficulty through the engine. Note that with the Nakamoto retarget the
// difficulty does not depend on the child block's time, so within a retarget
// epoch this leaves difficulty unchanged; forks must be made heavier by
// length instead.
...
func (cr *fakeChainReader) Config() *chainparams.ChainConfig {
	return cr.config
}

func (cr *fakeChainReader) CurrentHeader() *types.Header                          { return nil }
func (cr *fakeChainReader) GetHeaderByNumber(number uint64) *types.Header         { return nil }
func (cr *fakeChainReader) GetHeaderByHash(hash util.Hash) *types.Header          { return nil }
func (cr *fakeChainReader) GetHeader(hash util.Hash, number uint64) *types.Header { return nil }
func (cr *fakeChainReader) GetBlock(hash util.Hash, number uint64) *types.Block   { return nil }
func (cr *fakeChainReader) GetTd(hash util.Hash, number uint64) *big.Int          { return nil }
