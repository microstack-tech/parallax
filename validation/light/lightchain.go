...
func NewLightChain(odr OdrBackend, config *chainparams.ChainConfig, engine kernel.Engine, checkpoint *chainparams.TrustedCheckpoint) (*LightChain, error) {
	...
	bc.forker = validation.NewForkChoice(bc, nil)
	var err error
	bc.hc, err = validation.NewHeaderChain(odr.Database(), config, bc.engine, bc.getProcInterrupt)
	if err != nil {
		return nil, err
	}
	bc.genesisBlock, _ = bc.GetBlockByNumber(NoOdr, 0)
	if bc.genesisBlock == nil {
		return nil, validation.ErrNoGenesis
	}
	if checkpoint != nil {
...
func (lc *LightChain) AddTrustedCheckpoint(cp *chainparams.TrustedCheckpoint) {
	...
	logging.Info("Added trusted checkpoint", "block", (cp.SectionIndex+1)*lc.indexerConfig.ChtSize-1, "hash", cp.SectionHead)
}
...
func (lc *LightChain) HeaderChain() *validation.HeaderChain {
	return lc.hc
}
...
func (lc *LightChain) loadLastState() error {
	if head := rawdb.ReadHeadHeaderHash(lc.chainDb); head == (util.Hash{}) {
	...
	logging.Info("Loaded most recent local header", "number", header.Number, "hash", header.Hash(), "td", headerTd, "age", util.PrettyAge(time.Unix(int64(header.Time), 0)))
	return nil
}
...
func (lc *LightChain) ResetWithGenesisBlock(genesis *types.Block) {
	...
	if err := batch.Write(); err != nil {
		logging.Crit("Failed to reset genesis block", "err", err)
	}
...
func (lc *LightChain) GetBody(ctx context.Context, hash util.Hash) (*types.Body, error) {
...
func (lc *LightChain) GetBodyRLP(ctx context.Context, hash util.Hash) (rlp.RawValue, error) {
	// Short circuit if the body's already in the cache, retrieve otherwise
...
// it if present.
func (lc *LightChain) HasBlock(hash util.Hash, number uint64) bool {
...
// caching it if found.
func (lc *LightChain) GetBlock(ctx context.Context, hash util.Hash, number uint64) (*types.Block, error) {
...
// caching it if found.
func (lc *LightChain) GetBlockByHash(ctx context.Context, hash util.Hash) (*types.Block, error) {
	number := lc.hc.GetBlockNumber(hash)
	if number == nil {
		return nil, errors.New("unknown block")
	}
...
func (lc *LightChain) Rollback(chain []util.Hash) {
	lc.chainmu.Lock()
	defer lc.chainmu.Unlock()
	...
	if err := batch.Write(); err != nil {
		logging.Crit("Failed to rollback light chain", "error", err)
	}
}
...
func (lc *LightChain) InsertHeader(header *types.Header) error {
	...
	logging.Info("Inserted header", "number", header.Number, "hash", header.Hash())
	return err
}
...
func (lc *LightChain) SetCanonical(header *types.Header) error {
	...
	lc.chainFeed.Send(validation.ChainEvent{Block: block, Hash: block.Hash()})
	lc.chainHeadFeed.Send(validation.ChainHeadEvent{Block: block})
	logging.Info("Set the chain head", "number", block.Number(), "hash", block.Hash())
	return nil
}
...
func (lc *LightChain) InsertHeaderChain(chain []*types.Header, checkFreq int) (int, error) {
	...
	switch status {
	case validation.CanonStatTy:
		lc.chainFeed.Send(validation.ChainEvent{Block: block, Hash: block.Hash()})
		lc.chainHeadFeed.Send(validation.ChainHeadEvent{Block: block})
	case validation.SideStatTy:
		lc.chainSideFeed.Send(validation.ChainSideEvent{Block: block})
	}
	return 0, err
}
...
func (lc *LightChain) GetTd(hash util.Hash, number uint64) *big.Int {
...
func (lc *LightChain) GetTdOdr(ctx context.Context, hash util.Hash, number uint64) *big.Int {
...
// caching it if found.
func (lc *LightChain) GetHeader(hash util.Hash, number uint64) *types.Header {
	return lc.hc.GetHeader(hash, number)
}

// GetHeaderByHash retrieves a block header from the database by hash, caching it if
// found.
func (lc *LightChain) GetHeaderByHash(hash util.Hash) *types.Header {
	return lc.hc.GetHeaderByHash(hash)
}

// HasHeader checks if a block header is present in the database or not, caching
// it if present.
func (lc *LightChain) HasHeader(hash util.Hash, number uint64) bool {
	return lc.hc.HasHeader(hash, number)
}

// GetCanonicalHash returns the canonical hash for a given block number
func (lc *LightChain) GetCanonicalHash(number uint64) util.Hash {
...
func (lc *LightChain) GetAncestor(hash util.Hash, number, ancestor uint64, maxNonCanonical *uint64) (util.Hash, uint64) {
...
func (lc *LightChain) Config() *chainparams.ChainConfig { return lc.hc.Config() }
...
func (lc *LightChain) SyncCheckpoint(ctx context.Context, checkpoint *chainparams.TrustedCheckpoint) bool {
	...
	if header, err := GetHeaderByNumber(ctx, lc.odr, latest); header != nil && err == nil {
		...
		if lc.hc.CurrentHeader().Number.Uint64() < header.Number.Uint64() {
			logging.Info("Updated latest header based on CHT", "number", header.Number, "hash", header.Hash(), "age", util.PrettyAge(time.Unix(int64(header.Time), 0)))
...
func (lc *LightChain) SubscribeChainEvent(ch chan<- validation.ChainEvent) event.Subscription {
	return lc.scope.Track(lc.chainFeed.Subscribe(ch))
}

// SubscribeChainHeadEvent registers a subscription of ChainHeadEvent.
func (lc *LightChain) SubscribeChainHeadEvent(ch chan<- validation.ChainHeadEvent) event.Subscription {
	return lc.scope.Track(lc.chainHeadFeed.Subscribe(ch))
}

// SubscribeChainSideEvent registers a subscription of ChainSideEvent.
func (lc *LightChain) SubscribeChainSideEvent(ch chan<- validation.ChainSideEvent) event.Subscription {
...
func (lc *LightChain) SubscribeRemovedLogsEvent(ch chan<- validation.RemovedLogsEvent) event.Subscription {
	return lc.scope.Track(new(event.Feed).Subscribe(ch))
}
