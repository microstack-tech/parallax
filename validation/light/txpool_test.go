...
func (tr *testTxRelay) NewHead(head util.Hash, mined []util.Hash, rollback []util.Hash) {
	m := len(mined)
	if m != 0 {
		tr.mined <- m
	}
...
func (tr *testTxRelay) Discard(hashes []util.Hash) {
...
func txPoolTestChainGen(i int, block *validation.BlockGen) {
	s := minedTx(i)
	e := minedTx(i + 1)
	for i := s; i < e; i++ {
...
func TestTxPool(t *testing.T) {
	for i := range testTx {
		testTx[i], _ = types.SignTx(types.NewTransaction(uint64(i), acc1Addr, big.NewInt(10000), chainparams.TxGas, big.NewInt(chainparams.InitialBaseFee), nil), types.HomesteadSigner{}, testBankKey)
	}
	...
	gchain, _ := validation.GenerateChain(chainparams.TestChainConfig, genesis, xhash.NewFaker(), sdb, poolTestBlocks, txPoolTestChainGen)
	...
	lightchain, _ := NewLightChain(odr, chainparams.TestChainConfig, xhash.NewFullFaker(), nil)
	txPermanent = 50
	pool := NewTxPool(chainparams.TestChainConfig, lightchain, relay)
