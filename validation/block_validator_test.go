...
func TestHeaderVerification(t *testing.T) {
	...
	var (
		...
		gspec     = &Genesis{Config: chainparams.TestChainConfig}
		genesis   = gspec.MustCommit(testdb)
		blocks, _ = GenerateChain(chainparams.TestChainConfig, genesis, xhash.NewFaker(), testdb, 8, nil)
	)
	...
	chain, _ := NewBlockChain(testdb, nil, chainparams.TestChainConfig, xhash.NewFaker(), script.Config{}, nil, nil)
	defer chain.Stop()
...
func testHeaderConcurrentVerification(t *testing.T, threads int) {
	...
	for i, valid := range []bool{true, false} {
		...
		if valid {
			...
			chain, _ := NewBlockChain(testdb, nil, chainparams.TestChainConfig, xhash.NewFakeFailer(uint64(len(headers)-1)), script.Config{}, nil, nil)
			_, results = chain.engine.VerifyHeaders(chain, headers, seals)
			chain.Stop()
		}
...
func testHeaderConcurrentAbortion(t *testing.T, threads int) {
	...
	var (
		...
		gspec     = &Genesis{Config: chainparams.TestChainConfig}
		genesis   = gspec.MustCommit(testdb)
		blocks, _ = GenerateChain(chainparams.TestChainConfig, genesis, xhash.NewFaker(), testdb, 1024, nil)
	)
	headers := make([]*types.Header, len(blocks))
	seals := make([]bool, len(blocks))
	...
	chain, _ := NewBlockChain(testdb, nil, chainparams.TestChainConfig, xhash.NewFakeDelayer(time.Millisecond), script.Config{}, nil, nil)
	defer chain.Stop()
