...
func testRepair(t *testing.T, tt *rewindTest, snapshots bool) {
	...
	chain, err := NewBlockChain(db, config, chainparams.AllXHashProtocolChanges, engine, script.Config{}, nil, nil)
	if err != nil {
	...
	if tt.sidechainBlocks > 0 {
		sideblocks, _ = GenerateChain(chainparams.TestChainConfig, genesis, engine, rawdb.NewMemoryDatabase(), tt.sidechainBlocks, func(i int, b *BlockGen) {
			b.SetCoinbase(util.Address{0x01})
		})
		if _, err := chain.InsertChain(sideblocks); err != nil {
			t.Fatalf("Failed to import side chain: %v", err)
		}
	}
	canonblocks, _ := GenerateChain(chainparams.TestChainConfig, genesis, engine, rawdb.NewMemoryDatabase(), tt.canonicalBlocks, func(i int, b *BlockGen) {
		b.SetCoinbase(util.Address{0x02})
	...
	newChain, err := NewBlockChain(db, nil, chainparams.AllXHashProtocolChanges, engine, script.Config{}, nil, nil)
	if err != nil {
...
func TestIssue23496(t *testing.T) {
	...
	blocks, _ := GenerateChain(chainparams.TestChainConfig, genesis, engine, rawdb.NewMemoryDatabase(), 4, func(i int, b *BlockGen) {
		b.SetCoinbase(util.Address{0x02})
		b.SetDifficulty(big.NewInt(1000000))
	})
	...
	chain, err = NewBlockChain(db, nil, chainparams.AllXHashProtocolChanges, engine, script.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	defer chain.Stop()
