...
type txNoncer struct {
	...
	nonces   map[util.Address]uint64
	lock     sync.Mutex
}
...
func (txn *txNoncer) get(addr util.Address) uint64 {
...
func (txn *txNoncer) set(addr util.Address, nonce uint64) {
	txn.lock.Lock()
	defer txn.lock.Unlock()
...
func (txn *txNoncer) setIfLower(addr util.Address, nonce uint64) {
	txn.lock.Lock()
	defer txn.lock.Unlock()
...
func (txn *txNoncer) setAll(all map[util.Address]uint64) {
	txn.lock.Lock()
	defer txn.lock.Unlock()
