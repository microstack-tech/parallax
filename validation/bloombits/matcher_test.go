...
func TestMatcherWildcards(t *testing.T) {
	...
	matcher := NewMatcher(testSectionSize, [][][]byte{
		{util.Address{}.Bytes(), util.Address{0x01}.Bytes()}, // Default address is not a wildcard
		{util.Hash{}.Bytes(), util.Hash{0x01}.Bytes()},       // Default hash is not a wildcard
		{util.Hash{0x01}.Bytes()},                            // Plain rule, sanity check
		{util.Hash{0x01}.Bytes(), nil},                       // Wildcard suffix, drop rule
		{nil, util.Hash{0x01}.Bytes()},                       // Wildcard prefix, drop rule
