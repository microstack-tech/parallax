...
func TestUpdateLeaks(t *testing.T) {
	...
	state, _ := New(util.Hash{}, NewDatabase(db), nil)
...
func TestIntermediateLeaks(t *testing.T) {
	...
	transState, _ := New(util.Hash{}, NewDatabase(transDb), nil)
	finalState, _ := New(util.Hash{}, NewDatabase(finalDb), nil)

	modify := func(state *StateDB, addr util.Address, i, tweak byte) {
		state.SetBalance(addr, big.NewInt(int64(11*i)+int64(tweak)))
		state.SetNonce(addr, uint64(42*i+tweak))
		if i%2 == 0 {
			state.SetState(addr, util.Hash{i, i, i, 0}, util.Hash{})
			state.SetState(addr, util.Hash{i, i, i, tweak}, util.Hash{i, i, i, i, tweak})
		}
		if i%3 == 0 {
	...
	for i := byte(0); i < 255; i++ {
		modify(transState, util.Address{i}, i, 0)
	}
	...
	for i := byte(0); i < 255; i++ {
		modify(transState, util.Address{i}, i, 99)
		modify(finalState, util.Address{i}, i, 99)
	}
...
func TestCopy(t *testing.T) {
	...
	orig, _ := New(util.Hash{}, NewDatabase(rawdb.NewMemoryDatabase()), nil)
	...
	for i := byte(0); i < 255; i++ {
		...
		ccopyObj := ccopy.GetOrNewStateObject(util.BytesToAddress([]byte{i}))
...
type snapshotTest struct {
	addrs     []util.Address // all account addresses
...
func newTestAction(addr util.Address, r *rand.Rand) testAction {
	actions := []testAction{
		{
			name: "SetBalance",
...
func (test *snapshotTest) checkEqual(state, checkstate *StateDB) error {
	for _, addr := range test.addrs {
		...
		if obj := state.getStateObject(addr); obj != nil {
			state.ForEachStorage(addr, func(key, value util.Hash) bool {
				return checkeq("GetState("+key.Hex()+")", checkstate.GetState(addr, key), value)
			})
			checkstate.ForEachStorage(addr, func(key, value util.Hash) bool {
				return checkeq("GetState("+key.Hex()+")", checkstate.GetState(addr, key), value)
			})
		}
		if err != nil {
...
func TestTouchDelete(t *testing.T) {
	s := newStateTest()
	s.state.GetOrNewStateObject(util.Address{})
	root, _ := s.state.Commit(false)
	s.state, _ = New(root, s.state.db, s.state.snaps)

	snapshot := s.state.Snapshot()
	s.state.AddBalance(util.Address{}, new(big.Int))
...
func TestCopyCommitCopy(t *testing.T) {
	...
	addr := util.HexToAddress("0xaffeaffeaffeaffeaffeaffeaffeaffeaffeaffe")
	skey := util.HexToHash("aaa")
	sval := util.HexToHash("bbb")
	...
	if val := state.GetCommittedState(addr, skey); val != (util.Hash{}) {
		t.Fatalf("initial committed storage slot mismatch: have %x, want %x", val, util.Hash{})
	}
	...
	if val := copyOne.GetCommittedState(addr, skey); val != (util.Hash{}) {
		t.Fatalf("first copy pre-commit committed storage slot mismatch: have %x, want %x", val, util.Hash{})
	}
...
func TestCopyCopyCommitCopy(t *testing.T) {
	...
	if val := copyOne.GetCommittedState(addr, skey); val != (util.Hash{}) {
		t.Fatalf("first copy committed storage slot mismatch: have %x, want %x", val, util.Hash{})
	}
	// Copy the copy and check the balance once more
	copyTwo := copyOne.Copy()
	if balance := copyTwo.GetBalance(addr); balance.Cmp(big.NewInt(42)) != 0 {
	...
	if val := copyTwo.GetCommittedState(addr, skey); val != (util.Hash{}) {
		t.Fatalf("second copy pre-commit committed storage slot mismatch: have %x, want %x", val, util.Hash{})
	}
...
func TestDeleteCreateRevert(t *testing.T) {
	...
	addr := util.BytesToAddress([]byte("so"))
...
func TestMissingTrieNodes(t *testing.T) {
	...
	var root util.Hash
	state, _ := New(util.Hash{}, db, nil)
	addr := util.BytesToAddress([]byte("so"))
	{
		state.SetBalance(addr, big.NewInt(1))
		state.SetCode(addr, []byte{1, 2, 3})
		a2 := util.BytesToAddress([]byte("another"))
...
func TestStateDBAccessList(t *testing.T) {
	// Some helpers
	addr := func(a string) util.Address {
		return util.HexToAddress(a)
	}
	slot := func(a string) util.Hash {
		return util.HexToHash(a)
	}
