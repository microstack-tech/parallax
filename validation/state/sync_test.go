...
func makeTestState() (Database, util.Hash, []*testAccount) {
...
// account array.
func checkStateAccounts(t *testing.T, db dbstore.Database, root util.Hash, accounts []*testAccount) {
...
func checkTrieConsistency(db dbstore.Database, root util.Hash) error {
...
func checkStateConsistency(db dbstore.Database, root util.Hash) error {
...
func testIterativeStateSync(t *testing.T, count int, commit bool, bypath bool) {
	...
	for len(hashQueue)+len(pathQueue) > 0 {
		...
		for i, hash := range hashQueue {
			...
			if err != nil {
				data, err = srcDb.ContractCode(util.Hash{}, hash)
			}
			if err != nil {
...
func TestIterativeDelayedStateSync(t *testing.T) {
	...
	queue := append(append([]util.Hash{}, nodes...), codes...)
...
func testIterativeRandomStateSync(t *testing.T, count int) {
	...
	queue := make(map[util.Hash]struct{})
...
func TestIncompleteStateSync(t *testing.T) {
	...
	isCode := make(map[util.Hash]struct{})
	for _, acc := range srcAccounts {
		if len(acc.code) > 0 {
			isCode[crypto.Keccak256Hash(acc.code)] = struct{}{}
		}
	}
	isCode[util.BytesToHash(emptyCodeHash)] = struct{}{}
	checkTrieConsistency(srcDb.TrieDB().DiskDB().(dbstore.Database), srcRoot)
