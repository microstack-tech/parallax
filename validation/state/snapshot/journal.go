...
func ParseGeneratorStatus(generatorBlob []byte) string {
	...
	if err := rlp.DecodeBytes(generatorBlob, &generator); err != nil {
		logging.Warn("failed to decode snapshot generator", "err", err)
		return ""
	}
	// Figure out whether we're after or within an account
	var m string
	switch marker := generator.Marker; len(marker) {
	case util.HashLength:
		m = fmt.Sprintf("at %#x", marker)
	case 2 * util.HashLength:
		m = fmt.Sprintf("in %#x at %#x", marker[:util.HashLength], marker[util.HashLength:])
	default:
...
func loadAndParseJournal(db dbstore.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	if len(journal) == 0 {
		logging.Warn("Loaded snapshot journal", "diskroot", base.root, "diffs", "missing")
	...
	if !bytes.Equal(root.Bytes(), base.root.Bytes()) {
		logging.Warn("Loaded snapshot journal", "diskroot", base.root, "diffs", "unmatched")
		return base, generator, nil
	}
	...
	logging.Debug("Loaded snapshot journal", "diskroot", base.root, "diffhead", snapshot.Root())
	return snapshot, generator, nil
}

// loadSnapshot loads a pre-existing state snapshot backed by a key-value store.
func loadSnapshot(diskdb dbstore.KeyValueStore, triedb *trie.Database, cache int, root util.Hash, recovery bool) (snapshot, bool, error) {
	...
	if err != nil {
		logging.Warn("Failed to load new-format journal", "error", err)
	...
	if head := snapshot.Root(); head != root {
		...
		// disk layer.
		logging.Warn("Snapshot is not continuous with chain", "snaproot", head, "chainroot", root)
	}
	...
	if !generator.Done {
		...
		go base.generate(&generatorStats{
			...
			storage:  util.StorageSize(generator.Storage),
		})
	}
...
func loadDiffLayer(parent snapshot, r *rlp.Stream) (snapshot, error) {
	...
	destructSet := make(map[util.Hash]struct{})
	...
	storageData := make(map[util.Hash]map[util.Hash][]byte)
...
func (dl *diskLayer) Journal(buffer *bytes.Buffer) (util.Hash, error) {
	...
	if dl.stale {
		return util.Hash{}, ErrSnapshotStale
	}
	// Ensure the generator stats is written even if none was ran this cycle
	journalProgress(dl.diskdb, dl.genMarker, stats)

	logging.Debug("Journalled disk layer", "root", dl.root)
	return dl.root, nil
}

// Journal writes the memory layer contents into a buffer to be stored in the
// database as the snapshot journal.
func (dl *diffLayer) Journal(buffer *bytes.Buffer) (util.Hash, error) {
	...
	for hash, slots := range dl.storageData {
		keys := make([]util.Hash, 0, len(slots))
	...
	logging.Debug("Journalled diff layer", "root", dl.root, "parent", dl.parent.Root())
	return base, nil
}
