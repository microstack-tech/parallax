...
func copyDestructs(destructs map[util.Hash]struct{}) map[util.Hash]struct{} {
	copy := make(map[util.Hash]struct{})
...
func copyAccounts(accounts map[util.Hash][]byte) map[util.Hash][]byte {
	copy := make(map[util.Hash][]byte)
	maps.Copy(copy, accounts)
	return copy
}

func copyStorage(storage map[util.Hash]map[util.Hash][]byte) map[util.Hash]map[util.Hash][]byte {
	copy := make(map[util.Hash]map[util.Hash][]byte)
	for accHash, slots := range storage {
		copy[accHash] = make(map[util.Hash][]byte)
...
func TestMergeBasics(t *testing.T) {
	...
	parent := newDiffLayer(emptyLayer(), util.Hash{}, copyDestructs(destructs), copyAccounts(accounts), copyStorage(storage))
	child := newDiffLayer(parent, util.Hash{}, copyDestructs(destructs), copyAccounts(accounts), copyStorage(storage))
	child = newDiffLayer(child, util.Hash{}, copyDestructs(destructs), copyAccounts(accounts), copyStorage(storage))
	child = newDiffLayer(child, util.Hash{}, copyDestructs(destructs), copyAccounts(accounts), copyStorage(storage))
	child = newDiffLayer(child, util.Hash{}, copyDestructs(destructs), copyAccounts(accounts), copyStorage(storage))
	// And flatten
...
func TestMergeDelete(t *testing.T) {
	storage := make(map[util.Hash]map[util.Hash][]byte)
	// Fill up a parent
	h1 := util.HexToHash("0x01")
	h2 := util.HexToHash("0x02")

	flipDrops := func() map[util.Hash]struct{} {
		return map[util.Hash]struct{}{
			h2: {},
		}
	}
	flipAccs := func() map[util.Hash][]byte {
		return map[util.Hash][]byte{
			h1: randomAccount(),
		}
	}
	flopDrops := func() map[util.Hash]struct{} {
		return map[util.Hash]struct{}{
			h1: {},
		}
	}
	flopAccs := func() map[util.Hash][]byte {
		return map[util.Hash][]byte{
			h2: randomAccount(),
		}
	}
	// Add some flipAccs-flopping layers on top
	parent := newDiffLayer(emptyLayer(), util.Hash{}, flipDrops(), flipAccs(), storage)
	child := parent.Update(util.Hash{}, flopDrops(), flopAccs(), storage)
	child = child.Update(util.Hash{}, flipDrops(), flipAccs(), storage)
	child = child.Update(util.Hash{}, flopDrops(), flopAccs(), storage)
	child = child.Update(util.Hash{}, flipDrops(), flipAccs(), storage)
	child = child.Update(util.Hash{}, flopDrops(), flopAccs(), storage)
	child = child.Update(util.Hash{}, flipDrops(), flipAccs(), storage)
...
func TestInsertAndMerge(t *testing.T) {
	// Fill up a parent
	var (
		acc    = util.HexToHash("0x01")
		slot   = util.HexToHash("0x02")
		parent *diffLayer
		child  *diffLayer
	)
	{
		...
		parent = newDiffLayer(emptyLayer(), util.Hash{}, destructs, accounts, storage)
	}
	{
		var (
			destructs = make(map[util.Hash]struct{})
		...
		storage[acc] = make(map[util.Hash][]byte)
		storage[acc][slot] = []byte{0x01}
		child = newDiffLayer(parent, util.Hash{}, destructs, accounts, storage)
	}
	// And flatten
	merged := (child.flatten()).(*diffLayer)
...
func BenchmarkSearch(b *testing.B) {
	...
	fill := func(parent snapshot) *diffLayer {
		...
		return newDiffLayer(parent, util.Hash{}, destructs, accounts, storage)
	}
	var layer snapshot
	layer = emptyLayer()
	for i := 0; i < 128; i++ {
