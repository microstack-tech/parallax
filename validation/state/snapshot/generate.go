...
func generateSnapshot(diskdb dbstore.KeyValueStore, triedb *trie.Database, cache int, root util.Hash) *diskLayer {
	...
	if err := batch.Write(); err != nil {
		logging.Crit("Failed to write initialized state marker", "err", err)
	}
	base := &diskLayer{
		diskdb:     diskdb,
		triedb:     triedb,
	...
	logging.Debug("Start snapshot generation", "root", root)
	return base
}

// journalProgress persists the generator stats into the database to resume later.
func journalProgress(db dbstore.KeyValueWriter, marker []byte, stats *generatorStats) {
	...
	case bytes.Equal(marker, []byte{}):
		logstr = "empty"
	case len(marker) == util.HashLength:
		logstr = fmt.Sprintf("%#x", marker)
	default:
		logstr = fmt.Sprintf("%#x:%#x", marker[:util.HashLength], marker[util.HashLength:])
	}
	logging.Debug("Journalled generator progress", "progress", logstr)
...
func (dl *diskLayer) proveRange(ctx *generatorContext, root util.Hash, prefix []byte, kind string, origin []byte, max int, valueConvertFn func([]byte) ([]byte, error)) (*proofResult, error) {
	var (
		keys     [][]byte
		vals     [][]byte
	...
	for iter.Next() {
		...
		keys = append(keys, util.CopyBytes(key[len(prefix):]))

		if valueConvertFn == nil {
			vals = append(vals, util.CopyBytes(iter.Value()))
		} else {
			...
			if err != nil {
				...
				// value are aligned.
				vals = append(vals, util.CopyBytes(iter.Value()))
				logging.Error("Failed to convert account state data", "err", err)
			} else {
	...
	if origin == nil {
		origin = util.Hash{}.Bytes()
	}
	if err := tr.Prove(origin, 0, proof); err != nil {
		logging.Debug("Failed to prove range", "kind", kind, "origin", origin, "err", err)
		return &proofResult{
			keys:     keys,
			vals:     vals,
			diskMore: diskMore,
	...
	if last != nil {
		if err := tr.Prove(last, 0, proof); err != nil {
			logging.Debug("Failed to prove range", "kind", kind, "last", last, "err", err)
			return &proofResult{
				keys:     keys,
				vals:     vals,
				diskMore: diskMore,
...
func (dl *diskLayer) generateRange(ctx *generatorContext, root util.Hash, prefix []byte, kind string, origin []byte, max int, onState onStateCallback, valueConvertFn func([]byte) ([]byte, error)) (bool, []byte, error) {
	...
	var snapNodeCache dbstore.KeyValueStore
	if len(result.keys) > 0 {
		snapNodeCache = memorydb.New()
		snapTrieDb := trie.NewDatabase(snapNodeCache)
		snapTrie, _ := trie.New(util.Hash{}, snapTrieDb)
...
func (dl *diskLayer) checkAndFlush(ctx *generatorContext, current []byte) error {
	...
	if ctx.batch.ValueSize() > dbstore.IdealBatchSize || abort != nil {
		if bytes.Compare(current, dl.genMarker) < 0 {
			logging.Error("Snapshot generator went backwards", "current", fmt.Sprintf("%x", current), "genMarker", fmt.Sprintf("%x", dl.genMarker))
		}
...
func generateStorages(ctx *generatorContext, dl *diskLayer, account util.Hash, storageRoot util.Hash, storeMarker []byte) error {
	onStorage := func(key []byte, val []byte, write bool, delete bool) error {
		...
		if delete {
			rawdb.DeleteStorageSnapshot(ctx.batch, account, util.BytesToHash(key))
			snapWipedStorageMeter.Mark(1)
			return nil
		}
		if write {
			rawdb.WriteStorageSnapshot(ctx.batch, account, util.BytesToHash(key), val)
			snapGeneratedStorageMeter.Mark(1)
		} else {
			snapRecoveredStorageMeter.Mark(1)
		}
		ctx.stats.storage += util.StorageSize(1 + 2*util.HashLength + len(val))
		ctx.stats.slots++
	...
	origin := util.CopyBytes(storeMarker)
	for {
...
func generateAccounts(ctx *generatorContext, dl *diskLayer, accMarker []byte) error {
	onAccount := func(key []byte, val []byte, write bool, delete bool) error {
		...
		if accMarker == nil || !bytes.Equal(account[:], accMarker) {
			...
			ctx.stats.storage += util.StorageSize(1 + util.HashLength + dataLen)
			ctx.stats.accounts++
		}
		// If the snap generation goes here after interrupted, genMarker may go backward
		// when last genMarker is consisted of accountHash and storageHash
		marker := account[:]
		if accMarker != nil && bytes.Equal(marker, accMarker) && len(dl.genMarker) > util.HashLength {
			marker = dl.genMarker[:]
		}
		// If we've exceeded our batch allowance or termination was requested, flush to disk
		...
		if acc.Root == emptyRoot {
			...
			if accMarker != nil && bytes.Equal(account[:], accMarker) && len(dl.genMarker) > util.HashLength {
				storeMarker = dl.genMarker[util.HashLength:]
			}
	...
	origin := util.CopyBytes(accMarker)
	for {
...
func (dl *diskLayer) generate(stats *generatorStats) {
	...
	if len(dl.genMarker) > 0 { // []byte{} is the start, use nil for that
		accMarker = dl.genMarker[:util.HashLength]
	}
	...
	if err := ctx.batch.Write(); err != nil {
		logging.Error("Failed to flush batch", "err", err)
	...
	logging.Info("Generated state snapshot", "accounts", stats.accounts, "slots", stats.slots,
		"storage", stats.storage, "dangling", stats.dangling, "elapsed", util.PrettyDuration(time.Since(stats.start)))
