...
type diffLayer struct {
	...
	root  util.Hash // Root hash to which this snapshot diff belongs to
	...
	destructSet map[util.Hash]struct{}             // Keyed markers for deleted (and potentially) recreated accounts
	accountList []util.Hash                        // List of account for iteration. If it exists, it's sorted, otherwise it's nil
	accountData map[util.Hash][]byte               // Keyed accounts for direct retrieval (nil means deleted)
	storageList map[util.Hash][]util.Hash          // List of storage slots for iterated retrievals, one per account. Any existing lists are sorted if non-nil
	storageData map[util.Hash]map[util.Hash][]byte // Keyed storage slots for direct retrieval. one per account (nil means deleted)
...
func newDiffLayer(parent snapshot, root util.Hash, destructs map[util.Hash]struct{}, accounts map[util.Hash][]byte, storage map[util.Hash]map[util.Hash][]byte) *diffLayer {
	...
	dl := &diffLayer{
		...
		storageData: storage,
		storageList: make(map[util.Hash][]util.Hash),
	}
	...
	for accountHash, blob := range accounts {
		...
		dl.memory += uint64(util.HashLength + len(blob))
	...
	for accountHash, slots := range storage {
		...
		for _, data := range slots {
			dl.memory += uint64(util.HashLength + len(data))
			snapshotDirtyStorageWriteMeter.Mark(int64(len(data)))
		}
	}
	dl.memory += uint64(len(destructs) * util.HashLength)
	return dl
}
...
func (dl *diffLayer) Root() util.Hash {
	return dl.root
}
...
func (dl *diffLayer) Account(hash util.Hash) (*Account, error) {
...
func (dl *diffLayer) accountRLP(hash util.Hash, depth int) ([]byte, error) {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
...
func (dl *diffLayer) storage(accountHash, storageHash util.Hash, depth int) ([]byte, error) {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
...
func (dl *diffLayer) Update(blockRoot util.Hash, destructs map[util.Hash]struct{}, accounts map[util.Hash][]byte, storage map[util.Hash]map[util.Hash][]byte) *diffLayer {
...
func (dl *diffLayer) AccountList() []util.Hash {
	...
	dl.accountList = make([]util.Hash, 0, len(dl.destructSet)+len(dl.accountData))
	for hash := range dl.accountData {
	...
	dl.memory += uint64(len(dl.accountList) * util.HashLength)
...
//
// Note, the returned slice is not a copy, so do not modify it.
func (dl *diffLayer) StorageList(accountHash util.Hash) ([]util.Hash, bool) {
	dl.lock.RLock()
	...
	storageList := make([]util.Hash, 0, len(storageMap))
