...
func TestAccountIteratorBasics(t *testing.T) {
	var (
		destructs = make(map[util.Hash]struct{})
		accounts  = make(map[util.Hash][]byte)
		storage   = make(map[util.Hash]map[util.Hash][]byte)
	)
	// Fill up a parent
	for i := 0; i < 100; i++ {
		h := randomHash()
		...
		if rand.Intn(2) == 0 {
			accStorage := make(map[util.Hash][]byte)
			value := make([]byte, 32)
			rand.Read(value)
	...
	diffLayer := newDiffLayer(emptyLayer(), util.Hash{}, copyDestructs(destructs), copyAccounts(accounts), copyStorage(storage))
	it := diffLayer.AccountIterator(util.Hash{})
	verifyIterator(t, 100, it, verifyNothing) // Nil is allowed for single layer iterator

	diskLayer := diffToDisk(diffLayer)
	it = diskLayer.AccountIterator(util.Hash{})
	verifyIterator(t, 100, it, verifyNothing) // Nil is allowed for single layer iterator
}
...
func TestStorageIteratorBasics(t *testing.T) {
	var (
		nilStorage = make(map[util.Hash]int)
		accounts   = make(map[util.Hash][]byte)
		storage    = make(map[util.Hash]map[util.Hash][]byte)
	)
	...
	for i := 0; i < 10; i++ {
		...
		for i := 0; i < 100; i++ {
			rand.Read(value)
			if rand.Intn(2) == 0 {
				accStorage[randomHash()] = util.CopyBytes(value)
			} else {
	...
	// Add some (identical) layers on top
	diffLayer := newDiffLayer(emptyLayer(), util.Hash{}, nil, copyAccounts(accounts), copyStorage(storage))
	for account := range accounts {
		it, _ := diffLayer.StorageIterator(account, util.Hash{})
		verifyIterator(t, 100, it, verifyNothing) // Nil is allowed for single layer iterator
	}

	diskLayer := diffToDisk(diffLayer)
	for account := range accounts {
		it, _ := diskLayer.StorageIterator(account, util.Hash{})
...
func (ti *testIterator) Seek(util.Hash) {
	panic("implement me")
}
...
func (ti *testIterator) Hash() util.Hash {
	return util.BytesToHash([]byte{ti.values[0]})
}
...
func TestAccountIteratorTraversal(t *testing.T) {
	...
	base := &diskLayer{
		...
		root:   util.HexToHash("0x01"),
		cache:  fastcache.New(1024 * 500),
	}
	snaps := &Tree{
		layers: map[util.Hash]snapshot{
			base.root: base,
		},
	}
	// Stack three diff layers on top with various overlaps
	snaps.Update(util.HexToHash("0x02"), util.HexToHash("0x01"), nil,
		randomAccountSet("0xaa", "0xee", "0xff", "0xf0"), nil)

	snaps.Update(util.HexToHash("0x03"), util.HexToHash("0x02"), nil,
		randomAccountSet("0xbb", "0xdd", "0xf0"), nil)

	snaps.Update(util.HexToHash("0x04"), util.HexToHash("0x03"), nil,
		randomAccountSet("0xcc", "0xf0", "0xff"), nil)

	// Verify the single and multi-layer iterators
	head := snaps.Snapshot(util.HexToHash("0x04"))

	verifyIterator(t, 3, head.(snapshot).AccountIterator(util.Hash{}), verifyNothing)
	verifyIterator(t, 7, head.(*diffLayer).newBinaryAccountIterator(), verifyAccount)

	it, _ := snaps.AccountIterator(util.HexToHash("0x04"), util.Hash{})
	...
	snaps.Cap(util.HexToHash("0x04"), 2)
	verifyIterator(t, 7, head.(*diffLayer).newBinaryAccountIterator(), verifyAccount)
...
func TestStorageIteratorTraversal(t *testing.T) {
	...
	diffIter, _ := head.(snapshot).StorageIterator(util.HexToHash("0xaa"), util.Hash{})
	verifyIterator(t, 3, diffIter, verifyNothing)
	verifyIterator(t, 6, head.(*diffLayer).newBinaryStorageIterator(util.HexToHash("0xaa")), verifyStorage)

	it, _ := snaps.StorageIterator(util.HexToHash("0x04"), util.HexToHash("0xaa"), util.Hash{})
...
func TestAccountIteratorTraversalValues(t *testing.T) {
	...
	var (
		a = make(map[util.Hash][]byte)
		b = make(map[util.Hash][]byte)
		c = make(map[util.Hash][]byte)
		d = make(map[util.Hash][]byte)
		e = make(map[util.Hash][]byte)
		f = make(map[util.Hash][]byte)
		g = make(map[util.Hash][]byte)
		h = make(map[util.Hash][]byte)
	)
	for i := byte(2); i < 0xff; i++ {
		a[util.Hash{i}] = []byte(fmt.Sprintf("layer-%d, key %d", 0, i))
		if i > 20 && i%2 == 0 {
			b[util.Hash{i}] = []byte(fmt.Sprintf("layer-%d, key %d", 1, i))
		}
		if i%4 == 0 {
			c[util.Hash{i}] = []byte(fmt.Sprintf("layer-%d, key %d", 2, i))
		}
		if i%7 == 0 {
			d[util.Hash{i}] = []byte(fmt.Sprintf("layer-%d, key %d", 3, i))
		}
		if i%8 == 0 {
			e[util.Hash{i}] = []byte(fmt.Sprintf("layer-%d, key %d", 4, i))
		}
		if i > 50 || i < 85 {
			f[util.Hash{i}] = []byte(fmt.Sprintf("layer-%d, key %d", 5, i))
		}
		if i%64 == 0 {
			g[util.Hash{i}] = []byte(fmt.Sprintf("layer-%d, key %d", 6, i))
		}
		if i%128 == 0 {
			h[util.Hash{i}] = []byte(fmt.Sprintf("layer-%d, key %d", 7, i))
		}
	}
	// Assemble a stack of snapshots from the account layers
	snaps.Update(util.HexToHash("0x02"), util.HexToHash("0x01"), nil, a, nil)
	snaps.Update(util.HexToHash("0x03"), util.HexToHash("0x02"), nil, b, nil)
	snaps.Update(util.HexToHash("0x04"), util.HexToHash("0x03"), nil, c, nil)
	snaps.Update(util.HexToHash("0x05"), util.HexToHash("0x04"), nil, d, nil)
	snaps.Update(util.HexToHash("0x06"), util.HexToHash("0x05"), nil, e, nil)
	snaps.Update(util.HexToHash("0x07"), util.HexToHash("0x06"), nil, f, nil)
	snaps.Update(util.HexToHash("0x08"), util.HexToHash("0x07"), nil, g, nil)
	snaps.Update(util.HexToHash("0x09"), util.HexToHash("0x08"), nil, h, nil)

	it, _ := snaps.AccountIterator(util.HexToHash("0x09"), util.Hash{})
	head := snaps.Snapshot(util.HexToHash("0x09"))
	for it.Next() {
		hash := it.Hash()
	...
	aggregatorMemoryLimit = 0 // Force pushing the bottom-most layer into disk
	snaps.Cap(util.HexToHash("0x09"), 2)
...
func TestStorageIteratorTraversalValues(t *testing.T) {
	...
	wrapStorage := func(storage map[util.Hash][]byte) map[util.Hash]map[util.Hash][]byte {
		return map[util.Hash]map[util.Hash][]byte{
			util.HexToHash("0xaa"): storage,
		}
	}
	...
	// Assemble a stack of snapshots from the account layers
	snaps.Update(util.HexToHash("0x02"), util.HexToHash("0x01"), nil, randomAccountSet("0xaa"), wrapStorage(a))
	snaps.Update(util.HexToHash("0x03"), util.HexToHash("0x02"), nil, randomAccountSet("0xaa"), wrapStorage(b))
	snaps.Update(util.HexToHash("0x04"), util.HexToHash("0x03"), nil, randomAccountSet("0xaa"), wrapStorage(c))
	snaps.Update(util.HexToHash("0x05"), util.HexToHash("0x04"), nil, randomAccountSet("0xaa"), wrapStorage(d))
	snaps.Update(util.HexToHash("0x06"), util.HexToHash("0x05"), nil, randomAccountSet("0xaa"), wrapStorage(e))
	snaps.Update(util.HexToHash("0x07"), util.HexToHash("0x06"), nil, randomAccountSet("0xaa"), wrapStorage(e))
	snaps.Update(util.HexToHash("0x08"), util.HexToHash("0x07"), nil, randomAccountSet("0xaa"), wrapStorage(g))
	snaps.Update(util.HexToHash("0x09"), util.HexToHash("0x08"), nil, randomAccountSet("0xaa"), wrapStorage(h))

	it, _ := snaps.StorageIterator(util.HexToHash("0x09"), util.HexToHash("0xaa"), util.Hash{})
	head := snaps.Snapshot(util.HexToHash("0x09"))
	for it.Next() {
		hash := it.Hash()
		want, err := head.Storage(util.HexToHash("0xaa"), hash)
		if err != nil {
...
func TestAccountIteratorLargeTraversal(t *testing.T) {
	...
	makeAccounts := func(num int) map[util.Hash][]byte {
		accounts := make(map[util.Hash][]byte)
		for i := 0; i < num; i++ {
			h := util.Hash{}
	...
	for i := 1; i < 128; i++ {
		snaps.Update(util.HexToHash(fmt.Sprintf("0x%02x", i+1)), util.HexToHash(fmt.Sprintf("0x%02x", i)), nil, makeAccounts(200), nil)
	}
	// Iterate the entire stack and ensure everything is hit only once
	head := snaps.Snapshot(util.HexToHash("0x80"))
	verifyIterator(t, 200, head.(snapshot).AccountIterator(util.Hash{}), verifyNothing)
	verifyIterator(t, 200, head.(*diffLayer).newBinaryAccountIterator(), verifyAccount)

	it, _ := snaps.AccountIterator(util.HexToHash("0x80"), util.Hash{})
	...
	aggregatorMemoryLimit = 0 // Force pushing the bottom-most layer into disk
	snaps.Cap(util.HexToHash("0x80"), 2)
...
func TestAccountIteratorSeek(t *testing.T) {
	...
	it, _ := snaps.AccountIterator(util.HexToHash("0x02"), util.HexToHash("0xdd"))
	defer it.Release()
	verifyIterator(t, 3, it, verifyAccount) // expected: ee, f0, ff

	it, _ = snaps.AccountIterator(util.HexToHash("0x02"), util.HexToHash("0xaa"))
	defer it.Release()
	verifyIterator(t, 4, it, verifyAccount) // expected: aa, ee, f0, ff

	it, _ = snaps.AccountIterator(util.HexToHash("0x02"), util.HexToHash("0xff"))
	defer it.Release()
	verifyIterator(t, 1, it, verifyAccount) // expected: ff

	it, _ = snaps.AccountIterator(util.HexToHash("0x02"), util.HexToHash("0xff1"))
	defer it.Release()
	verifyIterator(t, 0, it, verifyAccount) // expected: nothing

	it, _ = snaps.AccountIterator(util.HexToHash("0x04"), util.HexToHash("0xbb"))
	defer it.Release()
	verifyIterator(t, 6, it, verifyAccount) // expected: bb, cc, dd, ee, f0, ff

	it, _ = snaps.AccountIterator(util.HexToHash("0x04"), util.HexToHash("0xef"))
	defer it.Release()
	verifyIterator(t, 2, it, verifyAccount) // expected: f0, ff

	it, _ = snaps.AccountIterator(util.HexToHash("0x04"), util.HexToHash("0xf0"))
	defer it.Release()
	verifyIterator(t, 2, it, verifyAccount) // expected: f0, ff

	it, _ = snaps.AccountIterator(util.HexToHash("0x04"), util.HexToHash("0xff"))
	defer it.Release()
	verifyIterator(t, 1, it, verifyAccount) // expected: ff

	it, _ = snaps.AccountIterator(util.HexToHash("0x04"), util.HexToHash("0xff1"))
	defer it.Release()
	verifyIterator(t, 0, it, verifyAccount) // expected: nothing
}
...
func TestStorageIteratorSeek(t *testing.T) {
	...
	// Construct various iterators and ensure their traversal is correct
	it, _ := snaps.StorageIterator(util.HexToHash("0x02"), util.HexToHash("0xaa"), util.HexToHash("0x01"))
	defer it.Release()
	verifyIterator(t, 3, it, verifyStorage) // expected: 01, 03, 05

	it, _ = snaps.StorageIterator(util.HexToHash("0x02"), util.HexToHash("0xaa"), util.HexToHash("0x02"))
	defer it.Release()
	verifyIterator(t, 2, it, verifyStorage) // expected: 03, 05

	it, _ = snaps.StorageIterator(util.HexToHash("0x02"), util.HexToHash("0xaa"), util.HexToHash("0x5"))
	defer it.Release()
	verifyIterator(t, 1, it, verifyStorage) // expected: 05

	it, _ = snaps.StorageIterator(util.HexToHash("0x02"), util.HexToHash("0xaa"), util.HexToHash("0x6"))
	defer it.Release()
	verifyIterator(t, 0, it, verifyStorage) // expected: nothing

	it, _ = snaps.StorageIterator(util.HexToHash("0x04"), util.HexToHash("0xaa"), util.HexToHash("0x01"))
	defer it.Release()
	verifyIterator(t, 6, it, verifyStorage) // expected: 01, 02, 03, 05, 06, 08

	it, _ = snaps.StorageIterator(util.HexToHash("0x04"), util.HexToHash("0xaa"), util.HexToHash("0x05"))
	defer it.Release()
	verifyIterator(t, 3, it, verifyStorage) // expected: 05, 06, 08

	it, _ = snaps.StorageIterator(util.HexToHash("0x04"), util.HexToHash("0xaa"), util.HexToHash("0x08"))
	defer it.Release()
	verifyIterator(t, 1, it, verifyStorage) // expected: 08

	it, _ = snaps.StorageIterator(util.HexToHash("0x04"), util.HexToHash("0xaa"), util.HexToHash("0x09"))
	defer it.Release()
	verifyIterator(t, 0, it, verifyStorage) // expected: nothing
}
...
func TestAccountIteratorDeletions(t *testing.T) {
	...
	// Stack three diff layers on top with various overlaps
	snaps.Update(util.HexToHash("0x02"), util.HexToHash("0x01"),
		nil, randomAccountSet("0x11", "0x22", "0x33"), nil)

	deleted := util.HexToHash("0x22")
	destructed := map[util.Hash]struct{}{
		deleted: {},
	}
	snaps.Update(util.HexToHash("0x03"), util.HexToHash("0x02"),
		destructed, randomAccountSet("0x11", "0x33"), nil)

	snaps.Update(util.HexToHash("0x04"), util.HexToHash("0x03"),
...
func TestStorageIteratorDeletions(t *testing.T) {
	...
	it, _ := snaps.StorageIterator(util.HexToHash("0x03"), util.HexToHash("0xaa"), util.Hash{})
	verifyIterator(t, 4, it, verifyStorage)
	it.Release()

	// The output should be 04,05,06
	it, _ = snaps.StorageIterator(util.HexToHash("0x03"), util.HexToHash("0xaa"), util.HexToHash("0x03"))
	...
	snaps.Update(util.HexToHash("0x04"), util.HexToHash("0x03"), destructed, nil, nil)
	...
	snaps.Update(util.HexToHash("0x05"), util.HexToHash("0x04"), nil,
		randomAccountSet("0xaa"), randomStorageSet([]string{"0xaa"}, [][]string{{"0x07", "0x08", "0x09"}}, nil))

	// The output should be 07,08,09
	it, _ = snaps.StorageIterator(util.HexToHash("0x05"), util.HexToHash("0xaa"), util.Hash{})
	verifyIterator(t, 3, it, verifyStorage)
	it.Release()

	// Destruct the whole storage but re-create the account in the same layer
	snaps.Update(util.HexToHash("0x06"), util.HexToHash("0x05"), destructed, randomAccountSet("0xaa"), randomStorageSet([]string{"0xaa"}, [][]string{{"0x11", "0x12"}}, nil))
	it, _ = snaps.StorageIterator(util.HexToHash("0x06"), util.HexToHash("0xaa"), util.Hash{})
	verifyIterator(t, 2, it, verifyStorage) // The output should be 11,12
	it.Release()

	verifyIterator(t, 2, snaps.Snapshot(util.HexToHash("0x06")).(*diffLayer).newBinaryStorageIterator(util.HexToHash("0xaa")), verifyStorage)
}
...
func BenchmarkAccountIteratorTraversal(b *testing.B) {
	...
	head := snaps.Snapshot(util.HexToHash("0x65"))
	...
	b.Run("fast iterator keys", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			it, _ := snaps.AccountIterator(util.HexToHash("0x65"), util.Hash{})
			defer it.Release()
...
func BenchmarkAccountIteratorLargeBaselayer(b *testing.B) {
	...
	snaps.Update(util.HexToHash("0x02"), util.HexToHash("0x01"), nil, makeAccounts(2000), nil)
	for i := 2; i <= 100; i++ {
		snaps.Update(util.HexToHash(fmt.Sprintf("0x%02x", i+1)), util.HexToHash(fmt.Sprintf("0x%02x", i)), nil, makeAccounts(20), nil)
	}
	// We call this once before the benchmark, so the creation of
	// sorted accountlists are not included in the results.
	head := snaps.Snapshot(util.HexToHash("0x65"))
