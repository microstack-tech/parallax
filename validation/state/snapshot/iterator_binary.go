...
func (dl *diffLayer) initBinaryAccountIterator() Iterator {
	...
	if !ok {
		l := &binaryIterator{
			a:               dl.AccountIterator(util.Hash{}),
			b:               dl.Parent().AccountIterator(util.Hash{}),
...
func (dl *diffLayer) initBinaryStorageIterator(account util.Hash) Iterator {
	parent, ok := dl.parent.(*diffLayer)
	if !ok {
		...
		a, destructed := dl.StorageIterator(account, util.Hash{})
		if destructed {
			l := &binaryIterator{
				a:       a,
				account: account,
		...
		// anymore.
		b, _ := dl.Parent().StorageIterator(account, util.Hash{})
		l := &binaryIterator{
			a:       a,
			b:       b,
			account: account,
...
func (it *binaryIterator) Hash() util.Hash {
	return it.k
}
...
func (dl *diffLayer) newBinaryStorageIterator(account util.Hash) StorageIterator {
