...
func TestGeneration(t *testing.T) {
	...
	stTrie, _ := trie.NewSecure(util.Hash{}, triedb)
	...
	accTrie, _ := trie.NewSecure(util.Hash{}, triedb)
	...
	if have, want := root, util.HexToHash("0xe3712f1a226f3782caca78ca770ccc19ee000552813a9f59d479f8611db9b1fd"); have != want {
...
func hashData(input []byte) util.Hash {
...
func checkSnapRoot(t *testing.T, snap *diskLayer, trieRoot util.Hash) {
	t.Helper()

	accIt := snap.AccountIterator(util.Hash{})
	defer accIt.Release()

	snapRoot, err := generateTrieRoot(nil, accIt, util.Hash{}, stackTrieGenerate,
		func(db dbstore.KeyValueWriter, accountHash, codeHash util.Hash, stat *generateStats) (util.Hash, error) {
			storageIt, _ := snap.StorageIterator(accountHash, util.Hash{})
...
func (t *testHelper) makeStorageTrie(keys []string, vals []string) []byte {
	stTrie, _ := trie.NewSecure(util.Hash{}, t.triedb)
	for i, k := range keys {
...
func (t *testHelper) Generate() (util.Hash, *diskLayer) {
...
func TestGenerateExistentStateWithWrongAccounts(t *testing.T) {
	...
	// Wrong accounts
	{
		...
		helper.addSnapAccount("acc-2", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: util.Hex2Bytes("0x1234")})
	...
	// Extra accounts, only in the snap
	{
		...
		helper.addSnapAccount("acc-5", &Account{Balance: big.NewInt(1), Root: emptyRoot.Bytes(), CodeHash: util.Hex2Bytes("0x1234")}) // Middle
...
func TestGenerateCorruptAccountTrie(t *testing.T) {
	...
	tr, _ := trie.NewSecure(util.Hash{}, triedb)
	...
	triedb.Commit(util.HexToHash("0xa04693ea110a31037fb5ee814308a6f1d76bdab0b11676bdf4541d2de55ba978"), false, nil)
	diskdb.Delete(util.HexToHash("0x65145f923027566669a1ae5ccac66f945b55ff6eaeb17d2ea8e048b7d381f2d7").Bytes())

	snap := generateSnapshot(diskdb, triedb, 16, util.HexToHash("0xa04693ea110a31037fb5ee814308a6f1d76bdab0b11676bdf4541d2de55ba978"))
	select {
	case <-snap.genPending:
		// Snapshot generation succeeded
...
func TestGenerateMissingStorageTrie(t *testing.T) {
	...
	triedb.Reference(
		util.HexToHash("0xddefcd9376dd029653ef384bd2f0a126bb755fe84fdcc9e7cf421ba454f2bc67"),
		util.HexToHash("0x9250573b9c18c664139f3b6a7a8081b7d8f8916a8fcc5d94feec6c29f5fd4e9e"),
	)
	triedb.Reference(
		util.HexToHash("0xddefcd9376dd029653ef384bd2f0a126bb755fe84fdcc9e7cf421ba454f2bc67"),
		util.HexToHash("0x50815097425d000edfc8b3a4a13e175fc2bdcfee8bdfbf2d1ff61041d3c235b2"),
	)
	triedb.Commit(util.HexToHash("0xe3712f1a226f3782caca78ca770ccc19ee000552813a9f59d479f8611db9b1fd"), false, nil)

	// Delete a storage trie root and ensure the generator chokes
	diskdb.Delete(util.HexToHash("0xddefcd9376dd029653ef384bd2f0a126bb755fe84fdcc9e7cf421ba454f2bc67").Bytes())

	snap := generateSnapshot(diskdb, triedb, 16, util.HexToHash("0xe3712f1a226f3782caca78ca770ccc19ee000552813a9f59d479f8611db9b1fd"))
	select {
	case <-snap.genPending:
		// Snapshot generation succeeded
...
func TestGenerateCorruptStorageTrie(t *testing.T) {
	...
	diskdb.Delete(util.HexToHash("0x18a0f4d79cff4459642dd7604f303886ad9d77c30cf3d7d7cedb3a693ab6d371").Bytes())
...
func enableLogging() {
	logging.Root().SetHandler(logging.LvlFilterHandler(logging.LvlTrace, logging.StreamHandler(os.Stderr, logging.TerminalFormat(true))))
}
...
func TestGenerateWithExtraBeforeAndAfter(t *testing.T) {
	...
	accTrie, _ := trie.New(util.Hash{}, triedb)
	{
		acc := &Account{Balance: big.NewInt(1), Root: emptyRoot.Bytes(), CodeHash: emptyCode.Bytes()}
		val, _ := rlp.EncodeToBytes(acc)
		accTrie.Update(util.HexToHash("0x03").Bytes(), val)
		accTrie.Update(util.HexToHash("0x07").Bytes(), val)

		rawdb.WriteAccountSnapshot(diskdb, util.HexToHash("0x01"), val)
		rawdb.WriteAccountSnapshot(diskdb, util.HexToHash("0x02"), val)
		rawdb.WriteAccountSnapshot(diskdb, util.HexToHash("0x03"), val)
		rawdb.WriteAccountSnapshot(diskdb, util.HexToHash("0x04"), val)
		rawdb.WriteAccountSnapshot(diskdb, util.HexToHash("0x05"), val)
		rawdb.WriteAccountSnapshot(diskdb, util.HexToHash("0x06"), val)
		rawdb.WriteAccountSnapshot(diskdb, util.HexToHash("0x07"), val)
	}
...
func TestGenerateWithMalformedSnapdata(t *testing.T) {
	...
	accTrie, _ := trie.New(util.Hash{}, triedb)
	{
		...
		rawdb.WriteAccountSnapshot(diskdb, util.HexToHash("0x02"), junk)
		rawdb.WriteAccountSnapshot(diskdb, util.HexToHash("0x03"), junk)
		rawdb.WriteAccountSnapshot(diskdb, util.HexToHash("0x04"), junk)
		rawdb.WriteAccountSnapshot(diskdb, util.HexToHash("0x05"), junk)
	}
...
func populateDangling(disk dbstore.KeyValueStore) {
	populate := func(accountHash util.Hash, keys []string, vals []string) {
		for i, key := range keys {
			rawdb.WriteStorageSnapshot(disk, accountHash, hashData([]byte(key)), []byte(vals[i]))
		}
	}
	// Dangling storages of the "first" account
	populate(util.Hash{}, []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"})

	// Dangling storages of the "last" account
	populate(util.HexToHash("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"), []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"})

	// Dangling storages around the account 1
	hash := decKey(hashData([]byte("acc-1")).Bytes())
	populate(util.BytesToHash(hash), []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"})
