...
func randomHash() util.Hash {
	var hash util.Hash
	if n, err := rand.Read(hash[:]); n != util.HashLength || err != nil {
		panic(err)
	}
	return hash
}
...
func randomAccountSet(hashes ...string) map[util.Hash][]byte {
	accounts := make(map[util.Hash][]byte)
	for _, hash := range hashes {
		accounts[util.HexToHash(hash)] = randomAccount()
	}
	return accounts
}
...
// the slot addresses.
func randomStorageSet(accounts []string, hashes [][]string, nilStorage [][]string) map[util.Hash]map[util.Hash][]byte {
	storages := make(map[util.Hash]map[util.Hash][]byte)
	for index, account := range accounts {
		storages[util.HexToHash(account)] = make(map[util.Hash][]byte)

		if index < len(hashes) {
			hashes := hashes[index]
			for _, hash := range hashes {
				storages[util.HexToHash(account)][util.HexToHash(hash)] = randomHash().Bytes()
			}
		}
		if index < len(nilStorage) {
			nils := nilStorage[index]
			for _, hash := range nils {
				storages[util.HexToHash(account)][util.HexToHash(hash)] = nil
			}
		}
	}
	return storages
...
func TestDiskLayerExternalInvalidationFullFlatten(t *testing.T) {
	...
	accounts := map[util.Hash][]byte{
		util.HexToHash("0xa1"): randomAccount(),
	}
	if err := snaps.Update(util.HexToHash("0x02"), util.HexToHash("0x01"), nil, accounts, nil); err != nil {
	...
	if err := snaps.Cap(util.HexToHash("0x02"), 0); err != nil {
		t.Fatalf("failed to merge diff layer onto disk: %v", err)
	}
	// Since the base layer was modified, ensure that data retrieval on the external reference fail
	if acc, err := ref.Account(util.HexToHash("0x01")); err != ErrSnapshotStale {
		t.Errorf("stale reference returned account: %#x (err: %v)", acc, err)
	}
	if slot, err := ref.Storage(util.HexToHash("0xa1"), util.HexToHash("0xb1")); err != ErrSnapshotStale {
...
func TestDiskLayerExternalInvalidationPartialFlatten(t *testing.T) {
	...
	if err := snaps.Update(util.HexToHash("0x03"), util.HexToHash("0x02"), nil, accounts, nil); err != nil {
		t.Fatalf("failed to create a diff layer: %v", err)
	}
	...
	if err := snaps.Cap(util.HexToHash("0x03"), 1); err != nil {
...
func TestDiffLayerExternalInvalidationPartialFlatten(t *testing.T) {
	...
	if err := snaps.Update(util.HexToHash("0x04"), util.HexToHash("0x03"), nil, accounts, nil); err != nil {
		t.Fatalf("failed to create a diff layer: %v", err)
	}
	if n := len(snaps.layers); n != 4 {
		t.Errorf("pre-cap layer count mismatch: have %d, want %d", n, 4)
	}
	ref := snaps.Snapshot(util.HexToHash("0x02"))

	// Doing a Cap operation with many allowed layers should be a no-op
	exp := len(snaps.layers)
	if err := snaps.Cap(util.HexToHash("0x04"), 2000); err != nil {
...
func TestPostCapBasicDataAccess(t *testing.T) {
	...
	setAccount := func(accKey string) map[util.Hash][]byte {
		return map[util.Hash][]byte{
			util.HexToHash(accKey): randomAccount(),
		}
	}
	...
	snaps.Update(util.HexToHash("0xa1"), util.HexToHash("0x01"), nil, setAccount("0xa1"), nil)
	snaps.Update(util.HexToHash("0xa2"), util.HexToHash("0xa1"), nil, setAccount("0xa2"), nil)
	snaps.Update(util.HexToHash("0xb2"), util.HexToHash("0xa1"), nil, setAccount("0xb2"), nil)

	snaps.Update(util.HexToHash("0xa3"), util.HexToHash("0xa2"), nil, setAccount("0xa3"), nil)
	snaps.Update(util.HexToHash("0xb3"), util.HexToHash("0xb2"), nil, setAccount("0xb3"), nil)

	// checkExist verifies if an account exiss in a snapshot
	checkExist := func(layer *diffLayer, key string) error {
		if data, _ := layer.Account(util.HexToHash(key)); data == nil {
			return fmt.Errorf("expected %x to exist, got nil", util.HexToHash(key))
		}
		return nil
	}
	// shouldErr checks that an account access errors as expected
	shouldErr := func(layer *diffLayer, key string) error {
		if data, err := layer.Account(util.HexToHash(key)); err == nil {
			return fmt.Errorf("expected error, got data %x", data)
		}
		return nil
	}
	// check basics
	snap := snaps.Snapshot(util.HexToHash("0xb3")).(*diffLayer)
	...
	if err := snaps.Cap(util.HexToHash("0x1337"), 0); err == nil {
		t.Errorf("expected error, got none")
	}
	// Now, merge the a-chain
	snaps.Cap(util.HexToHash("0xa3"), 0)
	...
	// is a disk layer
	if err := snaps.Cap(util.HexToHash("0xa3"), 0); err == nil {
...
func TestSnaphots(t *testing.T) {
	...
	makeRoot := func(height uint64) util.Hash {
		var buffer [8]byte
		binary.BigEndian.PutUint64(buffer[:], height)
		return util.BytesToHash(buffer[:])
	}
	// Create a starting base layer and a snapshot tree out of it
	base := &diskLayer{
		diskdb: rawdb.NewMemoryDatabase(),
	...
	var cases = []struct {
		...
		nodisk       bool
		expected     int
		expectBottom util.Hash
	}{
		{head, 0, false, 0, util.Hash{}},
...
func TestReadStateDuringFlattening(t *testing.T) {
	...
	snap := snaps.Snapshot(util.HexToHash("0xa3"))
	...
	snaps.onFlatten = func() {
		...
		go func() {
			account, _ := snap.Account(util.HexToHash("0xa1"))
			result <- account
		}()
		select {
		case res := <-result:
	...
	snaps.Cap(util.HexToHash("0xa3"), 1)
	select {
	case account := <-result:
		if account == nil {
