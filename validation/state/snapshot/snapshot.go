...
type Snapshot interface {
	...
	Account(hash util.Hash) (*Account, error)

	// AccountRLP directly retrieves the account RLP associated with a particular
	// hash in the snapshot slim data format.
	AccountRLP(hash util.Hash) ([]byte, error)

	// Storage directly retrieves the storage data associated with a particular hash,
	// within a particular account.
	Storage(accountHash, storageHash util.Hash) ([]byte, error)
}
...
type snapshot interface {
	...
	Update(blockRoot util.Hash, destructs map[util.Hash]struct{}, accounts map[util.Hash][]byte, storage map[util.Hash]map[util.Hash][]byte) *diffLayer

	// Journal commits an entire diff hierarchy to disk into a single journal entry.
	// This is meant to be used during shutdown to persist the snapshot without
	// flattening everything down (bad for reorgs).
	Journal(buffer *bytes.Buffer) (util.Hash, error)
	...
	AccountIterator(seek util.Hash) AccountIterator

	// StorageIterator creates a storage iterator over an arbitrary layer.
	StorageIterator(account util.Hash, seek util.Hash) (StorageIterator, bool)
}
...
type Tree struct {
	diskdb dbstore.KeyValueStore  // Persistent database to store the snapshot
	triedb *trie.Database         // In-memory cache to access the trie through
	cache  int                    // Megabytes permitted to use for read caches
	layers map[util.Hash]snapshot // Collection of all known layers
	lock   sync.RWMutex
...
//     a background thread.
func New(diskdb dbstore.KeyValueStore, triedb *trie.Database, cache int, root util.Hash, async bool, rebuild bool, recovery bool) (*Tree, error) {
	// Create a new, empty snapshot tree
	snap := &Tree{
		diskdb: diskdb,
		triedb: triedb,
		cache:  cache,
		layers: make(map[util.Hash]snapshot),
	}
	if !async {
	...
	if disabled {
		logging.Warn("Snapshot maintenance disabled (syncing)")
		return snap, nil
	}
	if err != nil {
		if rebuild {
			logging.Warn("Failed to load snapshot, regenerating", "err", err)
			snap.Rebuild(root)
			return snap, nil
		}
...
func (t *Tree) Disable() {
	...
	t.layers = map[util.Hash]snapshot{}
	...
	if err := batch.Write(); err != nil {
		logging.Crit("Failed to disable snapshots", "err", err)
	}
}

// Snapshot retrieves a snapshot belonging to the given block root, or nil if no
// snapshot is maintained for that block.
func (t *Tree) Snapshot(blockRoot util.Hash) Snapshot {
	t.lock.RLock()
...
func (t *Tree) Snapshots(root util.Hash, limits int, nodisk bool) []Snapshot {
	t.lock.RLock()
	defer t.lock.RUnlock()
...
func (t *Tree) Update(blockRoot util.Hash, parentRoot util.Hash, destructs map[util.Hash]struct{}, accounts map[util.Hash][]byte, storage map[util.Hash]map[util.Hash][]byte) error {
...
func (t *Tree) Cap(root util.Hash, layers int) error {
	...
	if layers == 0 {
		...
		t.layers = map[util.Hash]snapshot{base.root: base}
		return nil
	}
	persisted := t.cap(diff, layers)

	// Remove any layer that is stale or links into a stale layer
	children := make(map[util.Hash][]util.Hash)
	...
	var remove func(root util.Hash)
	remove = func(root util.Hash) {
	...
	if persisted != nil {
		var rebloom func(root util.Hash)
		rebloom = func(root util.Hash) {
...
func diffToDisk(bottom *diffLayer) *diskLayer {
	...
	for hash := range bottom.destructSet {
		...
		for it.Next() {
			...
			if batch.ValueSize() > dbstore.IdealBatchSize {
				if err := batch.Write(); err != nil {
					logging.Crit("Failed to write storage deletions", "err", err)
				}
				batch.Reset()
			}
		}
	...
	for accountHash, storage := range bottom.storageData {
		...
		midAccount := base.genMarker != nil && bytes.Equal(accountHash[:], base.genMarker[:util.HashLength])

		for storageHash, data := range storage {
			// Skip any slot not covered yet by the snapshot
			if midAccount && bytes.Compare(storageHash[:], base.genMarker[util.HashLength:]) > 0 {
				continue
			}
			if len(data) > 0 {
	...
	if err := batch.Write(); err != nil {
		logging.Crit("Failed to write leftover snapshot", "err", err)
	}
	logging.Debug("Journalled disk layer", "root", bottom.root, "complete", base.genMarker == nil)
	res := &diskLayer{
		root:       bottom.root,
		cache:      base.cache,
		diskdb:     base.diskdb,
...
func (t *Tree) Journal(root util.Hash) (util.Hash, error) {
	// Retrieve the head snapshot to journal from var snap snapshot
	snap := t.Snapshot(root)
	if snap == nil {
		return util.Hash{}, fmt.Errorf("snapshot [%#x] missing", root)
	}
	// Run the journaling
	t.lock.Lock()
	defer t.lock.Unlock()
	...
	if diskroot == (util.Hash{}) {
		return util.Hash{}, errors.New("invalid disk root")
	}
...
func (t *Tree) Rebuild(root util.Hash) {
	t.lock.Lock()
	defer t.lock.Unlock()
	...
	logging.Info("Rebuilding state snapshot")
	t.layers = map[util.Hash]snapshot{
...
func (t *Tree) AccountIterator(root util.Hash, seek util.Hash) (AccountIterator, error) {
	ok, err := t.generating()
	if err != nil {
		return nil, err
	}
...
func (t *Tree) StorageIterator(root util.Hash, account util.Hash, seek util.Hash) (StorageIterator, error) {
	ok, err := t.generating()
	if err != nil {
		return nil, err
	}
...
func (t *Tree) Verify(root util.Hash) error {
	acctIt, err := t.AccountIterator(root, util.Hash{})
	if err != nil {
		return err
	}
	defer acctIt.Release()

	got, err := generateTrieRoot(nil, acctIt, util.Hash{}, stackTrieGenerate, func(db dbstore.KeyValueWriter, accountHash, codeHash util.Hash, stat *generateStats) (util.Hash, error) {
		storageIt, err := t.StorageIterator(root, accountHash, util.Hash{})
		if err != nil {
			return util.Hash{}, err
		}
...
// The lock of snapTree is assumed to be held already.
func (t *Tree) diskRoot() util.Hash {
