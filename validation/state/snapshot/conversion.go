...
type (
	...
	trieGeneratorFn func(db dbstore.KeyValueWriter, in chan (trieKV), out chan (util.Hash))

	// leafCallbackFn is the callback invoked at the leaves of the trie,
	// returns the subtrie root with the specified subtrie identifier.
	leafCallbackFn func(db dbstore.KeyValueWriter, accountHash, codeHash util.Hash, stat *generateStats) (util.Hash, error)
)

// GenerateAccountTrieRoot takes an account iterator and reproduces the root hash.
func GenerateAccountTrieRoot(it AccountIterator) (util.Hash, error) {
	return generateTrieRoot(nil, it, util.Hash{}, stackTrieGenerate, nil, newGenerateStats(), true)
}

// GenerateStorageTrieRoot takes a storage iterator and reproduces the root hash.
func GenerateStorageTrieRoot(account util.Hash, it StorageIterator) (util.Hash, error) {
...
func GenerateTrie(snaptree *Tree, root util.Hash, src dbstore.Database, dst dbstore.KeyValueWriter) error {
	// Traverse all state by snapshot, re-generate the whole state trie
	acctIt, err := snaptree.AccountIterator(root, util.Hash{})
	if err != nil {
		return err // The required snapshot might not exist.
	}
	defer acctIt.Release()

	got, err := generateTrieRoot(dst, acctIt, util.Hash{}, stackTrieGenerate, func(dst dbstore.KeyValueWriter, accountHash, codeHash util.Hash, stat *generateStats) (util.Hash, error) {
		// Migrate the code first, commit the contract code into the tmp db.
		if codeHash != emptyCode {
			code := rawdb.ReadCode(src, codeHash)
			if len(code) == 0 {
				return util.Hash{}, errors.New("failed to read contract code")
			}
			rawdb.WriteCode(dst, codeHash, code)
		}
		// Then migrate all storage trie nodes into the tmp db.
		storageIt, err := snaptree.StorageIterator(root, accountHash, util.Hash{})
		if err != nil {
...
type generateStats struct {
	...
	slotsStart map[util.Hash]time.Time // Start time for account slot crawling
	slotsHead  map[util.Hash]util.Hash // Slot head for accounts being crawled
...
func newGenerateStats() *generateStats {
	return &generateStats{
		slotsStart: make(map[util.Hash]time.Time),
		slotsHead:  make(map[util.Hash]util.Hash),
		start:      time.Now(),
	}
}

// progressAccounts updates the generator stats for the account range.
func (stat *generateStats) progressAccounts(account util.Hash, done uint64) {
	stat.lock.Lock()
...
func (stat *generateStats) progressContract(account util.Hash, slot util.Hash, done uint64) {
	stat.lock.Lock()
	defer stat.lock.Unlock()
...
func (stat *generateStats) finishContract(account util.Hash, done uint64) {
	stat.lock.Lock()
	defer stat.lock.Unlock()
...
func (stat *generateStats) report() {
	...
	ctx := []any{
		...
		"slots", stat.slots,
		"elapsed", util.PrettyDuration(time.Since(stat.start)),
	}
	if stat.accounts > 0 {
		...
		if done := binary.BigEndian.Uint64(stat.head[:8]) / stat.accounts; done > 0 {
			...
			ctx = append(ctx, []any{
				"eta", util.PrettyDuration(eta),
			}...)
		}
	}
	logging.Info("Iterating state snapshot", ctx...)
}
...
func (stat *generateStats) reportDone() {
	...
	ctx = append(ctx, []any{"elapsed", util.PrettyDuration(time.Since(stat.start))}...)
	logging.Info("Iterated snapshot", ctx...)
}
...
func generateTrieRoot(db dbstore.KeyValueWriter, it Iterator, account util.Hash, generatorFn trieGeneratorFn, leafCallback leafCallbackFn, stats *generateStats, report bool) (util.Hash, error) {
	var (
		in      = make(chan trieKV)       // chan to pass leaves
		out     = make(chan util.Hash, 1) // chan to collect result
	...
	stop := func(fail error) (util.Hash, error) {
		close(in)
		result := <-out
		for i := 0; i < threads; i++ {
		...
		if account == (util.Hash{}) {
			...
			if leafCallback == nil {
				...
				go func(hash util.Hash) {
					subroot, err := leafCallback(db, hash, util.BytesToHash(account.CodeHash), stats)
					if err != nil {
						results <- err
						return
					}
			...
			leaf = trieKV{it.Hash(), util.CopyBytes(it.(StorageIterator).Slot())}
		}
		in <- leaf
...
func stackTrieGenerate(db dbstore.KeyValueWriter, in chan trieKV, out chan util.Hash) {
