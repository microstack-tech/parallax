...
func TestDiskMerge(t *testing.T) {
	...
	var (
		accNoModNoCache     = util.Hash{0x1}
		accNoModCache       = util.Hash{0x2}
		accModNoCache       = util.Hash{0x3}
		accModCache         = util.Hash{0x4}
		accDelNoCache       = util.Hash{0x5}
		accDelCache         = util.Hash{0x6}
		conNoModNoCache     = util.Hash{0x7}
		conNoModNoCacheSlot = util.Hash{0x70}
		conNoModCache       = util.Hash{0x8}
		conNoModCacheSlot   = util.Hash{0x80}
		conModNoCache       = util.Hash{0x9}
		conModNoCacheSlot   = util.Hash{0x90}
		conModCache         = util.Hash{0xa}
		conModCacheSlot     = util.Hash{0xa0}
		conDelNoCache       = util.Hash{0xb}
		conDelNoCacheSlot   = util.Hash{0xb0}
		conDelCache         = util.Hash{0xc}
		conDelCacheSlot     = util.Hash{0xc0}
		conNukeNoCache      = util.Hash{0xd}
		conNukeNoCacheSlot  = util.Hash{0xd0}
		conNukeCache        = util.Hash{0xe}
		conNukeCacheSlot    = util.Hash{0xe0}
		baseRoot            = randomHash()
		diffRoot            = randomHash()
	)
	...
	if err := snaps.Update(diffRoot, baseRoot, map[util.Hash]struct{}{
		accDelNoCache:  {},
		accDelCache:    {},
		conNukeNoCache: {},
		conNukeCache:   {},
	...
	}, map[util.Hash]map[util.Hash][]byte{
	...
	assertAccount := func(account util.Hash, data []byte) {
		t.Helper()
	...
	assertStorage := func(account util.Hash, slot util.Hash, data []byte) {
		t.Helper()
	...
	assertDatabaseAccount := func(account util.Hash, data []byte) {
		t.Helper()
	...
	assertDatabaseStorage := func(account util.Hash, slot util.Hash, data []byte) {
		t.Helper()
...
func TestDiskPartialMerge(t *testing.T) {
	...
	for i := 0; i < 1024; i++ {
		...
		insertAccount := func(account util.Hash, data []byte) {
		...
		insertStorage := func(account util.Hash, slot util.Hash, data []byte) {
...
func TestDiskGeneratorPersistence(t *testing.T) {
	...
	// Modify or delete some accounts, flatten everything onto disk
	if err := snaps.Update(diffRoot, baseRoot, nil, map[util.Hash][]byte{
		accTwo: accTwo[:],
	}, nil); err != nil {
		t.Fatalf("failed to update snapshot tree: %v", err)
	}
	...
	// Modify or delete some accounts, flatten everything onto disk
	if err := snaps.Update(diffTwoRoot, diffRoot, nil, map[util.Hash][]byte{
...
func TestDiskSeek(t *testing.T) {
	...
	for i, tc := range cases {
		it, err := snaps.AccountIterator(baseRoot, util.Hash{tc.pos})
		if err != nil {
