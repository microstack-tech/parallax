...
type generatorStats struct {
	...
	storage  util.StorageSize // Total account and storage slot size(generation or recovery)
}

// Log creates an contextual log with the given message and the context pulled
// from the internally maintained statistics.
func (gs *generatorStats) Log(msg string, root util.Hash, marker []byte) {
	var ctx []any
	if root != (util.Hash{}) {
	...
	case util.HashLength:
		ctx = append(ctx, []any{"at", util.BytesToHash(marker)}...)
	case 2 * util.HashLength:
		ctx = append(ctx, []any{
			"in", util.BytesToHash(marker[:util.HashLength]),
			"at", util.BytesToHash(marker[util.HashLength:]),
		}...)
	}
	...
	ctx = append(ctx, []any{
		...
		"elapsed", util.PrettyDuration(time.Since(gs.start)),
	}...)
	...
	if len(marker) > 0 {
		if done := binary.BigEndian.Uint64(marker[:8]) - gs.origin; done > 0 {
			...
			ctx = append(ctx, []any{
				"eta", util.PrettyDuration(time.Duration(left/speed) * time.Millisecond),
			}...)
		}
	}
...
type generatorContext struct {
	...
	db      dbstore.KeyValueStore // Key-value store containing the snapshot data
	account *holdableIterator     // Iterator of account snapshot data
	storage *holdableIterator     // Iterator of storage snapshot data
	batch   dbstore.Batch         // Database batch for writing batch data atomically
	logged  time.Time             // The timestamp when last generation progress was displayed
}

// newGeneratorContext initializes the context for generation.
func newGeneratorContext(stats *generatorStats, db dbstore.KeyValueStore, accMarker []byte, storageMarker []byte) *generatorContext {
...
func (ctx *generatorContext) openIterator(kind string, start []byte) {
	if kind == snapAccount {
		...
		ctx.account = newHoldableIterator(rawdb.NewKeyLengthIterator(iter, 1+util.HashLength))
		return
	}
	iter := ctx.db.NewIterator(rawdb.SnapshotStoragePrefix, start)
	ctx.storage = newHoldableIterator(rawdb.NewKeyLengthIterator(iter, 1+2*util.HashLength))
}
...
func (ctx *generatorContext) removeStorageBefore(account util.Hash) {
	var (
		count uint64
		start = time.Now()
		iter  = ctx.storage
	...
	for iter.Next() {
		key := iter.Key()
		if bytes.Compare(key[1:1+util.HashLength], account.Bytes()) >= 0 {
			iter.Hold()
			break
		}
		count++
		ctx.batch.Delete(key)
		if ctx.batch.ValueSize() > dbstore.IdealBatchSize {
			ctx.batch.Write()
			ctx.batch.Reset()
		}
	}
...
func (ctx *generatorContext) removeStorageAt(account util.Hash) error {
	var (
		count int64
		start = time.Now()
		iter  = ctx.storage
	...
	for iter.Next() {
		key := iter.Key()
		cmp := bytes.Compare(key[1:1+util.HashLength], account.Bytes())
		if cmp < 0 {
