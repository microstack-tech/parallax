...
type diffAccountIterator struct {
	...
	keys  []util.Hash // Keys left in the layer to iterate
	fail  error       // Any failures encountered (stale)
}

// AccountIterator creates an account iterator over a single diff layer.
func (dl *diffLayer) AccountIterator(seek util.Hash) AccountIterator {
...
func (it *diffAccountIterator) Hash() util.Hash {
	return it.curHash
}
...
func (dl *diskLayer) AccountIterator(seek util.Hash) AccountIterator {
	pos := util.TrimRightZeroes(seek[:])
...
func (it *diskAccountIterator) Next() bool {
	...
	for {
		...
		if len(it.it.Key()) == len(rawdb.SnapshotAccountPrefix)+util.HashLength {
			break
		}
	}
	return true
...
// Hash returns the hash of the account the iterator is currently at.
func (it *diskAccountIterator) Hash() util.Hash {
	return util.BytesToHash(it.it.Key()) // The prefix will be truncated
}
...
func (dl *diffLayer) StorageIterator(account util.Hash, seek util.Hash) (StorageIterator, bool) {
...
func (it *diffStorageIterator) Hash() util.Hash {
	return it.curHash
}
...
// is always false.
func (dl *diskLayer) StorageIterator(account util.Hash, seek util.Hash) (StorageIterator, bool) {
	pos := util.TrimRightZeroes(seek[:])
...
func (it *diskStorageIterator) Next() bool {
	...
	for {
		...
		if len(it.it.Key()) == len(rawdb.SnapshotStoragePrefix)+util.HashLength+util.HashLength {
			break
		}
	}
	return true
...
// Hash returns the hash of the storage slot the iterator is currently at.
func (it *diskStorageIterator) Hash() util.Hash {
	return util.BytesToHash(it.it.Key()) // The prefix will be truncated
}
