...
func NewPruner(db dbstore.Database, datadir, trieCachePath string, bloomSize uint64) (*Pruner, error) {
	...
	if bloomSize < 256 {
		logging.Warn("Sanitizing bloomfilter size", "provided(MB)", bloomSize, "updated(MB)", 256)
		bloomSize = 256
	}
...
func prune(snaptree *snapshot.Tree, root util.Hash, maindb dbstore.Database, stateBloom *stateBloom, bloomPath string, middleStateRoots map[util.Hash]struct{}, start time.Time) error {
	...
	for iter.Next() {
		...
		if len(key) == util.HashLength || isCode {
			checkKey := key
			if isCode {
				checkKey = codeKey
			}
			if _, exist := middleStateRoots[util.BytesToHash(checkKey)]; exist {
				logging.Debug("Forcibly delete the middle state roots", "hash", util.BytesToHash(checkKey))
			} else {
			...
			count += 1
			size += util.StorageSize(len(key) + len(iter.Value()))
			batch.Delete(key)
			...
			if time.Since(logged) > 8*time.Second {
				logging.Info("Pruning state data", "nodes", count, "size", size,
					"elapsed", util.PrettyDuration(time.Since(pstart)), "eta", util.PrettyDuration(eta))
				logged = time.Now()
			}
	...
	iter.Release()
	logging.Info("Pruned state data", "nodes", count, "size", size, "elapsed", util.PrettyDuration(time.Since(pstart)))
	...
	if count >= rangeCompactionThreshold {
		...
		for b := 0x00; b <= 0xf0; b += 0x10 {
			...
			logging.Info("Compacting database", "range", fmt.Sprintf("%#x-%#x", start, end), "elapsed", util.PrettyDuration(time.Since(cstart)))
			if err := maindb.Compact(start, end); err != nil {
				logging.Error("Database compaction failed", "error", err)
				return err
			}
		}
		logging.Info("Database compaction finished", "elapsed", util.PrettyDuration(time.Since(cstart)))
	}
	logging.Info("State pruning successful", "pruned", size, "elapsed", util.PrettyDuration(time.Since(start)))
	return nil
}
...
func (p *Pruner) Prune(root util.Hash) error {
	...
	if !rawdb.HasTrieNode(p.db, root) {
		...
		for i := len(layers) - 2; i >= 2; i-- {
			if rawdb.HasTrieNode(p.db, layers[i].Root()) {
				root = layers[i].Root()
				found = true
				logging.Info("Selecting middle-layer as the pruning target", "root", root, "depth", i)
				break
			}
		}
		if !found {
		...
		if len(layers) > 0 {
			logging.Info("Selecting bottom-most difflayer as the pruning target", "root", root, "height", p.headHeader.Number.Uint64()-127)
		} else {
			logging.Info("Selecting user-specified state as the pruning target", "root", root)
		}
	}
	...
	middleRoots := make(map[util.Hash]struct{})
	...
	logging.Info("Writing state bloom to disk", "name", filterName)
	if err := p.stateBloom.Commit(filterName, filterName+stateBloomFileTempSuffix); err != nil {
		return err
	}
	logging.Info("State bloom filter committed", "name", filterName)
...
// in the disk.
func RecoverPruning(datadir string, db dbstore.Database, trieCachePath string) error {
	...
	logging.Info("Loaded state bloom filter", "path", stateBloomPath)
	...
	if !found {
		logging.Error("Pruning target state is not existent")
...
func extractGenesis(db dbstore.Database, stateBloom *stateBloom) error {
	genesisHash := rawdb.ReadCanonicalHash(db, 0)
	if genesisHash == (util.Hash{}) {
...
func bloomFilterName(datadir string, hash util.Hash) string {
...
func isBloomFilter(filename string) (bool, util.Hash) {
	...
	middle, ok := strings.CutPrefix(filename, stateBloomFilePrefix+".")
	if !ok {
		return false, util.Hash{}
	}
	middle, ok = strings.CutSuffix(middle, "."+stateBloomFileSuffix)
	if !ok {
		return false, util.Hash{}
	}
	// Only accept names that bloomFilterName itself would have produced:
	// util.HexToHash accepts arbitrary strings, so require the exact
	// canonical spelling round-trips, rejecting stray near-miss files whose
	// mangled hash would otherwise be trusted as the pruning root.
	hash := util.HexToHash(middle)
	if middle != hash.Hex() {
		return false, util.Hash{}
	}
	return true, hash
...
func findBloomFilter(datadir string) (string, util.Hash, error) {
	var (
		stateBloomPath string
		stateBloomRoot util.Hash
	)
...
func deleteCleanTrieCache(path string) {
	if !util.FileExist(path) {
		logging.Warn(warningLog)
		return
	}
	os.RemoveAll(path)
	logging.Info("Deleted trie clean cache", "path", path)
}
