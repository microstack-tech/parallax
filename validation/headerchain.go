...
type HeaderChain struct {
	...
	currentHeaderHash util.Hash    // Hash of the current head of the header chain (prevent recomputing all the time)
...
func NewHeaderChain(chainDb dbstore.Database, config *chainparams.ChainConfig, engine kernel.Engine, procInterrupt func() bool) (*HeaderChain, error) {
	...
	if head := rawdb.ReadHeadBlockHash(chainDb); head != (util.Hash{}) {
...
func (hc *HeaderChain) GetBlockNumber(hash util.Hash) *uint64 {
...
func (hc *HeaderChain) WriteHeaders(headers []*types.Header) (int, error) {
	...
	// Commit to disk!
	if err := batch.Write(); err != nil {
		logging.Crit("Failed to write headers", "error", err)
	}
...
func (hc *HeaderChain) ValidateHeaderChain(chain []*types.Header, checkFreq int) (int, error) {
	...
	for i := 1; i < len(chain); i++ {
		if chain[i].Number.Uint64() != chain[i-1].Number.Uint64()+1 {
			...
			logging.Error("Non contiguous header insert", "number", chain[i].Number, "hash", hash,
...
func (hc *HeaderChain) GetAncestor(hash util.Hash, number, ancestor uint64, maxNonCanonical *uint64) (util.Hash, uint64) {
	if ancestor > number {
		return util.Hash{}, 0
	}
	if ancestor == 1 {
...
func (hc *HeaderChain) GetTd(hash util.Hash, number uint64) *big.Int {
...
// caching it if found.
func (hc *HeaderChain) GetHeader(hash util.Hash, number uint64) *types.Header {
...
// found.
func (hc *HeaderChain) GetHeaderByHash(hash util.Hash) *types.Header {
...
func (hc *HeaderChain) HasHeader(hash util.Hash, number uint64) bool {
...
func (hc *HeaderChain) GetCanonicalHash(number uint64) util.Hash {
...
type (
	...
	UpdateHeadBlocksCallback func(dbstore.KeyValueWriter, *types.Header) (uint64, bool)

	// DeleteBlockContentCallback is a callback function that is called by SetHead
	// before each header is deleted.
	DeleteBlockContentCallback func(dbstore.KeyValueWriter, util.Hash, uint64)
)
...
func (hc *HeaderChain) SetHead(head uint64, updateFn UpdateHeadBlocksCallback, delFn DeleteBlockContentCallback) {
	...
	for hdr := hc.CurrentHeader(); hdr != nil && hdr.Number.Uint64() > head; hdr = hc.CurrentHeader() {
		...
		if updateFn != nil {
			...
			if force && newHead < head {
				logging.Warn("Force rewinding till ancient limit", "head", newHead)
				head = newHead
			}
		}
		...
		if err := markerBatch.Write(); err != nil {
			logging.Crit("Failed to update chain markers", "error", err)
		}
	...
	if err := batch.Write(); err != nil {
		logging.Crit("Failed to rewind block", "error", err)
	}
...
func (hc *HeaderChain) GetBlock(hash util.Hash, number uint64) *types.Block {
	return nil
}
