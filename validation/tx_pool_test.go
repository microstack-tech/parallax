...
var (
	...
	eip1559Config *chainparams.ChainConfig
)
...
func init() {
	...
	cpy := *chainparams.TestChainConfig
	eip1559Config = &cpy
	eip1559Config.BerlinBlock = util.Big0
	eip1559Config.LondonBlock = util.Big0
}
...
func pricedTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *ecdsa.PrivateKey) *types.Transaction {
	tx, _ := types.SignTx(types.NewTransaction(nonce, util.Address{}, big.NewInt(100), gaslimit, gasprice, nil), types.HomesteadSigner{}, key)
	return tx
}
...
func pricedDataTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *ecdsa.PrivateKey, bytes uint64) *types.Transaction {
	...
	tx, _ := types.SignTx(types.NewTransaction(nonce, util.Address{}, big.NewInt(0), gaslimit, gasprice, data), types.HomesteadSigner{}, key)
	return tx
}

func dynamicFeeTx(nonce uint64, gaslimit uint64, gasFee *big.Int, tip *big.Int, key *ecdsa.PrivateKey) *types.Transaction {
	tx, _ := types.SignNewTx(key, types.LatestSignerForChainID(chainparams.TestChainConfig.ChainID), &types.DynamicFeeTx{
...
func setupTxPool() (*TxPool, *ecdsa.PrivateKey) {
	return setupTxPoolWithConfig(chainparams.TestChainConfig)
}

func setupTxPoolWithConfig(config *chainparams.ChainConfig) (*TxPool, *ecdsa.PrivateKey) {
...
func deriveSender(tx *types.Transaction) (util.Address, error) {
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	stdb := c.statedb
	if *c.trigger {
		c.statedb, _ = state.New(util.Hash{}, state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
		// simulate that the new head block included tx0 and tx1
		c.statedb.SetNonce(c.address, 2)
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(chainparams.Ether))
		*c.trigger = false
	}
	return stdb, nil
}
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(chainparams.Ether))
	blockchain := &testChain{&testBlockChain{1000000000, statedb, new(event.Feed)}, address, &trigger}

	tx0 := transaction(0, 100000, key)
	tx1 := transaction(1, 100000, key)

	pool := NewTxPool(testTxPoolConfig, chainparams.TestChainConfig, blockchain)
	defer pool.Stop()
...
func testAddBalance(pool *TxPool, addr util.Address, amount *big.Int) {
	pool.mu.Lock()
	pool.currentState.AddBalance(addr, amount)
	pool.mu.Unlock()
}

func testSetNonce(pool *TxPool, addr util.Address, nonce uint64) {
	pool.mu.Lock()
...
func TestTransactionQueue2(t *testing.T) {
	...
	pool.promoteExecutables([]util.Address{from})
	if len(pool.pending) != 1 {
...
func TestTransactionNegativeValue(t *testing.T) {
	...
	tx, _ := types.SignTx(types.NewTransaction(0, util.Address{}, big.NewInt(-1), 100, big.NewInt(1), nil), types.HomesteadSigner{}, key)
	from, _ := deriveSender(tx)
	testAddBalance(pool, from, big.NewInt(1))
...
func TestTransactionDoubleNonce(t *testing.T) {
	...
	tx1, _ := types.SignTx(types.NewTransaction(0, util.Address{}, big.NewInt(100), 100000, big.NewInt(1), nil), signer, key)
	tx2, _ := types.SignTx(types.NewTransaction(0, util.Address{}, big.NewInt(100), 1000000, big.NewInt(2), nil), signer, key)
	tx3, _ := types.SignTx(types.NewTransaction(0, util.Address{}, big.NewInt(100), 1000000, big.NewInt(1), nil), signer, key)
...
func TestTransactionPostponing(t *testing.T) {
	...
	accs := make([]util.Address, len(keys))
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	nonces := make(map[util.Address]uint64)
