...
type request struct {
	...
	hash util.Hash // Hash of the node data content to retrieve
...
type SyncResult struct {
	Hash util.Hash // Hash of the originally unknown trie node
...
type syncMemBatch struct {
	nodes map[util.Hash][]byte // In-memory membatch of recently completed nodes
	codes map[util.Hash][]byte // In-memory membatch of recently completed codes
}

// newSyncMemBatch allocates a new memory-buffer for not-yet persisted trie nodes.
func newSyncMemBatch() *syncMemBatch {
	return &syncMemBatch{
		nodes: make(map[util.Hash][]byte),
		codes: make(map[util.Hash][]byte),
	}
}

// hasNode reports the trie node with specific hash is already cached.
func (batch *syncMemBatch) hasNode(hash util.Hash) bool {
	_, ok := batch.nodes[hash]
	return ok
}

// hasCode reports the contract code with specific hash is already cached.
func (batch *syncMemBatch) hasCode(hash util.Hash) bool {
...
type Sync struct {
	database dbstore.KeyValueReader // Persistent database to check for existing entries
	membatch *syncMemBatch          // Memory buffer to avoid frequent database writes
	nodeReqs map[util.Hash]*request // Pending requests pertaining to a trie node hash
	codeReqs map[util.Hash]*request // Pending requests pertaining to a code hash
	queue    *prque.Prque           // Priority queue with the pending requests
	fetches  map[int]int            // Number of active fetches per trie node depth
}

// NewSync creates a new trie data download scheduler.
func NewSync(root util.Hash, database dbstore.KeyValueReader, callback LeafCallback) *Sync {
	ts := &Sync{
		database: database,
		membatch: newSyncMemBatch(),
		nodeReqs: make(map[util.Hash]*request),
		codeReqs: make(map[util.Hash]*request),
		queue:    prque.New(nil),
		fetches:  make(map[int]int),
	}
	ts.AddSubTrie(root, nil, util.Hash{}, callback)
	return ts
}

// AddSubTrie registers a new trie to the sync code, rooted at the designated parent.
func (s *Sync) AddSubTrie(root util.Hash, path []byte, parent util.Hash, callback LeafCallback) {
...
// as is.
func (s *Sync) AddCodeEntry(hash util.Hash, path []byte, parent util.Hash) {
...
func (s *Sync) Missing(max int) (nodes []util.Hash, paths []SyncPath, codes []util.Hash) {
	var (
		nodeHashes []util.Hash
		nodePaths  []SyncPath
		codeHashes []util.Hash
	)
...
func (s *Sync) Commit(dbw dbstore.Batch) error {
	...
	for _, child := range children {
		...
		if req.callback != nil {
			if node, ok := (child.node).(valueNode); ok {
				var paths [][]byte
				if len(child.path) == 2*util.HashLength {
					paths = append(paths, hexToKeybytes(child.path))
				} else if len(child.path) == 4*util.HashLength {
					paths = append(paths, hexToKeybytes(child.path[:2*util.HashLength]))
					paths = append(paths, hexToKeybytes(child.path[2*util.HashLength:]))
				}
