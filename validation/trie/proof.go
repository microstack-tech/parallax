...
func (t *Trie) Prove(key []byte, fromLevel uint, proofDb dbstore.KeyValueWriter) error {
...
// nodes of the longest existing prefix of the key (at least the root node), ending
// with the node that proves the absence of the key.
func (t *SecureTrie) Prove(key []byte, fromLevel uint, proofDb dbstore.KeyValueWriter) error {
...
func VerifyProof(rootHash util.Hash, key []byte, proofDb dbstore.KeyValueReader) (value []byte, err error) {
	key = keybytesToHex(key)
	wantHash := rootHash
	for i := 0; ; i++ {
...
func proofToPath(rootHash util.Hash, root node, key []byte, proofDb dbstore.KeyValueReader, allowNonExistent bool) (node, []byte, error) {
	// resolveNode retrieves and resolves trie node from merkle proof stream
	resolveNode := func(hash util.Hash) (node, error) {
	...
	for {
		...
		case *fullNode:
			...
			continue
		case hashNode:
			child, err = resolveNode(util.BytesToHash(cld))
			if err != nil {
				return nil, nil, err
			}
		case valueNode:
...
func VerifyRangeProof(rootHash util.Hash, firstKey []byte, lastKey []byte, keys [][]byte, values [][]byte, proof dbstore.KeyValueReader) (bool, error) {
