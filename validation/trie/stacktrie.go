...
func stackTrieFromPool(db dbstore.KeyValueWriter) *StackTrie {
...
func NewStackTrie(db dbstore.KeyValueWriter) *StackTrie {
	return &StackTrie{
		nodeType: emptyNode,
		db:       db,
	}
...
func NewFromBinary(data []byte, db dbstore.KeyValueWriter) (*StackTrie, error) {
	var st StackTrie
...
func (st *StackTrie) setDb(db dbstore.KeyValueWriter) {
	st.db = db
	for _, child := range st.children {
		if child != nil {
			child.setDb(db)
...
func newLeaf(key, val []byte, db dbstore.KeyValueWriter) *StackTrie {
...
func newExt(key []byte, child *StackTrie, db dbstore.KeyValueWriter) *StackTrie {
	st := stackTrieFromPool(db)
	st.nodeType = extNode
	st.key = append(st.key, key...)
	st.children[0] = child
...
func (st *StackTrie) hashRec(hasher *hasher) {
	...
	if len(encodedNode) < 32 {
		st.val = util.CopyBytes(encodedNode)
		return
	}
...
func (st *StackTrie) Hash() (h util.Hash) {
...
func (st *StackTrie) Commit() (h util.Hash, err error) {
	if st.db == nil {
		return util.Hash{}, ErrCommitDisabled
	}
