...
func TestNodeIteratorCoverage(t *testing.T) {
	...
	hashes := make(map[util.Hash]struct{})
...
func TestIteratorNoDups(t *testing.T) {
	tr, _ := New(util.Hash{}, NewDatabase(rawdb.NewMemoryDatabase()))
	for _, val := range testdata1 {
...
func (l *loggingDb) NewBatch() dbstore.Batch {
	return l.backend.NewBatch()
}

func (l *loggingDb) NewBatchWithSize(size int) dbstore.Batch {
	return l.backend.NewBatchWithSize(size)
}

func (l *loggingDb) NewIterator(prefix []byte, start []byte) dbstore.Iterator {
	return l.backend.NewIterator(prefix, start)
}

func (l *loggingDb) NewSnapshot() (dbstore.Snapshot, error) {
...
func makeLargeTestTrie() (*Database, *SecureTrie, *loggingDb) {
	...
	trie, _ := NewSecure(util.Hash{}, triedb)
...
func TestNodeIteratorLargeTrie(t *testing.T) {
	...
	// Do a seek operation
	trie.NodeIterator(util.FromHex("0x77667766776677766778855885885885"))
...
func TestIteratorNodeBlob(t *testing.T) {
	...
	var count int
	for dbIter.Next() {
		got, present := found[util.BytesToHash(dbIter.Key())]
		if !present {
