...
func TestStackTrieInsertAndHash(t *testing.T) {
	...
	for i, test := range tests {
		...
		for l := 1; l <= len(test); l++ {
			...
			for j := 0; j < l; j++ {
				kv := &test[j]
				if err := st.TryUpdate(util.FromHex(kv.K), []byte(kv.V)); err != nil {
					t.Fatal(err)
				}
			}
			expected := util.HexToHash(test[l-1].H)
...
func TestSizeBug(t *testing.T) {
	st := NewStackTrie(nil)
	nt, _ := New(util.Hash{}, NewDatabase(memorydb.New()))

	leaf := util.FromHex("290decd9548b62a8d60345a988386fc84ba6bc95484008f6362f93160ef3e563")
	value := util.FromHex("94cf40d0d2b44f2b66e07cace1372ca42b73cf21a3")
...
func TestEmptyBug(t *testing.T) {
	...
	for _, kv := range kvs {
		nt.TryUpdate(util.FromHex(kv.K), util.FromHex(kv.V))
		st.TryUpdate(util.FromHex(kv.K), util.FromHex(kv.V))
	}
...
func TestStacktrieNotModifyValues(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		key := util.BigToHash(keyB)
		value := getValue(i)
		st.TryUpdate(key.Bytes(), value)
		vals = append(vals, value)
		keyB = keyB.Add(keyB, keyDelta)
		keyDelta.Add(keyDelta, util.Big1)
	}
	st.Hash()
	for i := 0; i < 1000; i++ {
		want := getValue(i)
...
func TestStacktrieSerialization(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		keys = append(keys, util.BigToHash(keyB).Bytes())
		keyB = keyB.Add(keyB, keyDelta)
		keyDelta.Add(keyDelta, util.Big1)
	}
	for i, k := range keys {
		nt.TryUpdate(k, util.CopyBytes(vals[i]))
	}
	...
	for i, k := range keys {
		...
		st = newSt
		st.TryUpdate(k, util.CopyBytes(vals[i]))
	}
