...
type NodeIterator interface {
	...
	AddResolver(dbstore.KeyValueReader)
}

// nodeIteratorState represents the iteration state at one particular node of the
// trie, which can be resumed at a later invocation.
type nodeIteratorState struct {
	hash    util.Hash // Hash of the node being iterated (nil if not standalone)
	node    node      // Trie node being iterated
	parent  util.Hash // Hash of the first full ancestor node (nil if current is the root)
...
type nodeIterator struct {
	...
	resolver dbstore.KeyValueReader // Optional intermediate resolver above the disk layer
}
...
func (it *nodeIterator) AddResolver(resolver dbstore.KeyValueReader) {
	it.resolver = resolver
}

func (it *nodeIterator) Hash() util.Hash {
	if len(it.stack) == 0 {
		return util.Hash{}
	}
...
func (it *nodeIterator) Parent() util.Hash {
	if len(it.stack) == 0 {
		return util.Hash{}
	}
...
func findChild(n *fullNode, index int, path []byte, ancestor util.Hash) (node, *nodeIteratorState, []byte, int) {
	var (
		child     node
...
func (it *nodeIterator) nextChild(parent *nodeIteratorState, ancestor util.Hash) (*nodeIteratorState, []byte, bool) {
...
func (it *nodeIterator) nextChildAt(parent *nodeIteratorState, ancestor util.Hash, key []byte) (*nodeIteratorState, []byte, bool) {
...
func (it *differenceIterator) Hash() util.Hash {
	return it.b.Hash()
}

func (it *differenceIterator) Parent() util.Hash {
	return it.b.Parent()
}
...
func (it *differenceIterator) AddResolver(resolver dbstore.KeyValueReader) {
...
func (it *unionIterator) Hash() util.Hash {
	return (*it.items)[0].Hash()
}

func (it *unionIterator) Parent() util.Hash {
...
func (it *unionIterator) AddResolver(resolver dbstore.KeyValueReader) {
	panic("not implemented")
}
...
func (it *unionIterator) Next(descend bool) bool {
	...
	for len(*it.items) > 0 && ((!descend && bytes.HasPrefix((*it.items)[0].Path(), least.Path())) || compareNodes(least, (*it.items)[0]) == 0) {
		...
		if skipped.Next(skipped.Hash() == util.Hash{}) {
			it.count++
