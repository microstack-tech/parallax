...
type LeafCallback func(paths [][]byte, hexpath []byte, leaf []byte, parent util.Hash) error
...
func New(root util.Hash, db *Database) (*Trie, error) {
	if db == nil {
	...
	if root != (util.Hash{}) && root != emptyRoot {
...
func (t *Trie) tryGetNode(origNode node, path []byte, pos int) (item []byte, newnode node, resolved int, err error) {
	...
	if pos >= len(path) {
		...
		blob, err := t.db.Node(util.BytesToHash(hash))
...
func (t *Trie) Commit(onleaf LeafCallback) (util.Hash, int, error) {
	if t.db == nil {
