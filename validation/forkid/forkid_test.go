...
func TestCreation(t *testing.T) {
	...
	tests := []struct {
		...
		// Mainnet test cases
		{
			chainparams.MainnetChainConfig,
			chainparams.MainnetGenesisHash,
			[]testcase{
		...
		// Testnet test cases
		{
			chainparams.TestnetChainConfig,
			chainparams.TestnetGenesisHash,
			[]testcase{
...
func TestValidation(t *testing.T) {
	...
	for i, tt := range tests {
		filter := newFilter(chainparams.MainnetChainConfig, chainparams.MainnetGenesisHash, func() uint64 { return tt.head })
...
func TestEncoding(t *testing.T) {
	tests := []struct {
		...
		{ID{Hash: checksumToBytes(0), Next: 0}, util.Hex2Bytes("c6840000000080")},
		{ID{Hash: checksumToBytes(0xdeadbeef), Next: 0xBADDCAFE}, util.Hex2Bytes("ca84deadbeef84baddcafe,")},
		{ID{Hash: checksumToBytes(math.MaxUint32), Next: math.MaxUint64}, util.Hex2Bytes("ce84ffffffff88ffffffffffffffff")},
	}
	for i, tt := range tests {
