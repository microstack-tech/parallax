...
func (c *Compiler) compileElement(element token) error {
	...
	if isJump(element.text) {
		...
		c.pushBin(script.OpCode(int(script.PUSH1) - 1 + len(value)))
		c.pushBin(value)
	} else {
		c.pushBin(toBinary(element.text))
	}
...
func toBinary(text string) script.OpCode {
	return script.StringToOp(strings.ToUpper(text))
}
