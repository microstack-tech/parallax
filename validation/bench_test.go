...
var (
	...
	ringAddrs = make([]util.Address, len(ringKeys))
)
...
func genTxRing(naccounts int) func(int, *BlockGen) {
	...
	return func(i int, gen *BlockGen) {
		...
		for {
			...
			if gas < chainparams.TxGas {
				break
			}
			to := (from + 1) % naccounts
			burn := new(big.Int).SetUint64(chainparams.TxGas)
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	chainman, _ := NewBlockChain(db, nil, gspec.Config, xhash.NewFaker(), script.Config{}, nil, nil)
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
// into a database.
func makeChainForBench(db dbstore.Database, full bool, count uint64) {
	var hash util.Hash
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, &cacheConfig, chainparams.TestChainConfig, xhash.NewFaker(), script.Config{}, nil, nil)
		if err != nil {
