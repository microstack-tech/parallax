...
func TestStateProcessorErrors(t *testing.T) {
	...
	makeTx := func(key *ecdsa.PrivateKey, nonce uint64, to util.Address, amount *big.Int, gasLimit uint64, gasPrice *big.Int, data []byte) *types.Transaction {
		tx, _ := types.SignTx(types.NewTransaction(nonce, to, amount, gasLimit, gasPrice, data), signer, key)
		return tx
	}
	mkDynamicTx := func(nonce uint64, to util.Address, gasLimit uint64, gasTipCap, gasFeeCap *big.Int) *types.Transaction {
	...
	{ // Tests against a 'recent' chain definition
		var (
			...
			gspec = &Genesis{
				Config: config,
				Alloc: GenesisAlloc{
					util.HexToAddress("0x71562b71999873DB5b286dF957af199Ec94617F7"): GenesisAccount{
						Balance: big.NewInt(1000000000000000000), // 1 lax
						Nonce:   0,
					},
					util.HexToAddress("0xfd0810DD14796680f72adf1a371963d0745BCc64"): GenesisAccount{
						Balance: big.NewInt(1000000000000000000), // 1 lax
		...
		bigNumber := new(big.Int).SetBytes(util.FromHex("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"))
		tooBigNumber := new(big.Int).Set(bigNumber)
		tooBigNumber.Add(tooBigNumber, util.Big1)
		for i, tt := range []struct {
			...
			{ // ErrNonceTooLow
				txs: []*types.Transaction{
					makeTx(key1, 0, util.Address{}, big.NewInt(0), chainparams.TxGas, big.NewInt(875000000), nil),
					makeTx(key1, 0, util.Address{}, big.NewInt(0), chainparams.TxGas, big.NewInt(875000000), nil),
				},
			...
			{ // ErrNonceTooHigh
				txs: []*types.Transaction{
					makeTx(key1, 100, util.Address{}, big.NewInt(0), chainparams.TxGas, big.NewInt(875000000), nil),
				},
			...
			{ // ErrNonceMax
				txs: []*types.Transaction{
					makeTx(key2, math.MaxUint64, util.Address{}, big.NewInt(0), chainparams.TxGas, big.NewInt(875000000), nil),
				},
			...
			{ // ErrGasLimitReached
				txs: []*types.Transaction{
					makeTx(key1, 0, util.Address{}, big.NewInt(0), 600000001, big.NewInt(875000000), nil),
				},
				want: "could not apply tx 0 [0x194cc7a1d154cb57e6007cca03c50b9b6a6c0dee7a3a2b80bb9441736a1c03bf]: gas limit reached",
			},
			{ // ErrInsufficientFundsForTransfer
				txs: []*types.Transaction{
					makeTx(key1, 0, util.Address{}, big.NewInt(1000000000000000000), chainparams.TxGas, big.NewInt(875000000), nil),
				},
			...
			{ // ErrInsufficientFunds
				txs: []*types.Transaction{
					makeTx(key1, 0, util.Address{}, big.NewInt(0), chainparams.TxGas, big.NewInt(900000000000000000), nil),
				},
			...
			{ // ErrIntrinsicGas
				txs: []*types.Transaction{
					makeTx(key1, 0, util.Address{}, big.NewInt(0), chainparams.TxGas-1000, big.NewInt(875000000), nil),
				},
			...
			{ // ErrGasLimitReached
				txs: []*types.Transaction{
					makeTx(key1, 0, util.Address{}, big.NewInt(0), chainparams.TxGas*50000, big.NewInt(875000000), nil),
				},
				want: "could not apply tx 0 [0xf075face8c34cb2d9a3a6537576491a2b9262373f4ad7ba55bd5b774307fdd46]: gas limit reached",
			},
			{ // ErrFeeCapTooLow
				txs: []*types.Transaction{
					mkDynamicTx(0, util.Address{}, chainparams.TxGas, big.NewInt(0), big.NewInt(0)),
				},
			...
			{ // ErrTipVeryHigh
				txs: []*types.Transaction{
					mkDynamicTx(0, util.Address{}, chainparams.TxGas, tooBigNumber, big.NewInt(1)),
				},
			...
			{ // ErrFeeCapVeryHigh
				txs: []*types.Transaction{
					mkDynamicTx(0, util.Address{}, chainparams.TxGas, big.NewInt(1), tooBigNumber),
				},
			...
			{ // ErrTipAboveFeeCap
				txs: []*types.Transaction{
					mkDynamicTx(0, util.Address{}, chainparams.TxGas, big.NewInt(2), big.NewInt(1)),
				},
			...
			{ // ErrInsufficientFunds
				...
				txs: []*types.Transaction{
					mkDynamicTx(0, util.Address{}, chainparams.TxGas, big.NewInt(1), big.NewInt(50000000000000)),
				},
			...
			{ // Another ErrInsufficientFunds, this one to ensure that feecap/tip of max u256 is allowed
				txs: []*types.Transaction{
					mkDynamicTx(0, util.Address{}, chainparams.TxGas, bigNumber, bigNumber),
				},
	...
	// ErrTxTypeNotSupported, For this, we need an older chain
	{
		...
		for i, tt := range []struct {
			...
			{ // ErrTxTypeNotSupported
				txs: []*types.Transaction{
					mkDynamicTx(0, util.Address{}, chainparams.TxGas-1000, big.NewInt(0), big.NewInt(0)),
				},
...
func GenerateBadBlock(parent *types.Block, engine kernel.Engine, txs types.Transactions, config *chainparams.ChainConfig) *types.Block {
	header := &types.Header{
		...
		Number:   new(big.Int).Add(parent.Number(), util.Big1),
		Time:     parent.Time() + 10,
	}
	if config.IsLondon(header.Number) {
		header.BaseFee = kernel.CalcBaseFee(config, parent.Header())
	}
	...
	header.Root = util.BytesToHash(hasher.Sum(nil))
