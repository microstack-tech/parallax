...
func (basic *snapshotTestBasic) prepare(t *testing.T) (*BlockChain, []*types.Block) {
	...
	chain, err := NewBlockChain(db, cacheConfig, chainparams.AllXHashProtocolChanges, engine, script.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to create chain: %v", err)
	}
	blocks, _ := GenerateChain(chainparams.TestChainConfig, genesis, engine, gendb, basic.chainBlocks, func(i int, b *BlockGen) {})
...
func (snaptest *snapshotTest) test(t *testing.T) {
	...
	chain.Stop()
	newchain, err := NewBlockChain(snaptest.db, nil, chainparams.AllXHashProtocolChanges, snaptest.engine, script.Config{}, nil, nil)
	if err != nil {
...
func (snaptest *crashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(newdb, nil, chainparams.AllXHashProtocolChanges, snaptest.engine, script.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	newchain.Stop()
...
func (snaptest *gappedSnapshotTest) test(t *testing.T) {
	...
	chain.Stop()
	gappedBlocks, _ := GenerateChain(chainparams.TestChainConfig, blocks[len(blocks)-1], snaptest.engine, snaptest.gendb, snaptest.gapped, func(i int, b *BlockGen) {})
	...
	newchain, err := NewBlockChain(snaptest.db, cacheConfig, chainparams.AllXHashProtocolChanges, snaptest.engine, script.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
...
func (snaptest *wipeCrashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(snaptest.db, config, chainparams.AllXHashProtocolChanges, snaptest.engine, script.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	newBlocks, _ := GenerateChain(chainparams.TestChainConfig, blocks[len(blocks)-1], snaptest.engine, snaptest.gendb, snaptest.newBlocks, func(i int, b *BlockGen) {})
