...
type freezerTable struct {
	...
	logger logging.Logger // Logger with database path and table name embedded
...
func newTable(path string, name string, readMeter metrics.Meter, writeMeter metrics.Meter, sizeGauge metrics.Gauge, maxFilesize uint32, noCompression, readonly bool) (*freezerTable, error) {
	...
	tab := &freezerTable{
		...
		name:          name,
		path:          path,
		logger:        logging.New("database", path, "table", name),
...
func (t *freezerTable) repair() error {
	...
	for contentExp != contentSize {
		...
		if contentExp < contentSize {
			t.logger.Warn("Truncating dangling head", "indexed", util.StorageSize(contentExp), "stored", util.StorageSize(contentSize))
		...
		if contentExp > contentSize {
			t.logger.Warn("Truncating dangling indexes", "indexed", util.StorageSize(contentExp), "stored", util.StorageSize(contentSize))
	...
	t.logger.Debug("Chain freezer table opened", "items", t.items, "size", util.StorageSize(t.headBytes))
	return nil
}
