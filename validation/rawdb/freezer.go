...
//     reserving it for parallax. This would also reduce the memory requirements
...
func NewFreezer(datadir string, namespace string, readonly bool, maxTableSize uint32, tables map[string]bool) (*Freezer, error) {
	...
	if info, err := os.Lstat(datadir); !os.IsNotExist(err) {
		if info.Mode()&os.ModeSymlink != 0 {
			logging.Warn("Symbolic link ancient database is not supported", "path", datadir)
	...
	logging.Info("Opened ancient database", "database", datadir, "readonly", readonly)
	return freezer, nil
}
...
func (f *Freezer) ReadAncients(fn func(dbstore.AncientReaderOp) error) (err error) {
	f.writeLock.RLock()
	defer f.writeLock.RUnlock()
...
func (f *Freezer) ModifyAncients(fn func(dbstore.AncientWriteOp) error) (writeSize int64, err error) {
	if f.readonly {
	...
	defer func() {
		if err != nil {
			...
			for name, table := range f.tables {
				...
				if err != nil {
					logging.Error("Freezer table roll-back failed", "table", name, "index", prevItem, "err", err)
				}
			}
		}
	}()
...
func (f *Freezer) MigrateTable(kind string, convert convertLegacyFn) error {
	...
	if offset > 0 {
		logging.Info("found previous migration attempt", "migrated", offset)
	}
	...
	if err := forEach(table, offset, func(i uint64, blob []byte) error {
		if i%10000 == 0 && time.Since(logged) > 16*time.Second {
			logging.Info("Processing legacy elements", "count", i, "elapsed", util.PrettyDuration(time.Since(start)))
			logged = time.Now()
		}
	...
	logging.Info("Replacing old table files with migrated ones", "elapsed", util.PrettyDuration(time.Since(start)))
