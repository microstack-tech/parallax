...
func (st *insertStats) report(chain []*types.Block, index int, dirty util.StorageSize, setHead bool) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		context := []any{
			...
			"elapsed", util.PrettyDuration(elapsed), "mgasps", float64(st.usedGas) * 1000 / float64(elapsed),
		...
		if setHead {
			logging.Info("Imported new chain segment", context...)
		} else {
			logging.Info("Imported new potential chain segment", context...)
		}
