...
func TestRemoteNotify(t *testing.T) {
	...
	case work := <-sink:
		...
		if want := util.BytesToHash(SeedHash(header.Number.Uint64())).Hex(); work[1] != want {
		...
		if want := util.BytesToHash(target.Bytes()).Hex(); work[2] != want {
...
func TestRemoteNotifyFull(t *testing.T) {
	...
	config := Config{
		PowMode:    ModeTest,
		NotifyFull: true,
		Log:        testlog.Logger(t, logging.LvlWarn),
	}
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom xhash engine.
	xhash := NewTester([]string{server.URL}, false)
	xhash.config.Log = testlog.Logger(t, logging.LvlWarn)
	defer xhash.Close()
...
func TestStaleSubmission(t *testing.T) {
	...
	fakeNonce, fakeDigest := types.BlockNonce{0x01, 0x02, 0x03}, util.HexToHash("deadbeef")
	...
	testcases := []struct {
		...
		{
			[]*types.Header{
				{ParentHash: util.BytesToHash([]byte{0xa}), Number: big.NewInt(1), Difficulty: big.NewInt(100000000)},
			},
			0,
			true,
		},
		...
		{
			[]*types.Header{
				{ParentHash: util.BytesToHash([]byte{0xb}), Number: big.NewInt(2), Difficulty: big.NewInt(100000000)},
				{ParentHash: util.BytesToHash([]byte{0xb}), Number: big.NewInt(2), Difficulty: big.NewInt(100000001)},
			},
			0,
			true,
		},
		...
		{
			[]*types.Header{
				{ParentHash: util.BytesToHash([]byte{0xc}), Number: big.NewInt(3), Difficulty: big.NewInt(100000000)},
				{ParentHash: util.BytesToHash([]byte{0xd}), Number: big.NewInt(9), Difficulty: big.NewInt(100000000)},
			},
			0,
			true,
		},
		...
		{
			[]*types.Header{
				{ParentHash: util.BytesToHash([]byte{0xe}), Number: big.NewInt(10), Difficulty: big.NewInt(100000000)},
				{ParentHash: util.BytesToHash([]byte{0xf}), Number: big.NewInt(17), Difficulty: big.NewInt(100000000)},
			},
			0,
			false,
		},
