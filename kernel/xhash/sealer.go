...
func (xhash *XHash) Seal(chain kernel.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	// If we're running a fake PoW, simply return a 0 nonce immediately
	if xhash.config.PowMode == ModeFake || xhash.config.PowMode == ModeFullFake {
		header := block.Header()
		header.Nonce, header.MixDigest = types.BlockNonce{}, util.Hash{}
		select {
	...
	for {
		...
		default:
			...
			if powBuffer.SetBytes(result).Cmp(target) <= 0 {
				...
				header.MixDigest = util.BytesToHash(digest)
...
type remoteSealer struct {
	works        map[util.Hash]*types.Block
	rates        map[util.Hash]hashrate
...
func startRemoteSealer(xhash *XHash, urls []string, noverify bool) *remoteSealer {
	...
	s := &remoteSealer{
		...
		notifyCtx:    ctx,
		cancelNotify: cancel,
		works:        make(map[util.Hash]*types.Block),
		rates:        make(map[util.Hash]hashrate),
...
func (s *remoteSealer) makeWork(block *types.Block) {
	...
	s.currentWork[1] = util.BytesToHash(SeedHash(block.NumberU64())).Hex()
	bound := new(big.Int).Div(new(big.Int).Set(two256m1), block.Difficulty())
	s.currentWork[2] = util.BytesToHash(bound.Bytes()).Hex()
...
func (s *remoteSealer) submitWork(nonce types.BlockNonce, mixDigest util.Hash, sealhash util.Hash) bool {
	if s.currentBlock == nil {
	...
	if !s.noverify {
		if err := s.xhash.verifySeal(nil, header, true); err != nil {
			s.xhash.config.Log.Warn("Invalid proof-of-work submitted", "sealhash", sealhash, "elapsed", util.PrettyDuration(time.Since(start)), "err", err)
			return false
		}
	}
	...
	s.xhash.config.Log.Trace("Verified correct proof-of-work", "sealhash", sealhash, "elapsed", util.PrettyDuration(time.Since(start)))
