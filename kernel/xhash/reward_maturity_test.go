...
func newTestStateDB(t *testing.T) *state.StateDB {
	...
	db, err := state.New(util.Hash{}, statedb, nil)
	if err != nil {
...
func TestPutAndPopScheduledPayout_NormalFlow(t *testing.T) {
	...
	height := uint64(100)
	addr := util.HexToAddress("0x00000000000000000000000000000000000000AA")
...
func TestPopDuePayout_HeightZeroIgnored(t *testing.T) {
	...
	if ok || (addr != (util.Address{})) || amt != nil {
...
func TestPutScheduledPayout_ZeroAmountActsAsAbsent(t *testing.T) {
	...
	height := uint64(7)
	addr := util.HexToAddress("0x00000000000000000000000000000000000000BB")
	zero := new(big.Int) // 0
	...
	gotAddr, gotAmt, ok := popDuePayout(sdb, height)
	if ok || (gotAddr != (util.Address{})) || gotAmt != nil {
...
func TestPutAndPopScheduledPayout_BurnAddress(t *testing.T) {
	...
	height := uint64(55)
	burn := util.HexToAddress("0x0000000000000000000000000000000000000000")
	amt := big.NewInt(42)
