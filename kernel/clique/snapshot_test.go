...
func TestClique(t *testing.T) {
	...
	for i, tt := range tests {
		...
		signers := make([]util.Address, len(tt.signers))
		...
		genesis := &validation.Genesis{
			ExtraData: make([]byte, extraVanity+util.AddressLength*len(signers)+extraSeal),
			BaseFee:   big.NewInt(chainparams.InitialBaseFee),
		}
		for j, signer := range signers {
			copy(genesis.ExtraData[extraVanity+j*util.AddressLength:], signer[:])
		}
		...
		blocks, _ := validation.GenerateChain(&config, genesis.ToBlock(db), engine, db, len(tt.votes), func(j int, gen *validation.BlockGen) {
		...
		for j, block := range blocks {
			...
			if auths := tt.votes[j].checkpoint; auths != nil {
				header.Extra = make([]byte, extraVanity+len(auths)*util.AddressLength+extraSeal)
		...
		chain, err := validation.NewBlockChain(db, nil, &config, engine, script.Config{}, nil, nil)
		if err != nil {
		...
		signers = make([]util.Address, len(tt.results))
