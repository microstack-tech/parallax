...
var (
	VersionCheckUrlFlag = cli.StringFlag{
		...
		Value: "https://parallaxprotocol.org/vulnerabilities/vulnerabilities.json",
	}
...
`,
	...
	versionCheckCommand = cli.Command{
		...
		Category:  "MISCELLANEOUS COMMANDS",
		Description: `
The version-check command fetches vulnerability-information from https://parallaxprotocol.org/vulnerabilities/vulnerabilities.json, 
...
func makecache(ctx *cli.Context) error {
	args := ctx.Args()
	if len(args) != 2 {
		utils.Fatalf(`Usage: parallaxd makecache <block number> <outputdir>`)
	}
...
func makedag(ctx *cli.Context) error {
	args := ctx.Args()
	if len(args) != 2 {
		utils.Fatalf(`Usage: parallaxd makedag <block number> <outputdir>`)
	}
	block, err := strconv.ParseUint(args[0], 0, 64)
	if err != nil {
		utils.Fatalf("Invalid block number: %v", err)
...
func license(_ *cli.Context) error {
	fmt.Println(`Parallax is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 3 of the License, or
(at your option) any later version.
