...
func dnsSeedCompile(ctx *cli.Context) error {
	...
	for _, n := range state.Nodes {
		...
		if n.SuccessCount < minSuccesses {
			continue
		}
		total := n.SuccessCount + n.FailCount
		if total == 0 || float64(n.SuccessCount)/float64(total) < minRate {
			continue
		}
		ip := net.ParseIP(n.IP)
		if !isDialableIP(ip) {
	...
	if len(zone.Records) < minRecords {
		return fmt.Errorf("compiled zone has %d records, below --min-records=%d threshold (refusing to publish a near-empty zone — likely a crawler outage)",
			len(zone.Records), minRecords)
	}

	fmt.Fprintf(os.Stderr, "compiled %d records (filter: keyType=0, port=%d, age<=%s, successCount>=%d, rate>=%.2f)\n",
		len(zone.Records), port, maxAge, minSuccesses, minRate)
