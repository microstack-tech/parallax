...
func (db *odrDatabase) OpenTrie(root util.Hash) (state.Trie, error) {
	return &odrTrie{db: db, id: db.id}, nil
}

func (db *odrDatabase) OpenStorageTrie(addrHash, root util.Hash) (state.Trie, error) {
...
func (db *odrDatabase) ContractCode(addrHash, codeHash util.Hash) ([]byte, error) {
	if codeHash == sha3Nil {
		return nil, nil
	}
...
func (db *odrDatabase) ContractCodeSize(addrHash, codeHash util.Hash) (int, error) {
...
func (t *odrTrie) Commit(onleaf trie.LeafCallback) (util.Hash, int, error) {
	if t.trie == nil {
...
func (t *odrTrie) Prove(key []byte, fromLevel uint, proofDb dbstore.KeyValueWriter) error {
