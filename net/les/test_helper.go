...
import (
	...
	"github.com/ParallaxProtocol/parallax/net/les/checkpointoracle"
...
var (
	...
	testEventEmitterCode = util.Hex2Bytes("60606040523415600e57600080fd5b7f57050ab73f6b9ebdd9f76b8d4997793f48cf956e965ee070551b9ca0bb71584e60405160405180910390a160358060476000396000f3006060604052600080fd00a165627a7a723058203f727efcad8b5811f8cb1fc2620ce5e8c63570d697aef968172de296ea3994140029")

	// Checkpoint oracle relative fields
	oracleAddr   util.Address
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			oracleAddr, _, _, _ = contract.DeployCheckpointOracle(auth, backend, []util.Address{signerAddr}, sectionSize, processConfirms, big.NewInt(1))

			// bankUser transfers some laxes to user1
			nonce, _ := backend.PendingNonceAt(ctx, bankAddr)
			tx, _ := types.SignTx(types.NewTransaction(nonce, userAddr1, big.NewInt(10_000_000_000_000_000), chainparams.TxGas, big.NewInt(chainparams.InitialBaseFee), nil), signer, bankKey)
		...
		case 1:
			...
			tx1, _ := types.SignTx(types.NewTransaction(bankNonce, userAddr1, big.NewInt(1_000_000_000_000_000), chainparams.TxGas, big.NewInt(chainparams.InitialBaseFee), nil), signer, bankKey)
			backend.SendTransaction(ctx, tx1)

			// user1 relays laxes to user2
			tx2, _ := types.SignTx(types.NewTransaction(userNonce1, userAddr2, big.NewInt(1_000_000_000_000_000), chainparams.TxGas, big.NewInt(chainparams.InitialBaseFee), nil), signer, userKey1)
			backend.SendTransaction(ctx, tx2)

			// user1 deploys a test contract
			tx3, _ := types.SignTx(types.NewContractCreation(userNonce1+1, big.NewInt(0), 200000, big.NewInt(chainparams.InitialBaseFee), testContractCode), signer, userKey1)
			...
			tx4, _ := types.SignTx(types.NewContractCreation(userNonce1+2, big.NewInt(0), 200000, big.NewInt(chainparams.InitialBaseFee), testEventEmitterCode), signer, userKey1)
		...
		case 2:
			...
			bankNonce, _ := backend.PendingNonceAt(ctx, bankAddr)
			tx1, _ := types.SignTx(types.NewTransaction(bankNonce, signerAddr, big.NewInt(1000000000), chainparams.TxGas, big.NewInt(chainparams.InitialBaseFee), nil), signer, bankKey)
			backend.SendTransaction(ctx, tx1)
			...
			tx2, _ := types.SignTx(types.NewTransaction(bankNonce+1, testContractAddr, big.NewInt(0), 100000, big.NewInt(chainparams.InitialBaseFee), data), signer, bankKey)
			backend.SendTransaction(ctx, tx2)
		case 3:
			// Builtin-block
			//    number: 4
			...
			tx, _ := types.SignTx(types.NewTransaction(bankNonce, testContractAddr, big.NewInt(0), 100000, big.NewInt(chainparams.InitialBaseFee), data), signer, bankKey)
			backend.SendTransaction(ctx, tx)
		}
		backend.Commit()
	}
...
func testIndexers(db dbstore.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*validation.ChainIndexer {
	var indexers [3]*validation.ChainIndexer
	indexers[0] = light.NewChtIndexer(db, odr, config.ChtSize, config.ChtConfirms, disablePruning)
	indexers[1] = validation.NewBloomIndexer(db, config.BloomSize, config.BloomConfirms)
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*validation.ChainIndexer, db dbstore.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) (*clientHandler, func()) {
	var (
	...
	if indexers != nil {
		checkpointConfig := &chainparams.CheckpointOracleConfig{
			Address:   crypto.CreateAddress(bankAddr, 0),
			Signers:   []util.Address{signerAddr},
			Threshold: 1,
		}
		getLocal := func(index uint64) chainparams.TrustedCheckpoint {
			chtIndexer := indexers[0]
			sectionHead := chtIndexer.SectionHead(index)
			return chainparams.TrustedCheckpoint{
				SectionIndex: index,
...
func newTestServerHandler(blocks int, indexers []*validation.ChainIndexer, db dbstore.Database, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend, func()) {
	var (
		gspec = validation.Genesis{
			Config:   chainparams.AllXHashProtocolChanges,
			Alloc:    validation.GenesisAlloc{bankAddr: {Balance: bankFunds}},
	...
	txpoolConfig := validation.DefaultTxPoolConfig
	txpoolConfig.Journal = ""
	txpool := validation.NewTxPool(txpoolConfig, gspec.Config, simulation.Blockchain())
	if indexers != nil {
		checkpointConfig := &chainparams.CheckpointOracleConfig{
			Address:   crypto.CreateAddress(bankAddr, 0),
			Signers:   []util.Address{signerAddr},
...
func (p *testPeer) handshakeWithServer(t *testing.T, td *big.Int, head util.Hash, headNum uint64, genesis util.Hash, forkID forkid.ID) {
...
func (p *testPeer) handshakeWithClient(t *testing.T, td *big.Int, head util.Hash, headNum uint64, genesis util.Hash, forkID forkid.ID, costList RequestCostList, recentTxLookup uint64) {
	// It only works for the simulated client peer
	if p.speer == nil {
	...
	sendList = sendList.add("serveRecentState", uint64(validation.TriesInMemory-4))
...
type indexerCallback func(*validation.ChainIndexer, *validation.ChainIndexer, *validation.ChainIndexer)
...
type testClient struct {
	...
	chtIndexer       *validation.ChainIndexer
	bloomIndexer     *validation.ChainIndexer
	bloomTrieIndexer *validation.ChainIndexer
}
