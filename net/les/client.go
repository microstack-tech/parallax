...
func New(stack *node.Node, config *prlconfig.Config) (*LightParallax, error) {
	...
	if _, isCompat := genesisErr.(*chainparams.ConfigCompatError); genesisErr != nil && !isCompat {
	...
	if leth.handler.ulc != nil {
		logging.Warn("Ultra light client is enabled", "trustedNodes", len(leth.handler.ulc.keys), "minTrustedFraction", leth.handler.ulc.fraction)
...
func (s *LightDummyAPI) Coinbase() (util.Address, error) {
	return util.Address{}, fmt.Errorf("mining is not supported in light mode")
}
...
func (s *LightParallax) APIs() []rpc.API {
	...
	case *xhash.XHash:
		api := xhash.NewAPI(s.BlockChain().HeaderChain(), e)
...
func (s *LightParallax) Start() error {
	logging.Warn("Light client mode is an experimental feature")
	...
	if s.udpEnabled && s.p2pServer.DiscV5 == nil {
		s.udpEnabled = false
		logging.Error("Discovery v5 is not initialized")
	}
...
func (s *LightParallax) Stop() error {
	...
	s.lesDb.Close()
	s.wg.Wait()
	logging.Info("Light parallax stopped")
	return nil
}
