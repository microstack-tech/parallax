...
type LesOdr struct {
	...
	chtIndexer, bloomTrieIndexer, bloomIndexer *validation.ChainIndexer
	peers                                      *serverPeerSet
	retriever                                  *retrieveManager
	stop                                       chan struct{}
}

func NewLesOdr(db dbstore.Database, config *light.IndexerConfig, peers *serverPeerSet, retriever *retrieveManager) *LesOdr {
	return &LesOdr{
		db:            db,
...
func (odr *LesOdr) Database() dbstore.Database {
	return odr.db
}

// SetIndexers adds the necessary chain indexers to the ODR backend
func (odr *LesOdr) SetIndexers(chtIndexer, bloomTrieIndexer, bloomIndexer *validation.ChainIndexer) {
...
func (odr *LesOdr) ChtIndexer() *validation.ChainIndexer {
	return odr.chtIndexer
}

// BloomTrieIndexer returns the bloom trie chain indexer
func (odr *LesOdr) BloomTrieIndexer() *validation.ChainIndexer {
	return odr.bloomTrieIndexer
}

// BloomIndexer returns the bloombits chain indexer
func (odr *LesOdr) BloomIndexer() *validation.ChainIndexer {
...
func (odr *LesOdr) RetrieveTxStatus(ctx context.Context, req *light.TxStatusRequest) error {
	...
	for {
		...
		for index, status := range req.Status {
			if result[index].Status != validation.TxStatusUnknown {
				continue
			}
			if status.Status == validation.TxStatusUnknown {
				continue
			}
