...
import (
	...
	ethdownloader "github.com/ParallaxProtocol/parallax/node/protocol/downloader"
...
func TestMain(m *testing.M) {
	...
	logging.Root().SetHandler(logging.LvlFilterHandler(logging.Lvl(*loglevel), logging.StreamHandler(colorable.NewColorableStderr(), logging.TerminalFormat(true))))
...
func getHead(ctx context.Context, t *testing.T, client *rpc.Client) (uint64, util.Hash) {
...
func newLesServerService(ctx *adapters.ServiceContext, stack *node.Node) (node.Lifecycle, error) {
	...
	parallax, err := protocol.New(stack, &config)
	if err != nil {
		return nil, err
	}
