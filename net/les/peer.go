...
import (
	...
	"github.com/ParallaxProtocol/parallax/net/les/utils"
...
func (p *peerCommons) Head() (hash util.Hash) {
	p.lock.RLock()
...
func (p *peerCommons) HeadAndTd() (hash util.Hash, td *big.Int) {
	p.lock.RLock()
	defer p.lock.RUnlock()
...
func (p *peerCommons) handshake(td *big.Int, head util.Hash, headNum uint64, genesis util.Hash, forkID forkid.ID, forkFilter forkid.Filter, sendCallback func(*keyValueList), recvCallback func(keyValueMap) error) error {
	p.lock.Lock()
	defer p.lock.Unlock()
...
type serverPeer struct {
	...
	checkpoint       chainparams.TrustedCheckpoint // The advertised checkpoint sent by server.
	...
	// Test callback hooks
	hasBlockHook func(util.Hash, uint64, bool) bool // Used to determine whether the server has the specified block.
}
...
func (p *serverPeer) requestHeadersByHash(reqID uint64, origin util.Hash, amount int, skip int, reverse bool) error {
...
// specified.
func (p *serverPeer) requestBodies(reqID uint64, hashes []util.Hash) error {
...
func (p *serverPeer) requestReceipts(reqID uint64, hashes []util.Hash) error {
...
func (p *serverPeer) requestTxStatus(reqID uint64, txHashes []util.Hash) error {
...
func (p *serverPeer) HasBlock(hash util.Hash, number uint64, hasState bool) bool {
	p.lock.RLock()
	defer p.lock.RUnlock()
...
// the peer.
func (p *serverPeer) updateHead(hash util.Hash, number uint64, td *big.Int) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *serverPeer) Handshake(genesis util.Hash, forkid forkid.ID, forkFilter forkid.Filter) error {
	...
	return p.handshake(util.Big0, util.Hash{}, 0, genesis, forkid, forkFilter, func(lists *keyValueList) {
...
func (p *clientPeer) Handshake(td *big.Int, head util.Hash, headNum uint64, genesis util.Hash, forkID forkid.ID, forkFilter forkid.Filter, server *LesServer) error {
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			stateRecent := uint64(validation.TriesInMemory - blockSafetyMargin)
