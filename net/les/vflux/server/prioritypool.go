...
func (pp *priorityPool) requestCapacity(node *enode.Node, minTarget, maxTarget uint64, bias time.Duration) uint64 {
	...
	if c == nil {
		logging.Error("requestCapacity called for unknown node", "id", node.ID())
		pp.lock.Unlock()
		return 0
	}
	pp.setTempState(c)
...
func (pp *priorityPool) setTempState(c *ppNodeInfo) {
	...
	if c.tempCapacity != c.capacity { // should never happen
		logging.Error("tempCapacity != capacity when entering tempState")
	}
...
func (pp *priorityPool) unsetTempState(c *ppNodeInfo) {
	...
	c.tempState = false
	if c.tempCapacity != c.capacity { // should never happen
		logging.Error("tempCapacity != capacity when leaving tempState")
	}
	c.minTarget = pp.minCap
	c.stepDiv = pp.capacityStepDiv
	c.bias = 0
...
func (pp *priorityPool) setTempCapacity(c *ppNodeInfo, cap uint64) {
	if !c.tempState { // should never happen
		logging.Error("Node is not in temporary state")
		return
	}
...
func (pp *priorityPool) getCapacityCurve() *capacityCurve {
	...
	for pp.activeCap > 0 {
		...
		if pp.activeCap > pp.maxCap {
			logging.Error("Active capacity is greater than allowed maximum", "active", pp.activeCap, "maximum", pp.maxCap)
		} else {
		...
		pp.activeCap -= tempCap
		if next == nil {
			logging.Error("getCapacityCurve: cannot remove next element from the priority queue")
			break
		}
		id := next.node.ID()
		if excludeFirst {
