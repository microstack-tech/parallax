...
func NewClientPool(balanceDb dbstore.KeyValueStore, minCap uint64, connectedBias time.Duration, clock mclock.Clock, synced func() bool) *ClientPool {
...
func (cp *ClientPool) SetCapacity(node *enode.Node, reqCap uint64, bias time.Duration, requested bool) (capacity uint64, err error) {
	...
	cp.ns.Operation(func() {
		...
		logging.Error("Unable to find maximum allowed capacity")
