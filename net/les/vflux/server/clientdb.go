...
func newNodeDB(db dbstore.KeyValueStore, clock mclock.Clock) *nodeDB {
...
func (db *nodeDB) getOrNewBalance(id []byte, neg bool) utils.ExpiredValue {
	...
	if err := rlp.DecodeBytes(enc, &b); err != nil {
		logging.Crit("Failed to decode positive balance", "err", err)
	}
...
func (db *nodeDB) setBalance(id []byte, neg bool, b utils.ExpiredValue) {
	...
	if err != nil {
		logging.Crit("Failed to encode positive balance", "err", err)
	}
	db.db.Put(key, enc)
	db.cache.Add(string(key), b)
}
...
func (db *nodeDB) expireNodes() {
	...
	for _, neg := range []bool{false, true} {
		...
		for iter.Next() {
			...
			if err := rlp.DecodeBytes(iter.Value(), &balance); err != nil {
				logging.Crit("Failed to decode negative balance", "err", err)
			}
	...
	logging.Debug("Expire nodes", "visited", visited, "deleted", deleted, "elapsed", util.PrettyDuration(time.Since(start)))
}
