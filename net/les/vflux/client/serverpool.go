...
func NewServerPool(db dbstore.KeyValueStore, dbKey []byte, mixTimeout time.Duration, query QueryFunc, clock mclock.Clock, trustedURLs []string, requestList []RequestInfo) (*ServerPool, enode.Iterator) {
	s := &ServerPool{
		db:           db,
		clock:        clock,
		...
		go func() {
			...
			if q == -1 {
				...
				if fails%warnQueryFails == 0 {
					...
					logging.Warn("UDP connection queries failed", "count", fails)
				}
			} else {
...
func (s *ServerPool) Start() {
	...
	for _, url := range s.trustedURLs {
		if node, err := enode.Parse(s.validSchemes, url); err == nil {
			...
			logging.Error("Invalid trusted server URL", "url", url, "error", err)
		}
	}
	unixTime := s.unixTime()
	s.ns.Operation(func() {
