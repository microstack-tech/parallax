...
func NewValueTracker(db dbstore.KeyValueStore, clock mclock.Clock, reqInfo []RequestInfo, updatePeriod time.Duration, transferRate, statsExpRate, offlineExpRate float64) *ValueTracker {
	now := clock.Now()
...
func (vt *ValueTracker) loadFromDb(mapping []string) error {
	...
	if err := rlp.Decode(r, &version); err != nil {
		logging.Error("Decoding value tracker state failed", "err", err)
		return err
	}
	if version != vtVersion {
		logging.Error("Unknown ValueTracker version", "stored", version, "current", nvtVersion)
...
loop:
	...
	if int(vte.RefBasketMapping) == vt.currentMapping {
		...
		if vte.RefBasketMapping >= uint(len(vt.mappings)) {
			logging.Error("Unknown request basket mapping", "stored", vte.RefBasketMapping, "current", vt.currentMapping)
...
func (vt *ValueTracker) saveToDb() {
	...
	if err != nil {
		logging.Error("Encoding value tracker state failed", "err", err)
		return
	}
	...
	if err := vt.db.Put(vtKey, append(enc1, enc2...)); err != nil {
		logging.Error("Saving value tracker state failed", "err", err)
	}
}
...
func (vt *ValueTracker) loadOrNewNode(id enode.ID) *NodeValueTracker {
	...
	var version uint
	if err := rlp.Decode(r, &version); err != nil {
		logging.Error("Failed to decode node value tracker", "id", id, "err", err)
		return nv
	}
	if version != nvtVersion {
		logging.Error("Unknown NodeValueTracker version", "stored", version, "current", nvtVersion)
		return nv
	}
	...
	if int(nve.ServerBasketMapping) == vt.currentMapping {
		...
		if nve.ServerBasketMapping >= uint(len(vt.mappings)) {
			logging.Error("Unknown request basket mapping", "stored", nve.ServerBasketMapping, "current", vt.currentMapping)
			return nv
		}
...
func (vt *ValueTracker) saveNode(id enode.ID, nv *NodeValueTracker) {
	...
	if err != nil {
		logging.Error("Failed to encode service value information", "id", id, "err", err)
		return
	}
	...
	if err := vt.db.Put(append(vtNodeKey, id[:]...), append(enc1, enc2...)); err != nil {
		logging.Error("Failed to save service value information", "id", id, "err", err)
	}
}
