...
type pruner struct {
	db       dbstore.Database
	indexers []*validation.ChainIndexer
	closeCh  chan struct{}
	wg       sync.WaitGroup
}

// newPruner returns a light chain pruner instance.
func newPruner(db dbstore.Database, indexers ...*validation.ChainIndexer) *pruner {
	pruner := &pruner{
		db:       db,
		indexers: indexers,
	...
	pruning := func() {
		...
		for _, indexer := range p.indexers {
			if err := indexer.Prune(min - 2); err != nil {
				logging.Debug("Failed to prune historical data", "err", err)
				return
			}
		}
