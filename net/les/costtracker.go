...
func (ct *costTracker) gfLoop() {
	...
	go func() {
		saveCostFactor := func() {
			...
			logging.Debug("global cost factor saved", "value", factor)
		}
			...
			case r := <-ct.reqInfoCh:
				...
				if gfCorr != 0 {
					...
					if time.Duration(now-lastUpdate) > time.Second {
						...
						logging.Debug("global cost factor updated", "factor", factor)
					}
				}
...
func (ct *costTracker) printStats() {
	...
	for code, arr := range ct.stats {
		logging.Info("Request cost statistics", "code", code, "1/16", arr[0], "1/8", arr[1], "1/4", arr[2], "1/2", arr[3], "1", arr[4], "2", arr[5], "4", arr[6], "8", arr[7], "16", arr[8], ">16", arr[9])
	}
}
