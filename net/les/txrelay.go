...
type lesTxRelay struct {
	txSent       map[util.Hash]*types.Transaction
	txPending    map[util.Hash]struct{}
	peerList     []*serverPeer
	peerStartPos int
	lock         sync.Mutex
	stop         chan struct{}
...
func newLesTxRelay(ps *serverPeerSet, retriever *retrieveManager) *lesTxRelay {
	r := &lesTxRelay{
		txSent:    make(map[util.Hash]*types.Transaction),
		txPending: make(map[util.Hash]struct{}),
		retriever: retriever,
...
func (ltrx *lesTxRelay) NewHead(head util.Hash, mined []util.Hash, rollback []util.Hash) {
	ltrx.lock.Lock()
	defer ltrx.lock.Unlock()
...
func (ltrx *lesTxRelay) Discard(hashes []util.Hash) {
	ltrx.lock.Lock()
	defer ltrx.lock.Unlock()
