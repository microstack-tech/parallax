...
type stateReq struct {
	...
	trieTasks map[util.Hash]*trieTask // Trie node download tasks to track previous attempts
	codeTasks map[util.Hash]*codeTask // Byte code download tasks to track previous attempts
...
func (d *Downloader) syncState(root util.Hash) *stateSync {
...
func (d *Downloader) runStateSync(s *stateSync) *stateSync {
	...
	logging.Trace("State sync starting", "root", s.root)
	...
	for {
		...
		case req := <-d.trackStateReq:
			...
			if old := active[req.peer.id]; old != nil {
				logging.Warn("Busy peer assigned new state fetch", "peer", old.peer.id)
...
func (d *Downloader) spindownStateSync(active map[string]*stateReq, finished []*stateReq, timeout chan *stateReq, peerDrop chan *peerConnection) {
	logging.Trace("State sync spinning down", "active", len(active), "finished", len(finished))
	for len(active) > 0 {
		var (
			req    *stateReq
			reason string
...
type stateSync struct {
	...
	root   util.Hash          // State root currently being synced
	sched  *trie.Sync         // State trie sync scheduler defining the tasks
	keccak crypto.KeccakState // Keccak256 hasher to verify deliveries with

	trieTasks map[util.Hash]*trieTask // Set of trie node tasks currently queued for retrieval
	codeTasks map[util.Hash]*codeTask // Set of byte code tasks currently queued for retrieval
...
func newStateSync(d *Downloader, root util.Hash) *stateSync {
	return &stateSync{
		d:         d,
		root:      root,
		sched:     state.NewStateSync(root, d.stateDB, nil),
		keccak:    sha3.NewLegacyKeccak256().(crypto.KeccakState),
		trieTasks: make(map[util.Hash]*trieTask),
		codeTasks: make(map[util.Hash]*codeTask),
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < dbstore.IdealBatchSize {
		return nil
	}
	start := time.Now()
...
func (s *stateSync) fillTasks(n int, req *stateReq) (nodes []util.Hash, paths []trie.SyncPath, codes []util.Hash) {
	...
	nodes = make([]util.Hash, 0, n)
	paths = make([]trie.SyncPath, 0, n)
	codes = make([]util.Hash, 0, n)

	req.trieTasks = make(map[util.Hash]*trieTask, n)
	req.codeTasks = make(map[util.Hash]*codeTask, n)
...
// error occurred.
func (s *stateSync) processNodeData(blob []byte) (util.Hash, error) {
...
func (s *stateSync) updateStats(written, duplicate, unexpected int, duration time.Duration) {
	...
	if written > 0 || duplicate > 0 || unexpected > 0 {
		logging.Info("Imported new state entries", "count", written, "elapsed", util.PrettyDuration(duration), "processed", s.d.syncStatsState.processed, "pending", s.d.syncStatsState.pending, "trieretry", len(s.trieTasks), "coderetry", len(s.codeTasks), "duplicate", s.d.syncStatsState.duplicate, "unexpected", s.d.syncStatsState.unexpected)
	}
	if written > 0 {
