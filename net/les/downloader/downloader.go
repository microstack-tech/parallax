...
type BlockChain interface {
	...
	FastSyncCommitHead(util.Hash) error
...
func New(checkpoint uint64, stateDb dbstore.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
func (d *Downloader) Synchronise(id string, head util.Hash, td *big.Int, mode SyncMode) error {
...
func (d *Downloader) synchronise(id string, hash util.Hash, td *big.Int, mode SyncMode) error {
	...
	if mode == SnapSync {
		if !d.snapSync {
			...
			logging.Warn("Enabling snapshot sync prototype")
			d.snapSync = true
		}
		mode = FastSync
	}
...
func (d *Downloader) fetchReceipts(from uint64) error {
	logging.Debug("Downloading transaction receipts", "origin", from)
			...
			for len(headers) > 0 {
				...
				if mode == FastSync || mode == LightSync {
					...
					if n, err := d.lightchain.InsertHeaderChain(chunk, frequency); err != nil {
						...
						logging.Warn("Invalid header encountered", "number", chunk[n].Number, "hash", chunk[n].Hash(), "parent", chunk[n].ParentHash, "err", err)
						return fmt.Errorf("%w: %v", errInvalidChain, err)
					}
...
func (d *Downloader) commitFastSyncData(results []*fetchResult, stateSync *stateSync) error {
	...
	// Retrieve the a batch of results to import
	first, last := results[0].Header, results[len(results)-1].Header
	logging.Debug("Inserting fast-sync blocks", "items", len(results),
		"firstnum", first.Number, "firsthash", first.Hash(),
...
func (d *Downloader) commitPivotBlock(result *fetchResult) error {
	...
	logging.Debug("Committing fast sync pivot as new head", "number", block.Number(), "hash", block.Hash())
