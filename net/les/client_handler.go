...
func newClientHandler(ulcServers []string, ulcFraction int, checkpoint *chainparams.TrustedCheckpoint, backend *LightParallax) *clientHandler {
	...
	if ulcServers != nil {
		...
		if err != nil {
			logging.Error("Failed to initialize ultra light client")
		}
		handler.ulc = ulc
		logging.Info("Enable ultra light client mode")
	}
	var height uint64
	if checkpoint != nil {
...
func (pc *peerConnection) Head() (util.Hash, *big.Int) {
	return pc.peer.HeadAndTd()
}

func (pc *peerConnection) RequestHeadersByHash(origin util.Hash, amount int, skip int, reverse bool) error {
	rq := &distReq{
