...
func (r *Receipt) setStatus(postStateOrStatus []byte) error {
	...
	case bytes.Equal(postStateOrStatus, receiptStatusFailedRLP):
		r.Status = ReceiptStatusFailed
	case len(postStateOrStatus) == len(util.Hash{}):
...
func (r *Receipt) Size() util.StorageSize {
	size := util.StorageSize(unsafe.Sizeof(*r)) + util.StorageSize(len(r.PostState))
	size += util.StorageSize(len(r.Logs)) * util.StorageSize(unsafe.Sizeof(Log{}))
	for _, log := range r.Logs {
		size += util.StorageSize(len(log.Topics)*util.HashLength + len(log.Data))
	}
	return size
}
...
func (rs Receipts) DeriveFields(config *chainparams.ChainConfig, hash util.Hash, number uint64, txs Transactions) error {
