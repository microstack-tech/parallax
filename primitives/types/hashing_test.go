...
func TestDeriveSha(t *testing.T) {
	...
	for len(txs) < 1000 {
		tr, _ := trie.New(util.Hash{}, trie.NewDatabase(rawdb.NewMemoryDatabase()))
...
func TestEIP2718DeriveSha(t *testing.T) {
	for _, tc := range []struct {
		...
		rlp.DecodeBytes(util.FromHex(tc.rlpData), &t1)
		rlp.DecodeBytes(util.FromHex(tc.rlpData), &t2)
...
func (d *hashToHumanReadable) Hash() util.Hash {
	return util.Hash{}
}
