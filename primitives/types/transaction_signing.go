...
func Sender(signer Signer, tx *Transaction) (util.Address, error) {
...
type Signer interface {
	...
	Sender(tx *Transaction) (util.Address, error)
...
func (s londonSigner) Sender(tx *Transaction) (util.Address, error) {
	...
	if tx.ChainId().Cmp(s.chainId) != 0 {
		return util.Address{}, ErrInvalidChainId
	}
...
func (s londonSigner) Hash(tx *Transaction) util.Hash {
	if tx.Type() != DynamicFeeTxType {
...
func (s eip2930Signer) Sender(tx *Transaction) (util.Address, error) {
	V, R, S := tx.RawSignatureValues()
	switch tx.Type() {
	case LegacyTxType:
		if !tx.Protected() {
	...
	case AccessListTxType:
		...
		V = new(big.Int).Add(V, big.NewInt(27))
	default:
		return util.Address{}, ErrTxTypeNotSupported
	}
	if tx.ChainId().Cmp(s.chainId) != 0 {
		return util.Address{}, ErrInvalidChainId
	}
...
// Hash returns the hash to be signed by the sender.
// It does not uniquely identify the transaction.
func (s eip2930Signer) Hash(tx *Transaction) util.Hash {
	switch tx.Type() {
	case LegacyTxType:
		return rlpHash([]any{
			tx.Nonce(),
...
func (s EIP155Signer) Sender(tx *Transaction) (util.Address, error) {
...
// Hash returns the hash to be signed by the sender.
// It does not uniquely identify the transaction.
func (s EIP155Signer) Hash(tx *Transaction) util.Hash {
	return rlpHash([]any{
		tx.Nonce(),
		tx.GasPrice(),
		tx.Gas(),
...
func (hs HomesteadSigner) Sender(tx *Transaction) (util.Address, error) {
	if tx.Type() != LegacyTxType {
		return util.Address{}, ErrTxTypeNotSupported
	}
	v, r, s := tx.RawSignatureValues()
...
func (fs FrontierSigner) Sender(tx *Transaction) (util.Address, error) {
	if tx.Type() != LegacyTxType {
		return util.Address{}, ErrTxTypeNotSupported
	}
	v, r, s := tx.RawSignatureValues()
...
// Hash returns the hash to be signed by the sender.
// It does not uniquely identify the transaction.
func (fs FrontierSigner) Hash(tx *Transaction) util.Hash {
	return rlpHash([]any{
		tx.Nonce(),
		tx.GasPrice(),
		tx.Gas(),
...
func recoverPlain(sighash util.Hash, R, S, Vb *big.Int, homestead bool) (util.Address, error) {
	if Vb.BitLen() > 8 {
		return util.Address{}, ErrInvalidSig
	}
	V := byte(Vb.Uint64() - 27)
	...
	if len(pub) == 0 || pub[0] != 4 {
		return util.Address{}, errors.New("invalid public key")
	}
