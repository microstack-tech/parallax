...
func (oracle *Oracle) processBlock(bf *blockFees, percentiles []float64) {
	...
	if chainconfig.IsLondon(big.NewInt(int64(bf.blockNumber + 1))) {
		bf.results.nextBaseFee = kernel.CalcBaseFee(chainconfig, bf.header)
	} else {
	...
	if bf.block == nil || (bf.receipts == nil && len(bf.block.Transactions()) != 0) {
		logging.Error("Block or receipts are missing while reward percentiles are requested")
		return
	}
...
// parallax semantics, so wallets and tools that derive fee suggestions
...
func (oracle *Oracle) FeeHistory(ctx context.Context, blocks int, unresolvedLastBlock rpc.BlockNumber, rewardPercentiles []float64) (*big.Int, [][]*big.Int, []*big.Int, []float64, error) {
	if blocks < 1 {
		return util.Big0, nil, nil, nil, nil // returning with no data and no error means there are no retrievable blocks
	}
	...
	if blocks > maxFeeHistory {
		logging.Warn("Sanitizing fee history length", "requested", blocks, "truncated", maxFeeHistory)
		blocks = maxFeeHistory
	}
	for i, p := range rewardPercentiles {
		if p < 0 || p > 100 {
			return util.Big0, nil, nil, nil, fmt.Errorf("%w: %f", errInvalidPercentile, p)
		}
		if i > 0 && p < rewardPercentiles[i-1] {
			return util.Big0, nil, nil, nil, fmt.Errorf("%w: #%d:%f > #%d:%f", errInvalidPercentile, i-1, rewardPercentiles[i-1], i, p)
		}
	}
	var (
		pendingBlock    *types.Block
	...
	if err != nil || blocks == 0 {
		return util.Big0, nil, nil, nil, err
	}
	...
	for ; blocks > 0; blocks-- {
		fees := <-results
		if fees.err != nil {
			return util.Big0, nil, nil, nil, fees.err
		}
	...
	if firstMissing == 0 {
		return util.Big0, nil, nil, nil, nil
	}
	if len(rewardPercentiles) != 0 {
