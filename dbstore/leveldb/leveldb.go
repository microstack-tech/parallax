...
func NewCustom(file string, namespace string, customize func(options *opt.Options)) (*Database, error) {
	...
	logger := logging.New("database", file)
	usedCache := options.GetBlockCacheCapacity() + options.GetWriteBuffer()*2
	logCtx := []any{"cache", util.StorageSize(usedCache), "handles", options.GetOpenFilesCacheCapacity()}
	if options.ReadOnly {
...
func (db *Database) NewBatch() dbstore.Batch {
	return &batch{
		db: db.db,
		b:  new(leveldb.Batch),
	}
...
func (db *Database) NewBatchWithSize(size int) dbstore.Batch {
	return &batch{
		db: db.db,
...
func (db *Database) NewIterator(prefix []byte, start []byte) dbstore.Iterator {
...
func (db *Database) NewSnapshot() (dbstore.Snapshot, error) {
...
func (db *Database) Compact(start []byte, limit []byte) error {
	return db.db.CompactRange(ldbutil.Range{Start: start, Limit: limit})
}
...
func (b *batch) Replay(w dbstore.KeyValueWriter) error {
	return b.b.Replay(&replayer{writer: w})
}

// replayer is a small wrapper to implement the correct replay methods.
type replayer struct {
	writer  dbstore.KeyValueWriter
	failure error
}
...
func bytesPrefixRange(prefix, start []byte) *ldbutil.Range {
	r := ldbutil.BytesPrefix(prefix)
