...
import (
	...
	ethproto "github.com/ParallaxProtocol/parallax/node/protocol/protocols/prl"
...
type backend interface {
	...
	SubscribeNewTxsEvent(ch chan<- validation.NewTxsEvent) event.Subscription
...
func (s *Service) Start() error {
	...
	txEventCh := make(chan validation.NewTxsEvent, txChanSize)
	s.txSub = s.backend.SubscribeNewTxsEvent(txEventCh)
	go s.loop(chainHeadCh, txEventCh)

	logging.Info("Stats daemon started")
	return nil
}
...
func (s *Service) Stop() error {
	...
	s.txSub.Unsubscribe()
	logging.Info("Stats daemon stopped")
	return nil
}

// loop keeps trying to connect to the netstats server, reporting chain events
// until termination.
func (s *Service) loop(chainHeadCh chan validation.ChainHeadEvent, txEventCh chan validation.NewTxsEvent) {
	...
	for {
		...
		case <-errTimer.C:
			...
			if err != nil {
				logging.Warn("Stats server unreachable", "err", err)
				errTimer.Reset(10 * time.Second)
				continue
			}
			// Authenticate the client with the server
			if err = s.login(conn); err != nil {
				logging.Warn("Stats login failed", "err", err)
				conn.Close()
				errTimer.Reset(10 * time.Second)
				continue
			}
			...
			if err = s.report(conn); err != nil {
				logging.Warn("Initial stats report failed", "err", err)
				conn.Close()
				errTimer.Reset(0)
				continue
			}
			...
			for err == nil {
				...
				case <-fullReport.C:
					if err = s.report(conn); err != nil {
						logging.Warn("Full stats report failed", "err", err)
					}
				case list := <-s.histCh:
					if err = s.reportHistory(conn, list); err != nil {
						logging.Warn("Requested history report failed", "err", err)
					}
				case head := <-headCh:
					if err = s.reportBlock(conn, head); err != nil {
						logging.Warn("Block stats report failed", "err", err)
					}
					if err = s.reportPending(conn); err != nil {
						logging.Warn("Post-block transaction stats report failed", "err", err)
					}
				case <-txCh:
					if err = s.reportPending(conn); err != nil {
						logging.Warn("Transaction stats report failed", "err", err)
					}
				}
			}
			fullReport.Stop()
...
func (s *Service) readLoop(conn *connWrapper) {
	...
	for {
		...
		if err := conn.ReadJSON(&blob); err != nil {
			logging.Warn("Failed to retrieve stats server message", "err", err)
			return
		}
		...
		if err := json.Unmarshal(blob, &ping); err == nil && strings.HasPrefix(ping, "primus::ping::") {
			if err := conn.WriteJSON(strings.ReplaceAll(ping, "ping", "pong")); err != nil {
				logging.Warn("Failed to respond to system ping message", "err", err)
				return
			}
			continue
		}
		...
		if err := json.Unmarshal(blob, &msg); err != nil {
			logging.Warn("Failed to decode stats server message", "err", err)
			return
		}
		logging.Trace("Received message from stats server", "msg", msg)
		if len(msg["emit"]) == 0 {
			logging.Warn("Stats server sent non-broadcast", "msg", msg)
			return
		}
		command, ok := msg["emit"][0].(string)
		if !ok {
			logging.Warn("Invalid stats server message type", "type", msg["emit"][0])
			return
		}
		...
		if len(msg["emit"]) == 2 && command == "node-pong" {
			...
			default:
				...
				logging.Warn("Stats server pinger seems to have died")
				return
			}
		}
		...
		if len(msg["emit"]) == 2 && command == "history" {
			...
			if !ok {
				logging.Warn("Invalid stats history request", "msg", msg["emit"][1])
				select {
			...
			if !ok {
				logging.Warn("Invalid stats history block list", "list", request["list"])
				return
			}
			...
			for i, num := range list {
				n, ok := num.(float64)
				if !ok {
					logging.Warn("Invalid stats history block number", "number", num)
					return
				}
				numbers[i] = uint64(n)
			}
		...
		logging.Info("Unknown stats message", "msg", msg)
	}
}
...
func (s *Service) reportLatency(conn *connWrapper) error {
	...
	logging.Trace("Sending measured latency to prlstats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	logging.Trace("Sending new block to prlstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		logging.Trace("Sending historical blocks to prlstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
		logging.Trace("No history to send to stats server")
	}
	stats := map[string]any{
		"id":      s.node,
		"history": history,
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	logging.Trace("Sending pending transactions to prlstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	logging.Trace("Sending node details to prlstats")
