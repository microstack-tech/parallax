...
func (api *PublicParallaxAPI) Coinbase() (util.Address, error) {
...
func (api *PrivateMinerAPI) SetCoinbase(coinbase util.Address) bool {
...
func hasAllBlocks(chain *validation.BlockChain, bs []*types.Block) bool {
	for _, b := range bs {
...
type StorageRangeResult struct {
	...
	NextKey *util.Hash `json:"nextKey"` // nil if Storage includes the last key in the trie.
}

type storageMap map[util.Hash]storageEntry

type storageEntry struct {
	Key   *util.Hash `json:"key"`
	Value util.Hash  `json:"value"`
}
...
func storageRangeAt(st state.Trie, start []byte, maxResult int) (StorageRangeResult, error) {
	...
	for i := 0; i < maxResult && it.Next(); i++ {
		...
		e := storageEntry{Value: util.BytesToHash(content)}
		if preimage := st.GetKey(it.Key); preimage != nil {
			preimage := util.BytesToHash(preimage)
			e.Key = &preimage
		}
		result.Storage[util.BytesToHash(it.Key)] = e
	}
	// Add the 'next key' so clients can continue downloading.
	if it.Next() {
		next := util.BytesToHash(it.Key)
		result.NextKey = &next
	}
	return result, nil
}
...
func (s *PrivateDebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]util.Address, error) {
	...
	var dirty []util.Address
	for iter.Next() {
		key := newTrie.GetKey(iter.Key)
		if key == nil {
			return nil, fmt.Errorf("no preimage found for hash %x", iter.Key)
		}
		dirty = append(dirty, util.BytesToAddress(key))
	}
	return dirty, nil
}
...
func (s *PrivateDebugAPI) GetAccessibleState(from, to rpc.BlockNumber) (uint64, error) {
	...
	if p := rawdb.ReadLastPivotNumber(db); p != nil {
		pivot = *p
		logging.Info("Found fast-sync pivot marker", "number", pivot)
	}
	...
	for i := int64(start); i != int64(end); i += delta {
		if time.Since(lastLog) > 8*time.Second {
			logging.Info("Finding roots", "from", start, "to", end, "at", i)
			lastLog = time.Now()
		}
		if i < int64(pivot) {
			continue
