...
func (h *testEthHandler) Chain() *validation.BlockChain        { panic("no backing chain") }
...
func (h *testEthHandler) Handle(peer *prl.Peer, packet prl.Packet) error {
	...
	case *prl.NewPooledTransactionHashesPacket:
		h.txAnnounces.Send(([]util.Hash)(*packet))
		return nil
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	var (
		...
		configNoFork = &chainparams.ChainConfig{
			HomesteadBlock: big.NewInt(1),
			XHash: &chainparams.XHashConfig{
				CoinbaseMaturityBlocks: 0,
				RetargetIntervalBlocks: 10,
			},
		}
		configProFork = &chainparams.ChainConfig{
			HomesteadBlock: big.NewInt(1),
		...
		gspecNoFork = &validation.Genesis{
			Config:         configNoFork,
			EpochStartTime: 0,
			Timestamp:      0,
		}
		gspecProFork = &validation.Genesis{
		...
		chainNoFork, _  = validation.NewBlockChain(dbNoFork, nil, configNoFork, engine, script.Config{}, nil, nil)
		chainProFork, _ = validation.NewBlockChain(dbProFork, nil, configProFork, engine, script.Config{}, nil, nil)

		blocksNoFork, _  = validation.GenerateChain(configNoFork, genesisNoFork, engine, dbNoFork, 2, nil)
		blocksProFork, _ = validation.GenerateChain(configProFork, genesisProFork, engine, dbProFork, 2, nil)
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	txs := make(chan validation.NewTxsEvent)
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	for nonce := range insert {
		tx := types.NewTransaction(uint64(nonce), util.Address{}, big.NewInt(0), 100000, big.NewInt(0), make([]byte, 10240))
		tx, _ = types.SignTx(tx, types.HomesteadSigner{}, testKey)
	...
	seen := make(map[util.Hash]struct{})
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	txChs := make([]chan validation.NewTxsEvent, len(sinks))
	for i := 0; i < len(sinks); i++ {
		txChs[i] = make(chan validation.NewTxsEvent, 1024)
	...
	for nonce := range txs {
		tx := types.NewTransaction(uint64(nonce), util.Address{}, big.NewInt(0), 100000, big.NewInt(0), nil)
		tx, _ = types.SignTx(tx, types.HomesteadSigner{}, testKey)
