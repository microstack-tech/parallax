...
import (
	...
	jsassets "github.com/ParallaxProtocol/parallax/node/protocol/tracers/js/internal/tracers"
...
func fromBuf(vm *goja.Runtime, bufType goja.Value, buf goja.Value, allowString bool) ([]byte, error) {
	...
	case "String":
		...
		return util.FromHex(obj.String()), nil
	case "Array":
		var b []byte
...
type jsTracer struct {
	...
	activePrecompiles []util.Address        // List of active precompiles at current block
...
func newJsTracer(code string, ctx *tracers.Context) (tracers.Tracer, error) {
	...
	if ctx.BlockHash != (util.Hash{}) {
		t.ctx["blockHash"] = vm.ToValue(ctx.BlockHash.Bytes())
		if ctx.TxHash != (util.Hash{}) {
...
func (t *jsTracer) CaptureStart(env *script.PVM, from util.Address, to util.Address, create bool, input []byte, gas uint64, value *big.Int) {
	t.env = env
	...
	t.activePrecompiles = script.ActivePrecompiles(rules)
	t.ctx["intrinsicGas"] = t.vm.ToValue(t.gasLimit - gas)
}

// CaptureState implements the Tracer interface to trace a single step of VM execution.
func (t *jsTracer) CaptureState(pc uint64, op script.OpCode, gas, cost uint64, scope *script.ScopeContext, rData []byte, depth int, err error) {
	if !t.traceStep {
		return
	}
	if t.err != nil {
...
func (t *jsTracer) CaptureFault(pc uint64, op script.OpCode, gas, cost uint64, scope *script.ScopeContext, depth int, err error) {
	if t.err != nil {
		return
	}
...
func (t *jsTracer) CaptureEnter(typ script.OpCode, from util.Address, to util.Address, input []byte, gas uint64, value *big.Int) {
	if !t.traceFrame {
		return
	}
	if t.err != nil {
	...
	t.frame.from = from
	t.frame.to = to
	t.frame.input = util.CopyBytes(input)
...
func (t *jsTracer) CaptureExit(output []byte, gasUsed uint64, err error) {
	...
	t.frameResult.output = util.CopyBytes(output)
...
func (t *jsTracer) setBuiltinFunctions() {
	...
	vm.Set("toContract2", func(from goja.Value, salt string, initcode goja.Value) goja.Value {
		...
		code = util.CopyBytes(code)
		codeHash := crypto.Keccak256(code)
		b := crypto.CreateAddress2(addr, util.HexToHash(salt), codeHash).Bytes()
		res, err := t.toBuf(vm, b)
		if err != nil {
			vm.Interrupt(err)
			return nil
