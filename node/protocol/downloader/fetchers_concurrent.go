	...
	for {
			...
			for _, peer := range d.peers.AllPeers() {
				...
				if pending == nil && stale == nil {
					...
					if waited := time.Since(stale.Sent); waited > timeoutGracePeriod {
						...
						// stall the sync.
						peer.log.Warn("Peer stalling, dropping", "waited", util.PrettyDuration(waited))
						d.dropPeer(peer.id)
					}
				}
			}
		...
		case <-timeout.C:
			...
			if now, at := time.Now(), time.Unix(0, -exp); now.Before(at) {
				logging.Error("Timeout triggered but not reached", "left", at.Sub(now))
			...
			if peer == nil {
				...
				logging.Error("Delivery timeout from unknown peer", "peer", req.Peer)
				continue
			}
			if fails > 2 {
