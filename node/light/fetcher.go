...
import (
	...
	"github.com/ParallaxProtocol/parallax/node/light/fetcher"
...
type fetcherPeer struct {
	...
	announces map[util.Hash]*announce // Announcement map
	fifo      []util.Hash             // FIFO announces list
}
...
func newLightFetcher(chain *light.LightChain, engine consensus.Engine, peers *serverPeerSet, ulc *ulc, chaindb dbstore.Database, reqDist *requestDistributor, syncFn func(p *serverPeer)) *lightFetcher {
...
func (f *lightFetcher) registerPeer(p *serverPeer) {
	...
	f.peers[p.ID()] = &fetcherPeer{announces: make(map[util.Hash]*announce)}
}
...
func (f *lightFetcher) mainloop() {
	...
	var (
		...
		ulc          = f.ulc != nil
		headCh       = make(chan validation.ChainHeadEvent, 100)
	...
	trustedHeader := func(hash util.Hash, number uint64) (bool, []enode.ID) {
		var (
			agreed  []enode.ID
			trusted bool
		)
	...
	for {
		...
		case anno := <-f.announceCh:
			...
			logging.Debug("Received new announce", "peer", peerid, "number", data.Number, "hash", data.Hash, "reorg", data.ReorgDepth)

			peer := f.peer(peerid)
			if peer == nil {
				logging.Debug("Receive announce from unknown peer", "peer", peerid)
				continue
			}
			...
			if peer.latest != nil && data.Td.Cmp(peer.latest.Td) <= 0 {
				...
				logging.Debug("Non-monotonic td", "peer", peerid, "current", data.Td, "previous", peer.latest.Td)
				continue
			}
			peer.latest = data
			...
			if !ulc && !syncing {
				...
				if data.Number > localHead.Number.Uint64()+syncInterval || data.ReorgDepth > 0 {
					...
					logging.Debug("Trigger light sync", "peer", peerid, "local", localHead.Number, "localhash", localHead.Hash(), "remote", data.Number, "remotehash", data.Hash)
					continue
				}
				f.fetcher.Notify(peerid.String(), data.Hash, data.Number, time.Now(), f.requestHeaderByHash(peerid), nil)
				logging.Debug("Trigger header retrieval", "peer", peerid, "number", data.Number, "hash", data.Hash)
			}
			...
			if ulc && anno.trust {
				...
				if trusted && !syncing {
					if data.Number > localHead.Number.Uint64()+syncInterval || data.ReorgDepth > 0 {
						syncing = true
						go f.startSync(peerid)
						logging.Debug("Trigger trusted light sync", "local", localHead.Number, "localhash", localHead.Hash(), "remote", data.Number, "remotehash", data.Hash)
						continue
					}
					p := agreed[rand.Intn(len(agreed))]
					f.fetcher.Notify(p.String(), data.Hash, data.Number, time.Now(), f.requestHeaderByHash(p), nil)
					logging.Debug("Trigger trusted header retrieval", "number", data.Number, "hash", data.Hash)
				}
			}
		...
		case <-requestTimer.C:
			for reqid, request := range fetching {
				if time.Since(request.sendAt) > blockDelayTimeout-gatherSlack {
					...
					logging.Debug("Request timeout", "peer", request.peerid, "reqid", reqid)
				}
			}
			f.rescheduleTimer(fetching, requestTimer)
		...
		case resp := <-f.deliverCh:
			if req := fetching[resp.reqid]; req != nil {
				...
				if len(resp.headers) != 1 {
					...
					logging.Debug("Deliver more than requested", "peer", req.peerid, "reqid", req.reqid)
					continue
				}
				if resp.headers[0].Hash() != req.hash {
					f.peerset.unregister(req.peerid.String())
					logging.Debug("Deliver invalid header", "peer", req.peerid, "reqid", req.reqid)
					continue
				}
		...
		case ev := <-headCh:
			...
			for _, id := range droplist {
				...
				logging.Debug("Kicked out peer for invalid announcement")
			}
		...
		case origin := <-f.syncDone:
			...
			logging.Debug("light sync finished", "number", localHead.Number, "hash", localHead.Hash())
...
func (f *lightFetcher) trackRequest(peerid enode.ID, reqid uint64, hash util.Hash) {
	select {
...
func (f *lightFetcher) requestHeaderByHash(peerid enode.ID) func(util.Hash) error {
