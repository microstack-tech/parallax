...
func testGetBlockHeaders(t *testing.T, protocol int) {
	...
	tests := []struct {
		...
		expect []util.Hash          // The hashes of the block whose headers are expected
	}{
		// A single random block should be retrievable by hash and number too
		{
			&GetBlockHeadersData{Origin: hashOrNumber{Hash: bc.GetBlockByNumber(limit / 2).Hash()}, Amount: 1},
			[]util.Hash{bc.GetBlockByNumber(limit / 2).Hash()},
		},
		{
...
func testGetBlockBodies(t *testing.T, protocol int) {
	...
	tests := []struct {
		...
		explicit  []util.Hash // Explicitly requested blocks
		...
		{0, []util.Hash{bc.Genesis().Hash()}, []bool{true}, 1},      // The genesis block should be retrievable
		{0, []util.Hash{bc.CurrentBlock().Hash()}, []bool{true}, 1}, // The chains head block should be retrievable
		{0, []util.Hash{{}}, []bool{false}, 0},                      // A non existent block should not be returned
...
func testGetStaleCode(t *testing.T, protocol int) {
	netconfig := testnetConfig{
		blocks:    validation.TriesInMemory + 4,
		protocol:  protocol,
		nopruning: true,
	}
	server, _, tearDown := newClientServerEnv(t, netconfig)
...
func testGetProofs(t *testing.T, protocol int) {
	...
	accounts := []util.Address{bankAddr, userAddr1, userAddr2, signerAddr, {}}
	for i := uint64(0); i <= bc.CurrentBlock().NumberU64(); i++ {
		header := bc.GetHeaderByNumber(i)
...
func testGetCHTProofs(t *testing.T, protocol int) {
	var (
		...
		waitIndexers = func(cIndexer, bIndexer, btIndexer *validation.ChainIndexer) {
			for {
...
func testTransactionStatus(t *testing.T, protocol int) {
	...
	test := func(tx *types.Transaction, send bool, expStatus light.TxStatus) {
		...
		if send {
			...
			sendRequest(rawPeer.app, GetTxStatusMsg, reqID, []util.Hash{tx.Hash()})
		}
	...
	tx0, _ := types.SignTx(types.NewTransaction(0, userAddr1, big.NewInt(10000), chainparams.TxGas, nil, nil), signer, bankKey)
	test(tx0, true, light.TxStatus{Status: validation.TxStatusUnknown, Error: validation.ErrUnderpriced.Error()})

	tx1, _ := types.SignTx(types.NewTransaction(0, userAddr1, big.NewInt(10000), chainparams.TxGas, big.NewInt(100000000000), nil), signer, bankKey)
	test(tx1, false, light.TxStatus{Status: validation.TxStatusUnknown}) // query before sending, should be unknown
	test(tx1, true, light.TxStatus{Status: validation.TxStatusPending})  // send valid processable tx, should return pending
	test(tx1, true, light.TxStatus{Status: validation.TxStatusPending})  // adding it again should not return an error

	tx2, _ := types.SignTx(types.NewTransaction(1, userAddr1, big.NewInt(10000), chainparams.TxGas, big.NewInt(100000000000), nil), signer, bankKey)
	tx3, _ := types.SignTx(types.NewTransaction(2, userAddr1, big.NewInt(10000), chainparams.TxGas, big.NewInt(100000000000), nil), signer, bankKey)
	// send transactions in the wrong order, tx3 should be queued
	test(tx3, true, light.TxStatus{Status: validation.TxStatusQueued})
	test(tx2, true, light.TxStatus{Status: validation.TxStatusPending})
	// query again, now tx3 should be pending too
	test(tx3, false, light.TxStatus{Status: validation.TxStatusPending})

	// generate and add a block with tx1 and tx2 included
	gchain, _ := validation.GenerateChain(chainparams.TestChainConfig, chain.GetBlockByNumber(0), xhash.NewFaker(), server.db, 1, func(i int, block *validation.BlockGen) {
		block.AddTx(tx1)
		block.AddTx(tx2)
	})
	...
	test(tx1, false, light.TxStatus{Status: validation.TxStatusIncluded, Lookup: &rawdb.LegacyTxLookupEntry{BlockHash: block1hash, BlockIndex: 1, Index: 0}})

	test(tx2, false, light.TxStatus{Status: validation.TxStatusIncluded, Lookup: &rawdb.LegacyTxLookupEntry{BlockHash: block1hash, BlockIndex: 1, Index: 1}})

	// create a reorg that rolls them back
	gchain, _ = validation.GenerateChain(chainparams.TestChainConfig, chain.GetBlockByNumber(0), xhash.NewFaker(), server.db, 2, func(i int, block *validation.BlockGen) {})
	if _, err := chain.InsertChain(gchain); err != nil {
		panic(err)
	}
	...
	test(tx1, false, light.TxStatus{Status: validation.TxStatusPending})
	test(tx2, false, light.TxStatus{Status: validation.TxStatusPending})
}
