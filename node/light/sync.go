...
func (h *clientHandler) validateCheckpoint(peer *serverPeer) error {
	...
	logging.Warn("Verified advertised checkpoint", "peer", peer.id, "signers", len(signers))
	return nil
}
...
func (h *clientHandler) synchronise(peer *serverPeer) {
	...
	case checkpoint.Empty():
		mode = lightSync
		logging.Debug("Disable checkpoint syncing", "reason", "empty checkpoint")
	case latest.Number.Uint64() >= (checkpoint.SectionIndex+1)*h.backend.iConfig.ChtSize-1:
		mode = lightSync
		logging.Debug("Disable checkpoint syncing", "reason", "local chain beyond the checkpoint")
	case local:
		mode = legacyCheckpointSync
		logging.Debug("Disable checkpoint syncing", "reason", "checkpoint is hardcoded")
	case h.backend.oracle == nil || !h.backend.oracle.IsRunning():
		...
		logging.Debug("Disable checkpoint syncing", "reason", "checkpoint syncing is not activated")
	}
	...
	if mode == checkpointSync || mode == legacyCheckpointSync {
		...
		if mode == checkpointSync {
			if err := h.validateCheckpoint(peer); err != nil {
				logging.Debug("Failed to validate checkpoint", "reason", err)
				h.removePeer(peer.id)
				return
			}
			h.backend.blockchain.AddTrustedCheckpoint(checkpoint)
		}
		logging.Debug("Checkpoint syncing start", "peer", peer.id, "checkpoint", checkpoint.SectionIndex)
		...
		if !checkpoint.Empty() && !h.backend.blockchain.SyncCheckpoint(ctx, checkpoint) {
			logging.Debug("Sync checkpoint failed")
			h.removePeer(peer.id)
			return
		}
	}
	...
	if err := h.downloader.Synchronise(peer.id, peer.Head(), peer.Td(), downloader.LightSync); err != nil {
		logging.Debug("Synchronise failed", "reason", err)
		return
	}
	logging.Debug("Synchronise finished", "elapsed", util.PrettyDuration(time.Since(start)))
}
