...
func (lprl *LightParallax) stateAtTransaction(ctx context.Context, block *types.Block, txIndex int, reexec uint64) (validation.Message, script.BlockContext, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		context := validation.NewPVMBlockContext(block.Header(), lprl.blockchain, nil)
		statedb.Prepare(tx.Hash(), idx)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := script.NewPVM(context, txContext, statedb, lprl.blockchain.Config(), script.Config{})
