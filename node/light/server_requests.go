...
func handleGetBlockHeaders(msg Decoder) (serveRequestFn, uint64, uint64, error) {
	...
	return func(backend serverBackend, p *clientPeer, waitOrStop func() bool) *reply {
		...
		var (
			...
			hashMode        = r.Query.Origin.Hash != (util.Hash{})
			first           = true
		...
		for !unknown && len(headers) < int(r.Query.Amount) && bytes < softResponseLimit {
			...
			case hashMode && r.Query.Reverse:
				...
				if ancestor == 0 {
					...
					unknown = r.Query.Origin.Hash == util.Hash{}
				}
			case hashMode && !r.Query.Reverse:
...
func handleGetCode(msg Decoder) (serveRequestFn, uint64, uint64, error) {
	...
	return func(backend serverBackend, p *clientPeer, waitOrStop func() bool) *reply {
		...
		for i, request := range r.Reqs {
			...
			if !backend.ArchiveMode() && header.Number.Uint64()+validation.TriesInMemory <= local {
			...
			account, err := getAccount(triedb, header.Root, util.BytesToHash(request.AccKey))
			if err != nil {
				p.Log().Warn("Failed to retrieve account for code", "block", header.Number, "hash", header.Hash(), "account", util.BytesToHash(request.AccKey), "err", err)
				p.bumpInvalid()
				continue
			}
			code, err := bc.StateCache().ContractCode(util.BytesToHash(request.AccKey), util.BytesToHash(account.CodeHash))
			if err != nil {
				p.Log().Warn("Failed to retrieve account code", "block", header.Number, "hash", header.Hash(), "account", util.BytesToHash(request.AccKey), "codehash", util.BytesToHash(account.CodeHash), "err", err)
				continue
			}
	...
	return func(backend serverBackend, p *clientPeer, waitOrStop func() bool) *reply {
		...
		for i, request := range r.Reqs {
			...
			if request.BHash != lastBHash {
				root, lastBHash = util.Hash{}, request.BHash
			...
			default:
				...
				account, err := getAccount(statedb.TrieDB(), root, util.BytesToHash(request.AccKey))
				if err != nil {
					p.Log().Warn("Failed to retrieve account for proof", "block", header.Number, "hash", header.Hash(), "account", util.BytesToHash(request.AccKey), "err", err)
					p.bumpInvalid()
					continue
				}
				trie, err = statedb.OpenStorageTrie(util.BytesToHash(request.AccKey), account.Root)
				if trie == nil || err != nil {
					p.Log().Warn("Failed to open storage trie for proof", "block", header.Number, "hash", header.Hash(), "account", util.BytesToHash(request.AccKey), "root", account.Root, "err", err)
					continue
				}
			}
	...
	return func(backend serverBackend, p *clientPeer, waitOrStop func() bool) *reply {
		...
		for i, request := range r.Reqs {
			...
			if request.Type == htCanonical && request.AuxReq == htAuxHeader && len(request.Key) == 8 {
				...
				if err != nil {
					logging.Error("Failed to encode header", "err", err)
					return nil
				}
...
func handleSendTx(msg Decoder) (serveRequestFn, uint64, uint64, error) {
	...
	return func(backend serverBackend, p *clientPeer, waitOrStop func() bool) *reply {
		...
		for i, tx := range r.Txs {
			...
			if stats[i].Status == validation.TxStatusUnknown {
...
func txStatus(b serverBackend, hash util.Hash) light.TxStatus {
	var stat light.TxStatus
	// Looking the transaction in txpool first.
	stat.Status = b.TxPool().Status([]util.Hash{hash})[0]

	// If the transaction is unknown to the pool, try looking it up locally.
	if stat.Status == validation.TxStatusUnknown {
		lookup := b.BlockChain().GetTransactionLookup(hash)
		if lookup != nil {
			stat.Status = validation.TxStatusIncluded
			stat.Lookup = lookup
		}
	}
	return stat
