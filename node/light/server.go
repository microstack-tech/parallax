...
func NewLesServer(node *node.Node, e prlBackend, config *nodeconfig.Config) (*LesServer, error) {
	...
	if !checkpoint.Empty() {
		logging.Info("Loaded latest checkpoint", "section", checkpoint.SectionIndex, "head", checkpoint.SectionHead,
...
func (s *LesServer) Stop() error {
	...
	s.wg.Wait()
	logging.Info("Les server stopped")
	...
	for {
		...
		case totalCapacity = <-totalCapacityCh:
			...
			if newFreePeers < freePeers && newFreePeers < uint64(s.config.LightPeers) {
				logging.Warn("Reduced free peer connections", "from", freePeers, "to", newFreePeers)
			}
