...
import (
	...
	"github.com/ParallaxProtocol/parallax/node/light/contracts/checkpointoracle"
...
func New(config *chainparams.CheckpointOracleConfig, getLocal func(uint64) chainparams.TrustedCheckpoint) *CheckpointOracle {
...
func (oracle *CheckpointOracle) Start(backend bind.ContractBackend) {
	...
	if err != nil {
		logging.Error("Oracle contract binding failed", "err", err)
		return
	}
	if !atomic.CompareAndSwapInt32(&oracle.running, 0, 1) {
		logging.Error("Already bound and listening to registrar")
		return
	}
...
func (oracle *CheckpointOracle) StableCheckpoint() (*chainparams.TrustedCheckpoint, uint64) {
	oracle.checkMu.Lock()
...
func (oracle *CheckpointOracle) VerifySigners(index uint64, hash [32]byte, signatures [][]byte) (bool, []util.Address) {
	// Short circuit if the given signatures doesn't reach the threshold.
	if len(signatures) < int(oracle.config.Threshold) {
		return false, nil
	}
	var (
		signers []util.Address
		checked = make(map[util.Address]struct{})
	)
	...
	if uint64(len(signers)) < threshold {
		logging.Warn("Not enough signers to approve checkpoint", "signers", len(signers), "threshold", threshold)
		return false, nil
	}
	return true, signers
}
