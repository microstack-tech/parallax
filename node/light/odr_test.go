...
type odrTestFn func(ctx context.Context, db dbstore.Database, config *chainparams.ChainConfig, bc *validation.BlockChain, lc *light.LightChain, bhash util.Hash) []byte
...
func odrGetBlock(ctx context.Context, db dbstore.Database, config *chainparams.ChainConfig, bc *validation.BlockChain, lc *light.LightChain, bhash util.Hash) []byte {
...
func odrGetReceipts(ctx context.Context, db dbstore.Database, config *chainparams.ChainConfig, bc *validation.BlockChain, lc *light.LightChain, bhash util.Hash) []byte {
...
func odrAccounts(ctx context.Context, db dbstore.Database, config *chainparams.ChainConfig, bc *validation.BlockChain, lc *light.LightChain, bhash util.Hash) []byte {
	dummyAddr := util.HexToAddress("1234567812345678123456781234567812345678")
	acc := []util.Address{bankAddr, userAddr1, userAddr2, dummyAddr}
...
func odrContractCall(ctx context.Context, db dbstore.Database, config *chainparams.ChainConfig, bc *validation.BlockChain, lc *light.LightChain, bhash util.Hash) []byte {
	...
	for i := 0; i < 3; i++ {
		...
		if bc != nil {
			...
			if err == nil {
				...
				msg := callmsg{types.NewMessage(from.Address(), &testContractAddr, 0, new(big.Int), 100000, big.NewInt(chainparams.InitialBaseFee), big.NewInt(chainparams.InitialBaseFee), new(big.Int), data, nil, true)}

				context := validation.NewPVMBlockContext(header, bc, nil)
				txContext := validation.NewPVMTxContext(msg)
				vmenv := script.NewPVM(context, txContext, statedb, config, script.Config{NoBaseFee: true})
			...
			msg := callmsg{types.NewMessage(bankAddr, &testContractAddr, 0, new(big.Int), 100000, big.NewInt(chainparams.InitialBaseFee), big.NewInt(chainparams.InitialBaseFee), new(big.Int), data, nil, true)}
			context := validation.NewPVMBlockContext(header, lc, nil)
			txContext := validation.NewPVMTxContext(msg)
			vmenv := script.NewPVM(context, txContext, state, config, script.Config{NoBaseFee: true})
			gp := new(validation.GasPool).AddGas(math.MaxUint64)
			result, _ := validation.ApplyMessage(vmenv, msg, gp)
			if state.Error() == nil {
				res = append(res, result.Return()...)
...
func odrTxStatus(ctx context.Context, db dbstore.Database, config *chainparams.ChainConfig, bc *validation.BlockChain, lc *light.LightChain, bhash util.Hash) []byte {
...
func testOdr(t *testing.T, protocol int, expFail uint64, checkCached bool, fn odrTestFn) {
	...
	client.peer.speer.hasBlockHook = func(util.Hash, uint64, bool) bool { return false }
	client.handler.backend.peers.lock.Unlock()
	test(expFail)

	// expect all retrievals to pass
	client.handler.backend.peers.lock.Lock()
	client.peer.speer.hasBlockHook = func(util.Hash, uint64, bool) bool { return true }
	client.handler.backend.peers.lock.Unlock()
	test(5)
...
func testGetTxStatusFromUnindexedPeers(t *testing.T, protocol int) {
	...
	var (
		...
		txs          = make(map[util.Hash]*types.Transaction) // Transaction objects set
		blockNumbers = make(map[util.Hash]uint64)             // Transaction hash to block number mappings
		blockHashes  = make(map[util.Hash]util.Hash)          // Transaction hash to block hash mappings
		intraIndex   = make(map[util.Hash]uint64)             // Transaction intra-index in block
	)
	for number := uint64(1); number < server.backend.Blockchain().CurrentBlock().NumberU64(); number++ {
		...
		for index, tx := range block.Transactions() {
			...
			if testHash == (util.Hash{}) {
				testHash = tx.Hash()
				testStatus = light.TxStatus{
					Status: validation.TxStatusIncluded,
	...
	serveMsg := func(peer *testPeer, txLookup uint64) error {
		...
		for i, hash := range r.Hashes {
			...
			stats[i].Status = validation.TxStatusIncluded
	...
	testspecs := []struct {
		...
		// Retrieve mixed transactions from the full peers
		{
			peers:     3,
			txLookups: []uint64{txIndexUnlimited, txIndexUnlimited, txIndexUnlimited},
			txs:       []util.Hash{randomHash(), testHash},
			results:   []light.TxStatus{{}, testStatus},
		},
