...
func (f *fetcherTester) makeHeaderFetcher(peer string, blocks map[util.Hash]*types.Block, drift time.Duration) headerRequesterFn {
	...
	return func(hash util.Hash) error {
...
func (f *fetcherTester) makeBodyFetcher(peer string, blocks map[util.Hash]*types.Block, drift time.Duration) bodyRequesterFn {
	...
	return func(hashes []util.Hash) error {
...
func testConcurrentAnnouncements(t *testing.T, light bool) {
	...
	counter := uint32(0)
	firstHeaderWrapper := func(hash util.Hash) error {
		atomic.AddUint32(&counter, 1)
		return firstHeaderFetcher(hash)
	}
	secondHeaderWrapper := func(hash util.Hash) error {
		atomic.AddUint32(&counter, 1)
...
func testPendingDeduplication(t *testing.T, light bool) {
	...
	counter := uint32(0)
	headerWrapper := func(hash util.Hash) error {
		atomic.AddUint32(&counter, 1)
