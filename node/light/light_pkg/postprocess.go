...
func GetChtRoot(db dbstore.Database, sectionIdx uint64, sectionHead util.Hash) util.Hash {
...
func StoreChtRoot(db dbstore.Database, sectionIdx uint64, sectionHead, root util.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	disablePruning       bool
	diskdb, trieTable    dbstore.Database
	odr                  OdrBackend
...
func NewChtIndexer(db dbstore.Database, odr OdrBackend, size, confirms uint64, disablePruning bool) *validation.ChainIndexer {
	...
	return validation.NewChainIndexer(db, rawdb.NewTable(db, "chtIndexV2-"), backend, size, confirms, time.Millisecond*100, "cht")
}

// fetchMissingNodes tries to retrieve the last entry of the latest trusted CHT from the
// ODR backend in order to be able to add new entries and calculate subsequent root hashes
func (c *ChtIndexerBackend) fetchMissingNodes(ctx context.Context, section uint64, root util.Hash) error {
...
func (c *ChtIndexerBackend) Reset(ctx context.Context, section uint64, lastSectionHead util.Hash) error {
	var root util.Hash
	if section > 0 {
...
func (c *ChtIndexerBackend) Commit() error {
	...
	if !c.disablePruning {
		...
		c.trieset.Clear()
		c.triedb.Commit(root, false, func(hash util.Hash) { c.trieset.Add(hash) })
		...
		for it.Next() {
			...
			if !c.trieset.Contains(util.BytesToHash(trimmed)) {
		...
		logging.Debug("Prune historical CHT trie nodes", "deleted", deleted, "remaining", remaining, "elapsed", util.PrettyDuration(time.Since(t)))
	} else {
		c.triedb.Commit(root, false, nil)
	}
	logging.Info("Storing CHT", "section", c.section, "head", fmt.Sprintf("%064x", c.lastHash), "root", fmt.Sprintf("%064x", root))
...
func (c *ChtIndexerBackend) Prune(threshold uint64) error {
	...
	logging.Debug("Prune history headers", "threshold", threshold, "elapsed", util.PrettyDuration(time.Since(t)))
	return nil
}
...
func GetBloomTrieRoot(db dbstore.Database, sectionIdx uint64, sectionHead util.Hash) util.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db dbstore.Database, sectionIdx uint64, sectionHead, root util.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type BloomTrieIndexerBackend struct {
	...
	trie              *trie.Trie
	sectionHeads      []util.Hash
}

// NewBloomTrieIndexer creates a BloomTrie chain indexer
func NewBloomTrieIndexer(db dbstore.Database, odr OdrBackend, parentSize, size uint64, disablePruning bool) *validation.ChainIndexer {
	...
	backend.sectionHeads = make([]util.Hash, backend.bloomTrieRatio)
	return validation.NewChainIndexer(db, rawdb.NewTable(db, "bltIndex-"), backend, size, 0, time.Millisecond*100, "bloomtrie")
}

// fetchMissingNodes tries to retrieve the last entries of the latest trusted bloom trie from the
// ODR backend in order to be able to add new entries and calculate subsequent root hashes
func (b *BloomTrieIndexerBackend) fetchMissingNodes(ctx context.Context, section uint64, root util.Hash) error {
...
// Reset implements core.ChainIndexerBackend
func (b *BloomTrieIndexerBackend) Reset(ctx context.Context, section uint64, lastSectionHead util.Hash) error {
	var root util.Hash
	if section > 0 {
...
func (b *BloomTrieIndexerBackend) Commit() error {
	...
	if !b.disablePruning {
		// Flush the triedb and track the latest trie nodes.
		b.trieset.Clear()
		b.triedb.Commit(root, false, func(hash util.Hash) { b.trieset.Add(hash) })
		...
		for it.Next() {
			...
			if !b.trieset.Contains(util.BytesToHash(trimmed)) {
		...
		logging.Debug("Prune historical bloom trie nodes", "deleted", deleted, "remaining", remaining, "elapsed", util.PrettyDuration(time.Since(t)))
	} else {
		b.triedb.Commit(root, false, nil)
	}
	sectionHead := b.sectionHeads[b.bloomTrieRatio-1]
	StoreBloomTrieRoot(b.diskdb, b.section, sectionHead, root)
	logging.Info("Storing bloom trie", "section", b.section, "head", fmt.Sprintf("%064x", sectionHead), "root", fmt.Sprintf("%064x", root), "compression", float64(compSize)/float64(decompSize))
...
func (b *BloomTrieIndexerBackend) Prune(threshold uint64) error {
	...
	logging.Debug("Prune history bloombits", "threshold", threshold, "elapsed", util.PrettyDuration(time.Since(start)))
	return nil
}
