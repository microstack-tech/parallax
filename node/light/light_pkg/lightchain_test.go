...
func makeHeaderChain(parent *types.Header, n int, db dbstore.Database, seed int) []*types.Header {
	blocks, _ := validation.GenerateChain(chainparams.TestChainConfig, types.NewBlockWithHeader(parent), xhash.NewFaker(), db, n, func(i int, b *validation.BlockGen) {
...
// header only chain.
func newCanonical(n int) (dbstore.Database, *LightChain, error) {
	db := rawdb.NewMemoryDatabase()
	gspec := validation.Genesis{Config: chainparams.TestChainConfig}
...
func (odr *dummyOdr) Database() dbstore.Database {
	return odr.db
}
...
func TestBadHeaderHashes(t *testing.T) {
	...
	validation.BadHashes[headers[2].Hash()] = true
	if _, err = bc.InsertHeaderChain(headers, 1); !errors.Is(err, validation.ErrBannedHash) {
		t.Errorf("error mismatch: have: %v, want %v", err, validation.ErrBannedHash)
	}
}
...
func TestReorgBadHeaderHashes(t *testing.T) {
	...
	validation.BadHashes[headers[3].Hash()] = true
	defer func() { delete(validation.BadHashes, headers[3].Hash()) }()

	// Create a new LightChain and check that it rolled back the state.
	ncm, err := NewLightChain(&dummyOdr{db: bc.chainDb}, chainparams.TestChainConfig, xhash.NewFaker(), nil)
	if err != nil {
