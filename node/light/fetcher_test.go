...
func testGappedAnnouncements(t *testing.T, protocol int) {
	...
	blocks, _ := validation.GenerateChain(rawdb.ReadChainConfig(s.db, s.backend.Blockchain().Genesis().Hash()), s.backend.Blockchain().GetBlockByNumber(3),
		xhash.NewFaker(), s.db, 2, func(i int, gen *validation.BlockGen) {
...
func testTrustedAnnouncement(t *testing.T, protocol int) {
	...
	cp := &chainparams.TrustedCheckpoint{
		SectionIndex: 0,
		SectionHead:  head,
...
func testInvalidAnnounces(t *testing.T, protocol int) {
	...
	td := big.NewInt(chainparams.GenesisDifficulty.Int64() + 200) // bad td
