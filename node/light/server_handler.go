...
func newServerHandler(server *LesServer, blockchain *validation.BlockChain, chainDb dbstore.Database, txpool *validation.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) beforeHandle(p *clientPeer, reqID, responseCount uint64, msg p2p.Msg, reqCnt uint64, maxCount uint64) (*servingTask, uint64) {
	...
	if !accepted {
		p.freeze()
		p.Log().Error("Request came too early", "remaining", util.PrettyDuration(time.Duration(bufShort*1000000/p.fcParams.MinRecharge)))
		p.fcClient.OneTimeCost(inSizeCost)
		return nil, 0
	}
...
func (h *serverHandler) BlockChain() *validation.BlockChain {
	return h.blockchain
}

// TxPool implements serverBackend
func (h *serverHandler) TxPool() *validation.TxPool {
	return h.txpool
}
...
func getAccount(triedb *trie.Database, root, hash util.Hash) (types.StateAccount, error) {
...
func (h *serverHandler) broadcastLoop() {
	...
	headCh := make(chan validation.ChainHeadEvent, 10)
	...
	for {
		...
		case ev := <-headCh:
			...
			logging.Debug("Announcing block to peers", "number", number, "hash", hash, "td", td, "reorg", reorg)
