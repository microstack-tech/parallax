...
func (set *unconfirmedBlocks) Insert(index uint64, hash util.Hash) {
	...
	logging.Debug("Block found", "number", index, "hash", hash)
}
...
func (set *unconfirmedBlocks) Shift(height uint64) {
	...
	for set.blocks != nil {
		...
		switch {
		case header == nil:
			logging.Warn("Failed to retrieve header of mined block", "number", next.index, "hash", next.hash)
		case header.Hash() == next.hash:
			// log.Info("💰 Block reached coinbase maturity", "height", next.index, "hash", next.hash)
		default:
			logging.Info("✴️ Block lost", "height", next.index, "hash", next.hash)
		}
