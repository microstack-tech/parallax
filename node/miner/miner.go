...
type Config struct {
	Coinbase   util.Address  `toml:",omitempty"` // Public address for block mining rewards (default = first account)
...
func New(parallax Backend, config *Config, chainConfig *chainparams.ChainConfig, mux *event.TypeMux, engine kernel.Engine, isLocalBlock func(header *types.Header) bool) *Miner {
	miner := &Miner{
		...
		startCh:  make(chan util.Address),
		...
		case ev := <-dlEventCh:
			...
			case downloader.StartEvent:
				...
				if wasMining {
					...
					shouldStart = true
					logging.Info("Mining aborted due to sync")
				}
...
func (miner *Miner) Start(coinbase util.Address) {
...
func (miner *Miner) SetExtra(extra []byte) error {
	if uint64(len(extra)) > chainparams.MaximumExtraDataSize {
		return fmt.Errorf("extra exceeds max length. %d > %v", len(extra), chainparams.MaximumExtraDataSize)
	}
...
func (miner *Miner) SetCoinbase(addr util.Address) {
	miner.coinbase = addr
...
func (miner *Miner) GetSealingBlockAsync(parent util.Hash, timestamp uint64, coinbase util.Address, noTxs bool) (chan *types.Block, error) {
...
// will be returned.
func (miner *Miner) GetSealingBlockSync(parent util.Hash, timestamp uint64, coinbase util.Address, noTxs bool) (*types.Block, error) {
