...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	...
	for i := 0; i < benchFilterCnt; i++ {
		...
		addr[0] = byte(i)
		addr[1] = byte(i / 256)
		filter := NewRangeFilter(backend, 0, int64(cnt*sectionSize-1), []util.Address{addr}, nil)
...
func clearBloomBits(db dbstore.Database) {
...
func BenchmarkNoBloomBits(b *testing.B) {
	...
	filter := NewRangeFilter(backend, 0, int64(*headNum), []util.Address{{}}, nil)
