...
func (b *testBackend) GetReceipts(ctx context.Context, hash util.Hash) (types.Receipts, error) {
	if number := rawdb.ReadHeaderNumber(b.db, hash); number != nil {
		return rawdb.ReadReceipts(b.db, hash, *number, chainparams.TestChainConfig), nil
	}
	return nil, nil
}

func (b *testBackend) GetLogs(ctx context.Context, hash util.Hash) ([][]*types.Log, error) {
	number := rawdb.ReadHeaderNumber(b.db, hash)
	if number == nil {
		return nil, nil
	}
	receipts := rawdb.ReadReceipts(b.db, hash, *number, chainparams.TestChainConfig)
...
func (b *testBackend) SubscribeNewTxsEvent(ch chan<- validation.NewTxsEvent) event.Subscription {
	return b.txFeed.Subscribe(ch)
}

func (b *testBackend) SubscribeRemovedLogsEvent(ch chan<- validation.RemovedLogsEvent) event.Subscription {
...
func (b *testBackend) SubscribeChainEvent(ch chan<- validation.ChainEvent) event.Subscription {
...
func TestBlockSubscription(t *testing.T) {
	...
	for _, blk := range chain {
		chainEvents = append(chainEvents, validation.ChainEvent{Hash: blk.Hash(), Block: blk})
	}
...
func TestPendingTxFilter(t *testing.T) {
	...
	var (
		...
		transactions = []*types.Transaction{
			...
			types.NewTransaction(1, util.HexToAddress("0xb794f5ea0ba39494ce83a213fffba74279579268"), new(big.Int), 0, new(big.Int), nil),
			types.NewTransaction(2, util.HexToAddress("0xb794f5ea0ba39494ce83a213fffba74279579268"), new(big.Int), 0, new(big.Int), nil),
			types.NewTransaction(3, util.HexToAddress("0xb794f5ea0ba39494ce83a213fffba74279579268"), new(big.Int), 0, new(big.Int), nil),
			types.NewTransaction(4, util.HexToAddress("0xb794f5ea0ba39494ce83a213fffba74279579268"), new(big.Int), 0, new(big.Int), nil),
		}
	...
	time.Sleep(1 * time.Second)
	backend.txFeed.Send(validation.NewTxsEvent{Txs: transactions})
...
func TestLogFilter(t *testing.T) {
	...
	var (
		...
		thirdAddress   = util.HexToAddress("0x3333333333333333333333333333333333333333")
		notUsedAddress = util.HexToAddress("0x9999999999999999999999999999999999999999")
		firstTopic     = util.HexToHash("0x1111111111111111111111111111111111111111111111111111111111111111")
		secondTopic    = util.HexToHash("0x2222222222222222222222222222222222222222222222222222222222222222")
		notUsedTopic   = util.HexToHash("0x9999999999999999999999999999999999999999999999999999999999999999")

		// posted twice, once as regular logs and once as pending logs.
		allLogs = []*types.Log{
			{Address: firstAddr},
			{Address: firstAddr, Topics: []util.Hash{firstTopic}, BlockNumber: 1},
			{Address: secondAddr, Topics: []util.Hash{firstTopic}, BlockNumber: 1},
			{Address: thirdAddress, Topics: []util.Hash{secondTopic}, BlockNumber: 2},
			{Address: thirdAddress, Topics: []util.Hash{secondTopic}, BlockNumber: 3},
		}
		...
		testCases = []struct {
			...
			1: {FilterCriteria{Addresses: []util.Address{{}, notUsedAddress}, Topics: [][]util.Hash{nil}}, []*types.Log{}, ""},
			// match logs based on addresses, ignore topics
			2: {FilterCriteria{Addresses: []util.Address{firstAddr}}, allLogs[:2], ""},
			// match none due to no matching topics (match with address)
			3: {FilterCriteria{Addresses: []util.Address{secondAddr}, Topics: [][]util.Hash{{notUsedTopic}}}, []*types.Log{}, ""},
			// match logs based on addresses and topics
			4: {FilterCriteria{Addresses: []util.Address{thirdAddress}, Topics: [][]util.Hash{{firstTopic, secondTopic}}}, allLogs[3:5], ""},
			// match logs based on multiple addresses and "or" topics
			5: {FilterCriteria{Addresses: []util.Address{secondAddr, thirdAddress}, Topics: [][]util.Hash{{firstTopic, secondTopic}}}, allLogs[2:5], ""},
			// logs in the pending block
			6: {FilterCriteria{Addresses: []util.Address{firstAddr}, FromBlock: big.NewInt(rpc.PendingBlockNumber.Int64()), ToBlock: big.NewInt(rpc.PendingBlockNumber.Int64())}, allLogs[:2], ""},
			...
			10: {FilterCriteria{FromBlock: big.NewInt(1), ToBlock: big.NewInt(2), Topics: [][]util.Hash{{secondTopic}}}, allLogs[3:4], ""},
			// all "mined" and pending logs with topic firstTopic
			11: {FilterCriteria{FromBlock: big.NewInt(rpc.LatestBlockNumber.Int64()), ToBlock: big.NewInt(rpc.PendingBlockNumber.Int64()), Topics: [][]util.Hash{{firstTopic}}}, expectedCase11, ""},
			// match all logs due to wildcard topic
			12: {FilterCriteria{Topics: [][]util.Hash{nil}}, allLogs[1:], ""},
		}
	)
...
func TestPendingLogsSubscription(t *testing.T) {
	...
	var (
		...
		firstTopic     = util.HexToHash("0x1111111111111111111111111111111111111111111111111111111111111111")
		secondTopic    = util.HexToHash("0x2222222222222222222222222222222222222222222222222222222222222222")
		thirdTopic     = util.HexToHash("0x3333333333333333333333333333333333333333333333333333333333333333")
		fourthTopic    = util.HexToHash("0x4444444444444444444444444444444444444444444444444444444444444444")
		notUsedTopic   = util.HexToHash("0x9999999999999999999999999999999999999999999999999999999999999999")

		allLogs = [][]*types.Log{
			{{Address: firstAddr, Topics: []util.Hash{}, BlockNumber: 0}},
			{{Address: firstAddr, Topics: []util.Hash{firstTopic}, BlockNumber: 1}},
			{{Address: secondAddr, Topics: []util.Hash{firstTopic}, BlockNumber: 2}},
			{{Address: thirdAddress, Topics: []util.Hash{secondTopic}, BlockNumber: 3}},
			{{Address: thirdAddress, Topics: []util.Hash{secondTopic}, BlockNumber: 4}},
			{
				{Address: thirdAddress, Topics: []util.Hash{firstTopic}, BlockNumber: 5},
				{Address: thirdAddress, Topics: []util.Hash{thirdTopic}, BlockNumber: 5},
				{Address: thirdAddress, Topics: []util.Hash{fourthTopic}, BlockNumber: 5},
				{Address: firstAddr, Topics: []util.Hash{firstTopic}, BlockNumber: 5},
			},
		}
		...
		testCases = []struct {
			...
			// match none due to no matching addresses
			{
				parallax.FilterQuery{Addresses: []util.Address{{}, notUsedAddress}, Topics: [][]util.Hash{nil}, FromBlock: pendingBlockNumber, ToBlock: pendingBlockNumber},
				nil,
				nil, nil, nil,
			},
			// match logs based on addresses, ignore topics
			{
				parallax.FilterQuery{Addresses: []util.Address{firstAddr}, FromBlock: pendingBlockNumber, ToBlock: pendingBlockNumber},
				append(flattenLogs(allLogs[:2]), allLogs[5][3]),
				nil, nil, nil,
			},
			// match none due to no matching topics (match with address)
			{
				parallax.FilterQuery{Addresses: []util.Address{secondAddr}, Topics: [][]util.Hash{{notUsedTopic}}, FromBlock: pendingBlockNumber, ToBlock: pendingBlockNumber},
				nil,
				nil, nil, nil,
			},
			// match logs based on addresses and topics
			{
				parallax.FilterQuery{Addresses: []util.Address{thirdAddress}, Topics: [][]util.Hash{{firstTopic, secondTopic}}, FromBlock: pendingBlockNumber, ToBlock: pendingBlockNumber},
				append(flattenLogs(allLogs[3:5]), allLogs[5][0]),
				nil, nil, nil,
			},
			// match logs based on multiple addresses and "or" topics
			{
				parallax.FilterQuery{Addresses: []util.Address{secondAddr, thirdAddress}, Topics: [][]util.Hash{{firstTopic, secondTopic}}, FromBlock: pendingBlockNumber, ToBlock: pendingBlockNumber},
				append(flattenLogs(allLogs[2:5]), allLogs[5][0]),
				nil, nil, nil,
			},
			// multiple pending logs, should match only 2 topics from the logs in block 5
			{
				parallax.FilterQuery{Addresses: []util.Address{thirdAddress}, Topics: [][]util.Hash{{firstTopic, fourthTopic}}, FromBlock: pendingBlockNumber, ToBlock: pendingBlockNumber},
...
func TestPendingTxFilterDeadlock(t *testing.T) {
	...
	go func() {
		...
		for {
			...
			tx := types.NewTransaction(i, util.HexToAddress("0xb794f5ea0ba39494ce83a213fffba74279579268"), new(big.Int), 0, new(big.Int), nil)
			backend.txFeed.Send(validation.NewTxsEvent{Txs: []*types.Transaction{tx}})
			i++
		}
	}()
