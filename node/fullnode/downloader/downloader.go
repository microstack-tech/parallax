...
var (
	...
	fullMaxForkAncestry  uint64 = netparams.FullImmutabilityThreshold  // Maximum chain reorganisation (locally redeclared so tests can reduce it)
	lightMaxForkAncestry uint64 = netparams.LightImmutabilityThreshold // Maximum chain reorganisation (locally redeclared so tests can reduce it)
...
type Downloader struct {
	...
	stateDB dbstore.Database // Database to state sync into (and deduplicate via)
	...
	// Status
	synchroniseMock func(id string, hash util.Hash) error // Replacement for synchronise during testing
	synchronising   int32
	notified        int32
	committed       int32
...
type LightChain interface {
	...
	HasHeader(util.Hash, uint64) bool

	// GetHeaderByHash retrieves a header from the local chain.
	GetHeaderByHash(util.Hash) *types.Header

	// CurrentHeader retrieves the head header from the local chain.
	CurrentHeader() *types.Header

	// GetTd returns the total difficulty of a local block.
	GetTd(util.Hash, uint64) *big.Int
...
type BlockChain interface {
	...
	HasBlock(util.Hash, uint64) bool

	// HasFastBlock verifies a snap block's presence in the local chain.
	HasFastBlock(util.Hash, uint64) bool

	// GetBlockByHash retrieves a block from the local chain.
	GetBlockByHash(util.Hash) *types.Block
	...
	SnapSyncCommitHead(util.Hash) error
...
func New(checkpoint uint64, stateDb dbstore.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn, success func()) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
func (d *Downloader) Progress() parallax.SyncProgress {
	...
	default:
		logging.Error("Unknown downloader chain/mode combo", "light", d.lightchain != nil, "full", d.blockchain != nil, "mode", mode)
	}
...
func (d *Downloader) LegacySync(id string, head util.Hash, td *big.Int, mode SyncMode) error {
	...
	if errors.Is(err, errInvalidChain) || errors.Is(err, errBadPeer) || errors.Is(err, errTimeout) ||
		...
		logging.Warn("Synchronisation failed, dropping peer", "peer", id, "err", err)
		if d.dropPeer == nil {
			// The dropPeer method is nil when `--copydb` is used for a local copy.
			// Timeouts can occur if e.g. compaction hits at the wrong time, and can be ignored
			logging.Warn("Downloader wants to drop peer, but peerdrop-function is not set", "peer", id)
		} else {
			d.dropPeer(id)
		}
		return err
	}
	logging.Warn("Synchronisation failed, retrying", "err", err)
	return err
}
...
func (d *Downloader) synchronise(id string, hash util.Hash, td *big.Int, mode SyncMode) error {
	...
	if atomic.CompareAndSwapInt32(&d.notified, 0, 1) {
		logging.Info("Block synchronisation started")
	}
	if mode == SnapSync {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash util.Hash, td *big.Int) (err error) {
	...
	logging.Debug("Synchronising with the network", "peer", p.id, "parallax", p.version, "head", hash, "td", td, "mode", mode)
	defer func(start time.Time) {
		logging.Debug("Synchronisation terminated", "elapsed", util.PrettyDuration(time.Since(start)))
	}(time.Now())
	...
	if mode == SnapSync {
		...
		if origin >= frozen && frozen != 0 {
			d.ancientLimit = 0
			logging.Info("Disabling direct-ancient mode", "origin", origin, "ancient", frozen-1)
		} else if d.ancientLimit > 0 {
			logging.Debug("Enabling direct-ancient mode", "ancient", d.ancientLimit)
		}
...
func (d *Downloader) findAncestorSpanSearch(p *peerConnection, mode SyncMode, remoteHeight, localHeight uint64, floor int64) (uint64, error) {
	...
	number, hash := uint64(0), util.Hash{}
		...
		if pivoting {
			if len(headers) == 2 {
				if have, want := headers[0].Number.Uint64(), pivot+uint64(fsMinFullBlocks); have != want {
					logging.Warn("Peer sent invalid next pivot", "have", have, "want", want)
				...
				if have, want := headers[1].Number.Uint64(), pivot+2*uint64(fsMinFullBlocks)-8; have != want {
					logging.Warn("Peer sent invalid pivot confirmer", "have", have, "want", want)
					return fmt.Errorf("%w: next pivot confirmer number %d != requested %d", errInvalidChain, have, want)
				}
				logging.Warn("Pivot seemingly stale, moving", "old", pivot, "new", headers[0].Number)
...
func (d *Downloader) fillHeaderSkeleton(from uint64, skeleton []*types.Header) ([]*types.Header, []util.Hash, int, error) {
	logging.Debug("Filling up skeleton", "from", from)
	d.queue.ScheduleSkeleton(from, skeleton)

	err := d.concurrentFetch((*headerQueue)(d))
	if err != nil {
		logging.Debug("Skeleton fill failed", "err", err)
	}
	filled, hashes, proced := d.queue.RetrieveHeaders()
	if err == nil {
		logging.Debug("Skeleton fill succeeded", "filled", len(filled), "processed", proced)
	}
...
func (d *Downloader) fetchBodies(from uint64) error {
	logging.Debug("Downloading block bodies", "origin", from)
...
func (d *Downloader) fetchReceipts(from uint64) error {
	logging.Debug("Downloading receipts", "origin", from)
	err := d.concurrentFetch((*receiptQueue)(d))

	logging.Debug("Receipt download terminated", "err", err)
	return err
}
...
func (d *Downloader) processHeaders(origin uint64, td *big.Int) error {
	...
	defer func() {
		if rollback > 0 {
			lastHeader, lastFastBlock, lastBlock := d.lightchain.CurrentHeader().Number, util.Big0, util.Big0
			if mode != LightSync {
			...
			if err := d.lightchain.SetHead(rollback - 1); err != nil { // -1 to target the parent of the first uncertain block
				...
				logging.Error("Failed to roll back chain segment", "head", rollback-1, "err", err)
			}
			curFastBlock, curBlock := util.Big0, util.Big0
			if mode != LightSync {
				curFastBlock = d.blockchain.CurrentFastBlock().Number()
				curBlock = d.blockchain.CurrentBlock().Number()
			}
			logging.Warn("Rolled back chain segment",
				...
				if mode == SnapSync || mode == LightSync {
					...
					if len(chunkHeaders) > 0 {
						if n, err := d.lightchain.InsertHeaderChain(chunkHeaders, frequency); err != nil {
							...
							logging.Warn("Invalid header encountered", "number", chunkHeaders[n].Number, "hash", chunkHashes[n], "parent", chunkHeaders[n].ParentHash, "err", err)
							return fmt.Errorf("%w: %v", errInvalidChain, err)
						}
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	logging.Debug("Inserting downloaded chain", "items", len(results),
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			logging.Debug("Downloaded item processing failed on sidechain import", "index", index, "err", err)
		}
		return fmt.Errorf("%w: %v", errInvalidChain, err)
	}
	return nil
	...
	for {
		...
		if atomic.LoadInt32(&d.committed) == 0 {
			...
			if height := latest.Number.Uint64(); height >= pivot.Number.Uint64()+2*uint64(fsMinFullBlocks)-uint64(reorgProtHeaderDelay) {
				logging.Warn("Pivot became stale, moving", "old", pivot.Number.Uint64(), "new", height-uint64(fsMinFullBlocks)+uint64(reorgProtHeaderDelay))
...
func (d *Downloader) commitSnapSyncData(results []*fetchResult, stateSync *stateSync) error {
	...
	// Retrieve the a batch of results to import
	first, last := results[0].Header, results[len(results)-1].Header
	logging.Debug("Inserting snap-sync blocks", "items", len(results),
		"firstnum", first.Number, "firsthash", first.Hash(),
...
func (d *Downloader) commitPivotBlock(result *fetchResult) error {
	...
	logging.Debug("Committing snap sync pivot as new head", "number", block.Number(), "hash", block.Hash())
