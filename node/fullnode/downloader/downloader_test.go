...
func newTesterWithNotification(t *testing.T, success func()) *downloadTester {
	...
	validation.GenesisBlockForTesting(db, testAddress, big.NewInt(1000000000000000))

	chain, err := validation.NewBlockChain(db, nil, chainparams.TestChainConfig, xhash.NewFaker(), script.Config{}, nil, nil)
	if err != nil {
		panic(err)
	}
...
func (dlp *downloadTesterPeer) RequestHeadersByHash(origin util.Hash, amount int, skip int, reverse bool, sink chan *prl.Response) (*prl.Request, error) {
...
func (dlp *downloadTesterPeer) RequestReceipts(hashes []util.Hash, sink chan *prl.Response) (*prl.Request, error) {
...
func (dlp *downloadTesterPeer) RequestAccountRange(id uint64, root, origin, limit util.Hash, bytes uint64) error {
...
func (dlp *downloadTesterPeer) RequestStorageRanges(id uint64, root util.Hash, accounts []util.Hash, origin, limit []byte, bytes uint64) error {
	// Create the request and service it
...
func (dlp *downloadTesterPeer) RequestByteCodes(id uint64, hashes []util.Hash, bytes uint64) error {
...
func (dlp *downloadTesterPeer) Log() logging.Logger {
	return logging.New("peer", dlp.id)
}
...
func TestCanonicalSynchronisation66Snap(t *testing.T) {
	testCanonSync(t, prl.Parallax66, SnapSync)
}
...
func TestBoundedForkedSync66Snap(t *testing.T) {
	testBoundedForkedSync(t, prl.Parallax66, SnapSync)
}
...
func TestEmptyShortCircuit66Snap(t *testing.T) {
	testEmptyShortCircuit(t, prl.Parallax66, SnapSync)
}
...
func TestForkedSyncProgress66Snap(t *testing.T) {
	testForkedSyncProgress(t, prl.Parallax66, SnapSync)
}
...
func TestFailedSyncProgress66Snap(t *testing.T) {
	testFailedSyncProgress(t, prl.Parallax66, SnapSync)
}
...
func TestFakedSyncProgress66Snap(t *testing.T) {
	testFakedSyncProgress(t, prl.Parallax66, SnapSync)
}
