...
func (q *bodyQueue) unreserve(peer string) int {
	...
	if fails > 2 {
		logging.Trace("Body delivery timed out", "peer", peer)
	} else {
		logging.Debug("Body delivery stalling", "peer", peer)
	}
	return fails
}
...
func (q *bodyQueue) request(peer *peerConnection, req *fetchRequest, resCh chan *prl.Response) (*prl.Request, error) {
	...
	hashes := make([]util.Hash, 0, len(req.Headers))
...
func (q *bodyQueue) deliver(peer *peerConnection, packet *prl.Response) (int, error) {
	...
	hashsets := packet.Meta.([][]util.Hash) // {txs hashes, uncle hashes}
