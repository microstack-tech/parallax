...
func (d *Downloader) fetchHeadersByHash(p *peerConnection, hash util.Hash, amount int, skip int, reverse bool) ([]*types.Header, []util.Hash, error) {
	...
	case res := <-resCh:
		...
		return *res.Res.(*prl.BlockHeadersPacket), res.Meta.([]util.Hash), nil
	}
}
...
// handles all the cancellation, interruption and timeout mechanisms of a data
// retrieval to allow blocking API calls.
func (d *Downloader) fetchHeadersByNumber(p *peerConnection, number uint64, amount int, skip int, reverse bool) ([]*types.Header, []util.Hash, error) {
	// Create the response sink and send the network request
	start := time.Now()
	resCh := make(chan *prl.Response)
