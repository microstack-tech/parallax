...
type queue struct {
	...
	headerHead      util.Hash                      // Hash of the last queued header to verify order
	...
	headerHashes    []util.Hash                    // Result cache accumulating the completed header hashes
	...
	blockTaskPool  map[util.Hash]*types.Header // Pending block (body) retrieval tasks, mapping hashes to headers
	...
	receiptTaskPool  map[util.Hash]*types.Header // Pending receipt retrieval tasks, mapping hashes to headers
	...
	resultSize  util.StorageSize // Approximate size of a block (exponential moving average)
...
func (q *queue) Reset(blockCacheLimit int, thresholdInitialSize int) {
	...
	q.headerHead = util.Hash{}
	q.headerPendPool = make(map[string]*fetchRequest)

	q.blockTaskPool = make(map[util.Hash]*types.Header)
	q.blockTaskQueue.Reset()
	q.blockPendPool = make(map[string]*fetchRequest)

	q.receiptTaskPool = make(map[util.Hash]*types.Header)
...
func (q *queue) ScheduleSkeleton(from uint64, skeleton []*types.Header) {
	...
	q.headerHashes = make([]util.Hash, len(skeleton)*MaxHeaderFetch)
	q.headerProced = 0
	q.headerOffset = from
...
// skeleton.
func (q *queue) RetrieveHeaders() ([]*types.Header, []util.Hash, int) {
	q.lock.Lock()
	defer q.lock.Unlock()
...
func (q *queue) Schedule(headers []*types.Header, hashes []util.Hash, from uint64) []*types.Header {
	q.lock.Lock()
	defer q.lock.Unlock()
	...
	for i, header := range headers {
		...
		if header.Number == nil || header.Number.Uint64() != from {
			logging.Warn("Header broke chain ordering", "number", header.Number, "hash", hash, "expected", from)
			break
		}
		if q.headerHead != (util.Hash{}) && q.headerHead != header.ParentHash {
			logging.Warn("Header broke chain ancestry", "number", header.Number, "hash", hash)
			break
		}
		...
		if _, ok := q.blockTaskPool[hash]; ok {
			logging.Warn("Header already scheduled for block fetch", "number", header.Number, "hash", hash)
		} else {
		...
		if q.mode == SnapSync && !header.EmptyReceipts() {
			if _, ok := q.receiptTaskPool[hash]; ok {
				logging.Warn("Header already scheduled for receipt fetch", "number", header.Number, "hash", hash)
			} else {
...
func (q *queue) Results(block bool) []*fetchResult {
	...
	for _, result := range results {
		...
		q.resultSize = util.StorageSize(blockCacheSizeWeight)*size +
			(1-util.StorageSize(blockCacheSizeWeight))*q.resultSize
	}
	// Using the newly calibrated resultsize, figure out the new throttle limit
	// on the result cache
	throttleThreshold := uint64((util.StorageSize(blockCacheMemory) + q.resultSize - 1) / q.resultSize)
	...
	if time.Since(q.lastStatLog) > 60*time.Second {
		...
		logging.Info("Downloader queue stats", info...)
	}
	return results
}
...
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[util.Hash]*types.Header, taskQueue *prque.Prque,
	...
	for proc := 0; len(send) < count && !taskQueue.Empty(); proc++ {
		...
		if stale {
			...
			proc = proc - 1
			logging.Error("Fetch reservation already delivered", "number", header.Number.Uint64())
			continue
		}
		if throttle {
		...
		if err != nil {
			...
			logging.Warn("Failed to reserve headers", "err", err)
			// There are no resultslots available. Leave it in the task queue
			break
		}
		if item.Done(kind) {
...
func (q *queue) expire(peer string, pendPool map[string]*fetchRequest, taskQueue *prque.Prque) int {
	...
	req := pendPool[peer]
	if req == nil {
		logging.Error("Expired request does not exist", "peer", peer)
		return 0
	}
...
func (q *queue) DeliverHeaders(id string, headers []*types.Header, hashes []util.Hash, headerProcCh chan *headerTask) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()
	...
	if ready > 0 {
		...
		processHashes := make([]util.Hash, ready)
...
func (q *queue) DeliverReceipts(id string, receiptList [][]*types.Receipt, receiptListHashes []util.Hash) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()
...
// to access the queue, so they already need a lock anyway.
func (q *queue) deliver(id string, taskPool map[util.Hash]*types.Header,
	...
	for _, header := range request.Headers[:i] {
		if res, stale, err := q.resultCache.GetDeliverySlot(header.Number.Uint64()); err == nil {
			...
			logging.Error("Delivery stale", "stale", stale, "number", header.Number.Uint64(), "err", err)
