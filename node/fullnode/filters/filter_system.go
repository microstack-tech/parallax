...
type EventSystem struct {
	...
	txsCh         chan validation.NewTxsEvent      // Channel to receive new transactions event
	logsCh        chan []*types.Log                // Channel to receive new log event
	pendingLogsCh chan []*types.Log                // Channel to receive new log event
	rmLogsCh      chan validation.RemovedLogsEvent // Channel to receive removed log event
	chainCh       chan validation.ChainEvent       // Channel to receive new chain event
}
...
func NewEventSystem(backend Backend, lightMode bool) *EventSystem {
	m := &EventSystem{
		...
		rmLogsCh:      make(chan validation.RemovedLogsEvent, rmLogsChanSize),
		pendingLogsCh: make(chan []*types.Log, logsChanSize),
		chainCh:       make(chan validation.ChainEvent, chainEvChanSize),
	}
...
// transactions that enter the transaction pool.
func (es *EventSystem) SubscribePendingTxs(hashes chan []util.Hash) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) handleRemovedLogs(filters filterIndex, ev validation.RemovedLogsEvent) {
	for _, f := range filters[LogsSubscription] {
...
func (es *EventSystem) handleTxsEvent(filters filterIndex, ev validation.NewTxsEvent) {
	hashes := make([]util.Hash, 0, len(ev.Txs))
...
func (es *EventSystem) handleChainEvent(filters filterIndex, ev validation.ChainEvent) {
...
func (es *EventSystem) lightFilterLogs(header *types.Header, addresses []util.Address, topics [][]util.Hash, remove bool) []*types.Log {
	if bloomFilter(header.Bloom, addresses, topics) {
		...
		if len(logs) > 0 && logs[0].TxHash == (util.Hash{}) {
