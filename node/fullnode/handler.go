...
import (
	...
	"github.com/ParallaxProtocol/parallax/v2/node/fullnode/fetcher"
...
type txPool interface {
	...
	Pending(enforceTips bool) map[util.Address]types.Transactions
...
type handlerConfig struct {
	Database       dbstore.Database               // Database for direct sync insertions
	Chain          *validation.BlockChain         // Blockchain to serve data from
	TxPool         txPool                         // Transaction pool to propagate from
	Network        uint64                         // Network identifier to adfvertise
	Sync           downloader.SyncMode            // Whether to snap or full sync
	BloomCache     uint64                         // Megabytes to alloc for snap sync bloom
	EventMux       *event.TypeMux                 // Legacy event mux, deprecate for `feed`
	Checkpoint     *chainparams.TrustedCheckpoint // Hard coded checkpoint for sync challenges
	RequiredBlocks map[uint64]util.Hash           // Hard coded map of required block hashes for sync challenges
}
...
type handler struct {
	...
	checkpointHash   util.Hash // Block hash for the sync progress validator to cross reference
	...
	requiredBlocks map[uint64]util.Hash
...
func newHandler(config *handlerConfig) (*handler, error) {
	...
	if config.Sync == downloader.FullSync {
		...
		if fullBlock.NumberU64() == 0 && fastBlock.NumberU64() > 0 {
			h.snapSync = uint32(1)
			logging.Warn("Switch sync mode from full sync to snap sync")
		}
	} else {
		if h.chain.CurrentBlock().NumberU64() > 0 {
			// Print warning log if database is not empty to run snap sync.
			logging.Warn("Switch sync mode from snap sync to full sync")
		} else {
	...
	success := func() {
		...
		if atomic.LoadUint32(&h.snapSync) == 1 {
			logging.Info("Snap sync complete, auto disabling")
	...
	inserter := func(blocks types.Blocks) (int, error) {
		...
		if h.chain.CurrentBlock().NumberU64() < h.checkpointNumber {
			logging.Warn("Unsynced yet, discarded propagated block", "number", blocks[0].Number(), "hash", blocks[0].Hash())
			return 0, nil
		}
		...
		if atomic.LoadUint32(&h.snapSync) == 1 {
			logging.Warn("Fast syncing, discarded propagated block", "number", blocks[0].Number(), "hash", blocks[0].Hash())
			return 0, nil
		}
	...
	fetchTx := func(peer string, hashes []util.Hash) error {
		p := h.peers.peer(peer)
		if p == nil {
...
func (h *handler) runSnapExtension(peer *snap.Peer, handler snap.Handler) error {
	...
	if err := h.peers.registerSnapExtension(peer); err != nil {
		peer.Log().Debug("Snapshot extension registration failed", "err", err, "caps", peer.Caps())
		return err
	}
	return handler(peer)
}
...
func (h *handler) Start(maxPeers int) {
	...
	h.wg.Add(1)
	h.txsCh = make(chan validation.NewTxsEvent, txChanSize)
	h.txsSub = h.txpool.SubscribeNewTxsEvent(h.txsCh)
	go h.txBroadcastLoop()

	// broadcast mined blocks
	h.wg.Add(1)
	h.minedBlockSub = h.eventMux.Subscribe(validation.NewMinedBlockEvent{})
...
func (h *handler) Stop() {
	...
	logging.Info("Parallax protocol stopped")
}
...
func (h *handler) BroadcastBlock(block *types.Block, propagate bool) {
	...
	if propagate {
		...
		if parent := h.chain.GetBlock(block.ParentHash(), block.NumberU64()-1); parent != nil {
			...
			logging.Error("Propagating dangling block", "number", block.Number(), "hash", hash)
			return
		}
		...
		for _, peer := range peers {
			if peer.Info().Network.Trusted {
				logging.Trace("Sending full block to trusted peer", "number", block.Number(), "hash", hash, "peer", peer.ID())
				peer.AsyncSendNewBlock(block, td)
			}
		}

		// Send the block to a subset of our peers (untrusted)
		...
		logging.Trace("Propagated block", "hash", hash, "recipients", len(transfer), "duration", util.PrettyDuration(time.Since(block.ReceivedAt)))
		return
	}
	...
	if h.chain.HasBlock(hash, block.NumberU64()) {
		...
		logging.Trace("Announced block", "hash", hash, "recipients", len(peers), "duration", util.PrettyDuration(time.Since(block.ReceivedAt)))
	}
}
...
func (h *handler) BroadcastTransactions(txs types.Transactions) {
	var (
		...
		txset = make(map[*parallaxPeer][]util.Hash) // Set peer->hash to transfer directly
		annos = make(map[*parallaxPeer][]util.Hash) // Set peer->hash to announce
	...
	logging.Debug("Transaction broadcast", "txs", len(txs),
...
func (h *handler) minedBroadcastLoop() {
	...
	for obj := range h.minedBlockSub.Chan() {
		if ev, ok := obj.Data.(validation.NewMinedBlockEvent); ok {
