...
type txAnnounce struct {
	...
	hashes []util.Hash // Batch of transaction hashes being announced
}

// txRequest represents an in-flight transaction retrieval request destined to
// a specific peers.
type txRequest struct {
	hashes []util.Hash            // Transactions having been requested
	stolen map[util.Hash]struct{} // Deliveries by someone else (don't re-request)
...
type txDelivery struct {
	origin string      // Identifier of the peer originating the notification
	hashes []util.Hash // Batch of transaction hashes having been delivered
...
type TxFetcher struct {
	...
	waitlist  map[util.Hash]map[string]struct{} // Transactions waiting for an potential broadcast
	waittime  map[util.Hash]mclock.AbsTime      // Timestamps when transactions were added to the waitlist
	waitslots map[string]map[util.Hash]struct{} // Waiting announcement sgroupped by peer (DoS protection)

	// Stage 2: Queue of transactions that waiting to be allocated to some peer
	// to be retrieved directly.
	announces map[string]map[util.Hash]struct{} // Set of announced transactions, grouped by origin peer
	announced map[util.Hash]map[string]struct{} // Set of download locations, grouped by transaction hash

	// Stage 3: Set of transactions currently being retrieved, some which may be
	// fulfilled and some rescheduled. Note, this step shares 'announces' from the
	// previous stage to avoid having to duplicate (need it for DoS checks).
	fetching   map[util.Hash]string              // Transaction set currently being retrieved
...
func NewTxFetcher(hasTx func(util.Hash) bool, addTxs func([]*types.Transaction) []error, fetchTxs func(string, []util.Hash) error) *TxFetcher {
...
func NewTxFetcherForTests(
	hasTx func(util.Hash) bool, addTxs func([]*types.Transaction) []error, fetchTxs func(string, []util.Hash) error,
	...
	return &TxFetcher{
		...
		waitlist:    make(map[util.Hash]map[string]struct{}),
		waittime:    make(map[util.Hash]mclock.AbsTime),
		waitslots:   make(map[string]map[util.Hash]struct{}),
		announces:   make(map[string]map[util.Hash]struct{}),
		announced:   make(map[util.Hash]map[string]struct{}),
		fetching:    make(map[util.Hash]string),
		requests:    make(map[string]*txRequest),
		alternates:  make(map[util.Hash]map[string]struct{}),
...
func (f *TxFetcher) Notify(peer string, hashes []util.Hash) error {
	...
	var (
		unknowns               = make([]util.Hash, 0, len(hashes))
	...
	for {
			...
			for _, hash := range ann.hashes {
				...
				if f.alternates[hash] != nil {
					...
					if announces := f.announces[ann.origin]; announces != nil {
						...
						f.announces[ann.origin] = map[util.Hash]struct{}{hash: {}}
					}
					continue
				}
				...
				if f.waitlist[hash] != nil {
					...
					if waitslots := f.waitslots[ann.origin]; waitslots != nil {
						...
						f.waitslots[ann.origin] = map[util.Hash]struct{}{hash: {}}
					}
					continue
				}
				...
				if time.Duration(f.clock.Now()-instance)+txGatherSlack > txArriveTimeout {
					...
					for peer := range f.waitlist[hash] {
						if announces := f.announces[peer]; announces != nil {
							...
							f.announces[peer] = map[util.Hash]struct{}{hash: {}}
						}
		...
		case delivery := <-f.cleanup:
			...
			for _, hash := range delivery.hashes {
				if _, ok := f.waitlist[hash]; ok {
					...
					if origin, ok := f.fetching[hash]; ok && (origin != delivery.origin || !delivery.direct) {
						...
						if stolen == nil {
							f.requests[origin].stolen = make(map[util.Hash]struct{})
							stolen = f.requests[origin].stolen
						}
						stolen[hash] = struct{}{}
					}
			...
			if delivery.direct {
				...
				if req == nil {
					logging.Warn("Unexpected transaction delivery", "peer", delivery.origin)
					break
				}
				...
				delivered := make(map[util.Hash]struct{})
				for _, hash := range delivery.hashes {
...
func (f *TxFetcher) scheduleFetches(timer *mclock.Timer, timeout chan struct{}, whitelist map[string]struct{}) {
	...
	f.forEachPeer(actives, func(peer string) {
		...
		hashes := make([]util.Hash, 0, maxTxRetrievals)
		f.forEachHash(f.announces[peer], func(hash util.Hash) bool {
...
// testing it does a deterministic sorted random to allow reproducing issues.
func (f *TxFetcher) forEachHash(hashes map[util.Hash]struct{}, do func(hash util.Hash) bool) {
	// If we're running production, use whatever Go's map gives us
	if f.rand == nil {
	...
	// We're running the test suite, make iteration deterministic
	list := make([]util.Hash, 0, len(hashes))
	for hash := range hashes {
...
func sortHashes(slice []util.Hash) {
...
// used in tests to simulate random map iteration but keep it deterministic.
func rotateHashes(slice []util.Hash, n int) {
	orig := make([]util.Hash, len(slice))
	copy(orig, slice)
