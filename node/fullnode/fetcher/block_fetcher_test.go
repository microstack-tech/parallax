...
var (
	...
	genesis      = validation.GenesisBlockForTesting(testdb, testAddress, big.NewInt(1000000000000000))
	unknownBlock = types.NewBlock(&types.Header{GasLimit: chainparams.GenesisGasLimit, BaseFee: big.NewInt(chainparams.InitialBaseFee)}, nil, nil, nil, trie.NewStackTrie(nil))
)
...
// reassembly.
func makeChain(n int, seed byte, parent *types.Block) ([]util.Hash, map[util.Hash]*types.Block) {
	blocks, _ := validation.GenerateChain(chainparams.TestChainConfig, parent, xhash.NewFaker(), testdb, n, func(i int, block *validation.BlockGen) {
		block.SetCoinbase(util.Address{seed})

		// If the block number is multiple of 3, send a bonus transaction to the miner
		if parent == genesis && i%3 == 0 {
			signer := types.MakeSigner(chainparams.TestChainConfig, block.Number())
			tx, err := types.SignTx(types.NewTransaction(block.TxNonce(testAddress), util.Address{seed}, big.NewInt(1000), chainparams.TxGas, block.BaseFee(), nil), signer, testKey)
			if err != nil {
				panic(err)
			}
			block.AddTx(tx)
	...
	hashes := make([]util.Hash, n+1)
	hashes[len(hashes)-1] = parent.Hash()
	blockm := make(map[util.Hash]*types.Block, n+1)
...
type fetcherTester struct {
	...
	hashes  []util.Hash                 // Hash chain belonging to the tester
	headers map[util.Hash]*types.Header // Headers belonging to the tester
	blocks  map[util.Hash]*types.Block  // Blocks belonging to the tester
...
func newTester(light bool) *fetcherTester {
	tester := &fetcherTester{
		hashes:  []util.Hash{genesis.Hash()},
		headers: map[util.Hash]*types.Header{genesis.Hash(): genesis.Header()},
...
func (f *fetcherTester) getHeader(hash util.Hash) *types.Header {
	f.lock.RLock()
...
func (f *fetcherTester) getBlock(hash util.Hash) *types.Block {
	f.lock.RLock()
	defer f.lock.RUnlock()
...
func (f *fetcherTester) makeHeaderFetcher(peer string, blocks map[util.Hash]*types.Block, drift time.Duration) headerRequesterFn {
	closure := make(map[util.Hash]*types.Block)
	for hash, block := range blocks {
		closure[hash] = block
	}
	// Create a function that return a header from the closure
	return func(hash util.Hash, sink chan *prl.Response) (*prl.Request, error) {
...
func (f *fetcherTester) makeBodyFetcher(peer string, blocks map[util.Hash]*types.Block, drift time.Duration) bodyRequesterFn {
	closure := make(map[util.Hash]*types.Block)
	for hash, block := range blocks {
		closure[hash] = block
	}
	// Create a function that returns blocks from the closure
	return func(hashes []util.Hash, sink chan *prl.Response) (*prl.Request, error) {
...
func verifyFetchingEvent(t *testing.T, fetching chan []util.Hash, arrive bool) {
	t.Helper()
...
func verifyCompletingEvent(t *testing.T, completing chan []util.Hash, arrive bool) {
	t.Helper()
...
func testConcurrentAnnouncements(t *testing.T, light bool) {
	...
	counter := uint32(0)
	firstHeaderWrapper := func(hash util.Hash, sink chan *prl.Response) (*prl.Request, error) {
		atomic.AddUint32(&counter, 1)
		return firstHeaderFetcher(hash, sink)
	}
	secondHeaderWrapper := func(hash util.Hash, sink chan *prl.Response) (*prl.Request, error) {
		atomic.AddUint32(&counter, 1)
...
func testPendingDeduplication(t *testing.T, light bool) {
	...
	counter := uint32(0)
	headerWrapper := func(hash util.Hash, sink chan *prl.Response) (*prl.Request, error) {
		atomic.AddUint32(&counter, 1)
...
func TestImportDeduplication(t *testing.T) {
	...
	fetching := make(chan []util.Hash)
	imported := make(chan any, len(hashes)-1)
	tester.fetcher.fetchingHook = func(hashes []util.Hash) { fetching <- hashes }
	tester.fetcher.importedHook = func(header *types.Header, block *types.Block) { imported <- block }
...
func TestDistantPropagationDiscarding(t *testing.T) {
	...
	tester.lock.Lock()
	tester.hashes = []util.Hash{head}
	tester.blocks = map[util.Hash]*types.Block{head: blocks[head]}
	tester.lock.Unlock()
...
func testDistantAnnouncementDiscarding(t *testing.T, light bool) {
	...
	tester.lock.Lock()
	tester.hashes = []util.Hash{head}
	tester.headers = map[util.Hash]*types.Header{head: blocks[head].Header()}
	tester.blocks = map[util.Hash]*types.Block{head: blocks[head]}
	tester.lock.Unlock()
	...
	tester.fetcher.fetchingHook = func(hashes []util.Hash) { fetching <- struct{}{} }
...
func testInvalidNumberAnnouncement(t *testing.T, light bool) {
	...
	tester.fetcher.announceChangeHook = func(hash util.Hash, b bool) {
		announced <- nil
	}
...
func TestEmptyBlockShortCircuit(t *testing.T) {
	...
	completing := make(chan []util.Hash)
	tester.fetcher.completingHook = func(hashes []util.Hash) { completing <- hashes }
...
func TestHashMemoryExhaustionAttack(t *testing.T) {
	...
	tester.fetcher.importedHook = func(header *types.Header, block *types.Block) { imported <- block }
	tester.fetcher.announceChangeHook = func(hash util.Hash, added bool) {
		if added {
...
func TestBlockMemoryExhaustionAttack(t *testing.T) {
	...
	tester.fetcher.importedHook = func(header *types.Header, block *types.Block) { imported <- block }
	tester.fetcher.queueChangeHook = func(hash util.Hash, added bool) {
		if added {
	...
	targetBlocks := hashLimit + 2*maxQueueDist
	hashes, blocks := makeChain(targetBlocks, 0, genesis)
	attack := make(map[util.Hash]*types.Block)
