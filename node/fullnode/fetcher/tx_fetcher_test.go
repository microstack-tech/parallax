...
var (
	...
	testTxs = []*types.Transaction{
		types.NewTransaction(5577006791947779410, util.Address{0x0f}, new(big.Int), 0, new(big.Int), nil),
		types.NewTransaction(15352856648520921629, util.Address{0xbb}, new(big.Int), 0, new(big.Int), nil),
		types.NewTransaction(3916589616287113937, util.Address{0x86}, new(big.Int), 0, new(big.Int), nil),
		types.NewTransaction(9828766684487745566, util.Address{0xac}, new(big.Int), 0, new(big.Int), nil),
	}
	// testTxsHashes is the hashes of the test transactions above
	testTxsHashes = []util.Hash{testTxs[0].Hash(), testTxs[1].Hash(), testTxs[2].Hash(), testTxs[3].Hash()}
)
...
func TestTransactionFetcherWaiting(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []any{
			...
			doTxNotify{peer: "A", hashes: []util.Hash{{0x01}, {0x03}, {0x05}}},
			isWaiting(map[string][]util.Hash{
			...
			doTxNotify{peer: "C", hashes: []util.Hash{{0x06}, {0x07}}},
			isWaiting(map[string][]util.Hash{
				"C": {{0x06}, {0x07}},
			}),
			doWait{time: txArriveTimeout, step: true},
			...
			doTxNotify{peer: "D", hashes: []util.Hash{{0x06}, {0x07}}},
			isScheduled{
				tracking: map[string][]util.Hash{
					"A": {{0x01}, {0x02}, {0x03}, {0x05}},
					"B": {{0x03}, {0x04}},
...
func TestTransactionFetcherSkipWaiting(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []any{
			...
			doTxNotify{peer: "A", hashes: []util.Hash{{0x02}, {0x03}}},
			isWaiting(map[string][]util.Hash{
				"A": {{0x03}},
			}),
			isScheduled{
			...
			doTxNotify{peer: "B", hashes: []util.Hash{{0x02}, {0x03}, {0x04}}},
			isWaiting(map[string][]util.Hash{
				"A": {{0x03}},
				"B": {{0x03}, {0x04}},
			}),
...
func TestTransactionFetcherSingletonRequesting(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []any{
			...
			doTxNotify{peer: "A", hashes: []util.Hash{{0x03}, {0x04}}},
			isWaiting(map[string][]util.Hash{
				"A": {{0x03}, {0x04}},
			}),
			isScheduled{
			...
			doTxNotify{peer: "B", hashes: []util.Hash{{0x02}, {0x03}, {0x05}, {0x06}}},
			isWaiting(map[string][]util.Hash{
				"B": {{0x05}, {0x06}},
			}),
			isScheduled{
	...
	testTransactionFetcherParallel(t, txFetcherTest{
		init: func() *TxFetcher {
			return NewTxFetcher(
				func(util.Hash) bool { return false },
				nil,
				func(origin string, hashes []util.Hash) error {
					<-proceed
		...
		steps: []any{
			...
			// data source.
			doTxNotify{peer: "B", hashes: []util.Hash{{0x02}}},
			isWaiting(nil),
			isScheduled{
				tracking: map[string][]util.Hash{
					"A": {{0x01}, {0x02}},
...
func TestTransactionFetcherMissingRescheduling(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []any{
			// Push an initial announcement through to the scheduled stage
			doTxNotify{peer: "A", hashes: []util.Hash{testTxsHashes[0], testTxsHashes[1], testTxsHashes[2]}},
			isWaiting(map[string][]util.Hash{
...
func TestTransactionFetcherMissingCleanup(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []any{
			// Push an initial announcement through to the scheduled stage
			doTxNotify{peer: "A", hashes: []util.Hash{testTxsHashes[0], testTxsHashes[1]}},
			isWaiting(map[string][]util.Hash{
...
func TestTransactionFetcherBroadcasts(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []any{
			...
			doTxNotify{peer: "A", hashes: []util.Hash{testTxsHashes[0]}},
			doWait{time: txArriveTimeout, step: true},
			doTxNotify{peer: "A", hashes: []util.Hash{testTxsHashes[1]}},
			doWait{time: txArriveTimeout, step: true},
			doTxNotify{peer: "A", hashes: []util.Hash{testTxsHashes[2]}},
...
func TestTransactionFetcherWaitTimerResets(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []any{
			doTxNotify{peer: "A", hashes: []util.Hash{{0x01}}},
			isWaiting(map[string][]util.Hash{
				"A": {{0x01}},
			}),
			isScheduled{nil, nil, nil},
			...
			doTxNotify{peer: "A", hashes: []util.Hash{{0x02}}},
			isWaiting(map[string][]util.Hash{
				"A": {{0x01}, {0x02}},
			}),
			isScheduled{nil, nil, nil},
...
func TestTransactionFetcherRateLimiting(t *testing.T) {
	...
	for i := 0; i < maxTxAnnounces; i++ {
		hashes = append(hashes, util.Hash{byte(i / 256), byte(i % 256)})
	}
...
func TestTransactionFetcherDoSProtection(t *testing.T) {
	// Create a slew of transactions and to announce them
	var hashesA []util.Hash
	for i := 0; i < maxTxAnnounces+1; i++ {
		hashesA = append(hashesA, util.Hash{0x01, byte(i / 256), byte(i % 256)})
	}
	var hashesB []util.Hash
	for i := 0; i < maxTxAnnounces+1; i++ {
		hashesB = append(hashesB, util.Hash{0x02, byte(i / 256), byte(i % 256)})
	}
	testTransactionFetcherParallel(t, txFetcherTest{
		init: func() *TxFetcher {
			return NewTxFetcher(
		...
		steps: []any{
			...
			isScheduled{
				...
				fetching: map[string][]util.Hash{
					...
					"B": append(append([]util.Hash{}, hashesB[maxTxAnnounces/2-3:maxTxAnnounces/2-1]...), hashesB[:maxTxRetrievals-2]...),
				},
			},
			// Ensure that adding even one more hash results in dropping the hash
			doTxNotify{peer: "A", hashes: []util.Hash{hashesA[maxTxAnnounces]}},
...
func TestTransactionFetcherUnderpricedDedup(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
			...
			return NewTxFetcher(
				...
				func(txs []*types.Transaction) []error {
					...
					for i := 0; i < len(errs); i++ {
						if i%2 == 0 {
							errs[i] = validation.ErrUnderpriced
						} else {
							errs[i] = validation.ErrReplaceUnderpriced
						}
					}
					return errs
				},
		...
		steps: []any{
			...
			doTxNotify{peer: "A", hashes: []util.Hash{testTxsHashes[0], testTxsHashes[1], testTxsHashes[2]}}, // [2] is needed to force a step in the fetcher
			isWaiting(map[string][]util.Hash{
				"A": {testTxsHashes[2]},
			}),
			isScheduled{nil, nil, nil},
...
func TestTransactionFetcherUnderpricedDoSProtection(t *testing.T) {
	...
	for i := 0; i < maxTxUnderpricedSetSize+1; i++ {
		txs = append(txs, types.NewTransaction(rand.Uint64(), util.Address{byte(rand.Intn(256))}, new(big.Int), 0, new(big.Int), nil))
	}
	hashes := make([]util.Hash, len(txs))
	for i, tx := range txs {
		hashes[i] = tx.Hash()
	}
	...
	for i := 0; i < maxTxUnderpricedSetSize/maxTxRetrievals; i++ {
		...
		steps = append(steps, isWaiting(map[string][]util.Hash{
	...
	testTransactionFetcher(t, txFetcherTest{
		...
		steps: append(steps, []any{
			...
			doTxNotify{peer: "A", hashes: []util.Hash{hashes[maxTxUnderpricedSetSize]}},
			doWait{time: txArriveTimeout, step: true},
...
func TestTransactionFetcherDrop(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []any{
			...
			doTxNotify{peer: "A", hashes: []util.Hash{{0x02}}},
			doWait{time: txArriveTimeout, step: true},
			doTxNotify{peer: "A", hashes: []util.Hash{{0x03}}},
...
func TestTransactionFetcherDropRescheduling(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []any{
			...
			doTxNotify{peer: "A", hashes: []util.Hash{{0x01}}},
			doWait{time: txArriveTimeout, step: true},
			doTxNotify{peer: "B", hashes: []util.Hash{{0x01}}},
...
func containsHash(slice []util.Hash, hash util.Hash) bool {
