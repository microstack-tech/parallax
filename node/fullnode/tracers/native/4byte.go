...
func (t *fourByteTracer) isPrecompiled(addr util.Address) bool {
...
func (t *fourByteTracer) CaptureStart(env *script.PVM, from util.Address, to util.Address, create bool, input []byte, gas uint64, value *big.Int) {
	t.env = env
...
func (t *fourByteTracer) CaptureState(pc uint64, op script.OpCode, gas, cost uint64, scope *script.ScopeContext, rData []byte, depth int, err error) {
}

// CaptureEnter is called when PVM enters a new scope (via call, create or selfdestruct).
func (t *fourByteTracer) CaptureEnter(op script.OpCode, from util.Address, to util.Address, input []byte, gas uint64, value *big.Int) {
	...
	if op != script.DELEGATECALL && op != script.STATICCALL &&
		op != script.CALL && op != script.CALLCODE {
		return
	}
...
func (t *fourByteTracer) CaptureFault(pc uint64, op script.OpCode, gas, cost uint64, scope *script.ScopeContext, depth int, err error) {
}
