...
func (l *JSONLogger) CaptureStart(env *script.PVM, from, to util.Address, create bool, input []byte, gas uint64, value *big.Int) {
	l.env = env
}

func (l *JSONLogger) CaptureFault(pc uint64, op script.OpCode, gas uint64, cost uint64, scope *script.ScopeContext, depth int, err error) {
...
func (l *JSONLogger) CaptureEnd(output []byte, gasUsed uint64, t time.Duration, err error) {
	...
	l.encoder.Encode(endLog{util.Bytes2Hex(output), math.HexOrDecimal64(gasUsed), t, errMsg})
}

func (l *JSONLogger) CaptureEnter(typ script.OpCode, from util.Address, to util.Address, input []byte, gas uint64, value *big.Int) {
}
