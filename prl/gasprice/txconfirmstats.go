...
func (s *txConfirmStats) removeTx(entryHeight, bestSeenHeight uint64, bucketIdx int, inBlock bool) {
	...
	if blocksAgo < 0 {
		log.Debug("Fee estimator: blocksAgo is negative, ignoring", "entryHeight", entryHeight, "bestSeenHeight", bestSeenHeight)
		return
	}
	...
	for j := minBucket; j <= maxBucket; j++ {
		if s.txCtAvg[j] < halfTxSum {
			...
			if s.txCtAvg[j] > 0 {
				...
				log.Debug("Fee estimator: EstimateMedianVal result",
