...
parallax --testnet --syncmode "full" ... dumpconfig > testnet.toml

# Start Parallax client with given config file. Here too the testnet must be specified.
parallax --testnet --config testnet.toml
```
...
```sh
parallax --bootnodes enode://pubkey1@ip1:port1,enode://pubkey2@ip2:port2,enode://pubkey3@ip3:port3
```
...
```sh
parallax --datadir <datadir> dumpconfig > config.toml
```
...
```sh
parallax <otherflags> --maxpeers 15
```
