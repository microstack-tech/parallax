...
parallaxd --testnet --syncmode "full" ... dumpconfig > testnet.toml

# Start Parallax client with given config file. Here too the testnet must be specified.
parallaxd --testnet --config testnet.toml
```
...
```sh
parallaxd --bootnodes enode://pubkey1@ip1:port1,enode://pubkey2@ip2:port2,enode://pubkey3@ip3:port3
```
...
v2-native bootnode entries (both the hardcoded list and `--bootnodes` values) may also be plain `ip:port` pairs or Tor v3 onion addresses in the form `<addr>.onion:port` — no public key required, since the v2 handshake authenticates against whoever answers at the address. Onion entries are used only when the node has a Tor route configured.

## Connecting over Tor

The client can route its connections through Tor, host an onion service so peers can reach it without any port forwarding, and run entirely inside Tor with `--onlynet=onion`. With a local Tor daemon exposing its control port, all of this is enabled by default — see [Running over Tor](./tor) for setup, flags, and verification.
...
```sh
parallaxd --datadir <datadir> dumpconfig > config.toml
```
...
```sh
parallaxd <otherflags> --maxpeers 15
```
