...
```sh
parallaxd account import --datadir /some-dir ./keyfile
```
...
```sh
parallaxd account import --password path/password.txt path/keyfile
```
...
```sh
parallaxd account update a94f5374fce5edbc8e2a8697c15331677e6ebf0b
```
...
```sh
parallaxd account update a94f5374fce5edbc8e2a8697c15331677e6ebf0b --password path/password.txt
```

Updating the account using `parallaxd account update` replaces the original file with a new one - this means the original file is no longer available after it has been updated. This can be used to update a key file to the latest format.
