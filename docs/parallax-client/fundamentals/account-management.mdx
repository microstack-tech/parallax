...
```sh
parallax --testnet --datadir testnet <other flags> --signer=testnet/clef/clef.ipc
```
...
```sh
parallax account import --datadir /some-dir ./keyfile
```
...
```sh
parallax account import --password path/password.txt path/keyfile
```
...
```sh
parallax account update a94f5374fce5edbc8e2a8697c15331677e6ebf0b
```
...
```sh
parallax account update a94f5374fce5edbc8e2a8697c15331677e6ebf0b --password path/password.txt
```

Updating the account using `parallax account update` replaces the original file with a new one - this means the original file is no longer available after it has been updated. This can be used to update a key file to the latest format.
