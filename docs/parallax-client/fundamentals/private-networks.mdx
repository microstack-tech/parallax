...
Parallax's PoW algorithm, XHash, is a system that allows open participation by anyone willing to dedicate resources to mining. While this is a critical property for a public network, the overall security of the blockchain strictly depends on the total amount of resources used to secure it. As such, PoW is a poor choice for private networks with few miners. The XHash mining 'difficulty' is adjusted automatically so that new blocks are created approximately 10 minutes apart. As more mining resources are deployed on the network, creating a new block becomes harder so that the average block time matches the target block time.
...
* Initial block gas limit (gasLimit). This impacts how much EVM computation can happen within a single block. Mirroring the main Parallax network is generally a good choice. The block gas limit can be adjusted after launch using the `--miner.gaslimit` command-line flag.
...
```sh
parallaxd account new --datadir data
```
...
To create a blockchain node that uses this genesis block, first use `parallaxd init` to import and sets the canonical genesis block for the new chain. This requires the path to `genesis.json` to be passed as an argument.

```sh
parallaxd init --datadir data genesis.json
```
...
```sh
parallaxd --datadir data --networkid 15 --nat extip:172.16.254.4
```

The 'node record' of the bootnode can be extracted using the JS console:

```sh
parallaxd attach --exec admin.nodeInfo.enr data/parallax.ipc
```
...
```sh
parallaxd <other-flags> --netrestrict 172.16.254.0/24
```
...
```sh
parallaxd --datadir data2 --networkid 12345 --port 30305 --bootnodes <bootstrap-node-record>
```
...
```sh
parallaxd attach data2/parallax.ipc --exec admin.peers
```
...
```sh
parallaxd <other-flags> --unlock 0x7df9a875a174b3bc565e6424a0050ebc1b2d1d82 --mine
```

Mining can be further configured by changing the default gas limit blocks converge to (with `--miner.gaslimit`) and the price transactions are accepted at (with `--miner.gasprice`).
...
```sh
parallaxd <other-flags> --mine --miner.threads=1 --miner.coinbase=0xf41c74c9ae680c1aa78f42e5647a62f353b7bdde
```

This will start mining bocks and transactions on a single CPU thread, crediting all block rewards to the account specified by `--miner.coinbase`.
...
```sh
parallaxd --datadir node1 account new
```
...
The nodes can now be set up using `parallaxd init` as follows:

```sh
parallaxd init --datadir node1 genesis.json
```

This should be repeated for both nodes. The following will be returned to the terminal:

```
INFO [05-13|15:41:47.520] Maximum peer count                       total=50
...
INFO [05-13|15:41:47.520] Allocated cache and file handles         database=/home/parallax/node2/parallax/chaindata cache=16.00MiB handles=16
...
INFO [05-13|15:41:47.543] Allocated cache and file handles         database=/home/parallax/node2/parallax/chaindata cache=16.00MiB handles=16
...
```
INFO [05-13|16:17:40.061] Maximum peer count                       total=50
...
INFO [05-13|16:17:40.061] Allocated cache and file handles         database=/home/parallax/node1/parallax/chaindata cache=512.00MiB handles=524,288
INFO [05-13|16:17:40.094] Opened ancient database                  database=/home/parallax/node1/parallax/chaindata/ancient readonly=false
...
INFO [05-13|16:17:40.096] Initialising Parallax protocol           network=123,454,321 dbversion=8
...
INFO [05-13|16:17:40.100] Starting peer-to-peer node               instance=Parallax/v1.0.0-unstable-8d84a701-20220503/linux-amd64/go1.18.1
...
INFO [05-13|16:17:40.133] IPC endpoint opened                      url=/home/parallax/node1/parallax.ipc
...
```sh
parallaxd attach node1/parallax.ipc
```
...
[{
    ...
    name: "Parallax/v1.0.0-unstable-8d84a701-20220503/linux-amd64/go1.18.1",
    network: {
      inbound: false,
