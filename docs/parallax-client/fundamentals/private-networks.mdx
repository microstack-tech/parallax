...
```sh
parallax account new --datadir data
```
...
To create a blockchain node that uses this genesis block, first use `parallax init` to import and sets the canonical genesis block for the new chain. This requires the path to `genesis.json` to be passed as an argument.

```sh
parallax init --datadir data genesis.json
```
...
```sh
parallax --datadir data --networkid 15 --nat extip:172.16.254.4
```

The 'node record' of the bootnode can be extracted using the JS console:

```sh
parallax attach --exec admin.nodeInfo.enr data/parallax.ipc
```
...
```sh
parallax <other-flags> --netrestrict 172.16.254.0/24
```
...
```sh
parallax --datadir data2 --networkid 12345 --port 30305 --bootnodes <bootstrap-node-record>
```
...
```sh
parallax attach data2/parallax.ipc --exec admin.peers
```
...
```sh
parallax <other-flags> --unlock 0x7df9a875a174b3bc565e6424a0050ebc1b2d1d82 --mine
```
...
```sh
parallax <other-flags> --mine --miner.threads=1 --miner.coinbase=0xf41c74c9ae680c1aa78f42e5647a62f353b7bdde
```
...
```sh
parallax --datadir node1 account new
```
...
The nodes can now be set up using `parallax init` as follows:

```sh
parallax init --datadir node1 genesis.json
```
...
```sh
parallax attach node1/parallax.ipc
```
