...
```sh
#! /bin/bash
./parallaxd --testnet --datadir testnet --http --http.api eth,net --metrics.expensive --metric.addr 127.0.0.1 --metrics.port 6060
```
...
```sh
./parallaxd --testnet dumpconfig > parallax-config.toml
```
...
To run the Parallax client with the configuration defined in `parallax-config.toml`, pass the config file path to `--config`. The `network_id` is not persisted from the config file; it has to be explicitly defined on the command line on startup, for example:

```
./parallaxd --testnet --config parallax-config.toml
```
