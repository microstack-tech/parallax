...
Convenience wrapper for `parallax --daemon`. Starts the node in the background, detached from the controlling terminal, with logs redirected to `<datadir>/parallax.log`. An optional positional argument points at a TOML configuration file to pass through as `--config`.
...
```sh
parallax loglevel 4                   # bump everything to debug
parallax loglevel 3                   # back to info
parallax loglevel "eth/*=5,p2p=4"     # per-package overrides
```
...
```sh
parallax trace 0xabc...
parallax trace 0xabc... --tracer callTracer
parallax trace 0xabc... --tracer callTracer --timeout 2m
```
...
Intended for quick "am I running out of disk?" checks. For a full per-prefix chaindata walk, use `parallax db inspect` (offline, reads the database directly).
...
JSON array of addresses the running node can see. Equivalent to the offline `parallax account list`, but works without stopping the daemon. Uses `personal_listAccounts`.

```sh
$ parallax listaccounts
[
  "0xa202...5108",
  "0x96a5...6b4d"
]
...
```sh
parallax newaccount
parallax newaccount --password /etc/parallax/newkey.pass
```
...
```sh
parallax unlock 0xabc...123
parallax unlock 0xabc...123 --password /run/secrets/acct.pass --duration 1h
parallax unlock 0xabc...123 --duration 0     # keep unlocked until the node stops
```
...
```sh
parallax sign 0xabc...123 "approve this request"
parallax sign 0xabc...123 0xdeadbeef --password /run/secrets/acct.pass
```
...
```sh
parallax sendtx --from 0xabc... --to 0xdef... --value 1000000000000000000 --password /run/secrets/from.pass

parallax sendtx --from 0xabc... --data 0x608060...   # contract deploy
```
...
```sh
$ parallax decoderaw 0xf86580843b9aca00825208941111111111111111111111111111111111111111010180820a96a0...
{
  "chainid": "1337",
  "data": "0x",
...
```sh
$ parallax toaddr 0x4c0883a69102937d6231471b5dbb6204fe5129617082792ae468d01a3f362318
0x2c7536E3605D9C16a7a3D7b1898e529396a65c23

$ parallax toaddr 4c0883a69102937d6231471b5dbb6204fe5129617082792ae468d01a3f362318   # 0x prefix optional
0x2c7536E3605D9C16a7a3D7b1898e529396a65c23
```
