...
Returns a summary of on-disk database sizes. The `ancients` object lists per-category frozen-chain sizes in bytes; `leveldb` carries the same human-readable compaction-stats table that `debug_chaindbProperty("leveldb.stats")` returns. Intended for quick storage-use checks and backs the `parallax-cli dbstats` command-line helper.

| CLIENT  | METHOD INVOCATION                                           |
| ------- | ----------------------------------------------------------- |
| Go      | debug.DbStats() (map[string]interface{}, error)             |
| Console | debug.dbStats()                                             |
| RPC     | \{method": "debug\_dbStats"\}                                 |
...
| CLIENT  | METHOD INVOCATION                                   |
| ------- | --------------------------------------------------- |
| Go      | debug.DumpBlock(number uint64) (state.Dump, error)  |
...
| CLIENT  | METHOD INVOCATION                                                               |
| ------- | ------------------------------------------------------------------------------- |
| Go      | debug.TraceBlockByHash(hash util.Hash, config \*TraceConfig) BlockTraceResult |
...
| CLIENT  | METHOD INVOCATION                                                                           |
| ------- | ------------------------------------------------------------------------------------------- |
| Go      | debug.TraceTransaction(txHash util.Hash, config \*TraceConfig) (\*ExecutionResult, error) |
...
You can compose these basic patterns. If you want to see all output from peer.go in a package below node (node/peer.go, node/fullnode/peer.go) as well as output from package p2p at level \<\= 5, use:

```javascript
debug.vmodule('node/*/peer.go=6,p2p=5');
```
