...
Returns the number of seconds the node has been running since its most recent `Start`. Returns 0 if the node is not yet fully started. Mirrors Bitcoin Core's `uptime` command and backs the `parallax uptime` command-line helper.
...
This is the underlying method used by the `parallax stop` command-line helper, analogous to `bitcoin-cli stop` in Bitcoin Core.
