...
The Parallax client has a built-in command to check whether it is affected by any publically disclosed vulnerability, using the command `parallax version-check`. This command will fetch the latest json file (and the accompanying [signature-file](https://geth.ethereum.org/docs/vulnerabilities/vulnerabilities.json.minisig), and cross-check the data against its own version number.
