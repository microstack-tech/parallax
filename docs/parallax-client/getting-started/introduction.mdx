...
If the Parallax client was installed from source on Linux, make saves the binaries for the Parallax client and the associated tools in /build/bin. To run these programs it is convenient to move them to the top level project directory (e.g. running mv ./build/bin/\* ./) from /parallax. Then ./ must be prepended to the commands in the code snippets in order to execute a particular program, e.g. ./parallax instead of simply parallax. If the executables are not moved then either navigate to the bin directory to run them (e.g. cd ./build/bin and ./parallax) or provide their path (e.g. ./build/bin/parallax). These instructions can be ignored for other installations.
...
```sh
parallax attach http://127.0.0.1:8545
```
...
instance: Parallax/v1.0.0-stable/darwin-amd64/go1.24.0
