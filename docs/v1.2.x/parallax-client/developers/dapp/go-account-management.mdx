...
The encrypted keystore is implemented by the [accounts.Manager](https://godoc.org/github.com/ParallaxProtocol/parallax/accounts#Manager) struct from the [accounts](https://godoc.org/github.com/ParallaxProtocol/parallax/accounts) package, which also contains the configuration constants for the *standard* or *light* security modes described above. Hence client side account management simply requires importing the accounts package into the application code.

```go
import "github.com/ParallaxProtocol/parallax/accounts"
import "github.com/ParallaxProtocol/parallax/accounts/keystore"
...
The last two arguments of [keystore.NewKeyStore](https://godoc.org/github.com/ParallaxProtocol/parallax/accounts/keystore#NewKeyStore) are the crypto parameters defining how resource-intensive the keystore encryption should be. The options are [accounts.StandardScryptN, accounts.StandardScryptP, accounts.LightScryptN, accounts.LightScryptP](https://godoc.org/github.com/ParallaxProtocol/parallax/accounts#pkg-constants) or custom values (requiring understanding of the underlying cryptography). The *standard* version is recommended.
...
A Parallax account is implemented by the [accounts.Account](https://godoc.org/github.com/ParallaxProtocol/parallax/accounts#Account) struct from the Parallax client [accounts](https://pkg.go.dev/github.com/ParallaxProtocol/parallax/accounts) package. Assuming an instance of an [accounts.Manager](https://godoc.org/github.com/ParallaxProtocol/parallax/accounts#Manager) called am exists, all of the described lifecycle operations can be executed with a handful of function calls (error handling omitted).
...
Assuming an instance of an [accounts.Manager](https://godoc.org/github.com/ParallaxProtocol/parallax/accounts#Manager) called am exists, a new account can be created to sign transactions using [NewAccount](https://godoc.org/github.com/ParallaxProtocol/parallax/accounts#Manager.NewAccount). Creating transactions is out of scope for this page so instead a random [common.Hash](https://godoc.org/github.com/ParallaxProtocol/parallax/common#Hash) will be signed instead.
...
Note that [SignWithPassphrase](https://godoc.org/github.com/ParallaxProtocol/parallax/accounts#Manager.SignWithPassphrase) takes an [accounts.Account](https://godoc.org/github.com/ParallaxProtocol/parallax/accounts#Account) as the signer, whereas [Sign](https://godoc.org/github.com/ParallaxProtocol/parallax/accounts#Manager.Sign) takes only a [common.Address](https://godoc.org/github.com/ParallaxProtocol/parallax/common#Address). The reason for this is that an [accounts.Account](https://godoc.org/github.com/ParallaxProtocol/parallax/accounts#Account) object may also contain a custom key-path, allowing [SignWithPassphrase](https://godoc.org/github.com/ParallaxProtocol/parallax/accounts#Manager.SignWithPassphrase) to sign using accounts outside of the keystore; however [Sign](https://godoc.org/github.com/ParallaxProtocol/parallax/accounts#Manager.Sign) relies on accounts already unlocked within the keystore, so it cannot specify custom paths.
