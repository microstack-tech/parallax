...
If the Parallax client was installed from source on Linux, `make parallax` drops the three suite binaries — `parallaxd` (the full-node daemon), `parallax-cli` (the JSON-RPC client) and `parallax` (a multi-call wrapper that dispatches `parallax node …` → `parallaxd` and `parallax rpc …` → `parallax-cli`) — into `./build/bin/`. Either move them to somewhere on `$PATH`, add `./build/bin` to `$PATH`, or prefix each invocation with `./build/bin/` in the snippets below.
