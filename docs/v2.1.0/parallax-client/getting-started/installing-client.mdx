...
These commands create three executable files under `parallax/build/bin/`: `parallaxd` (the full-node daemon), `parallax-cli` (the JSON-RPC client) and `parallax` (a thin wrapper that dispatches `parallax node …` → `parallaxd` and `parallax rpc …` → `parallax-cli`). All three are statically linked and can be moved or copied independently; the wrapper resolves `parallaxd` / `parallax-cli` as siblings in its own install directory or via `$PATH`.
