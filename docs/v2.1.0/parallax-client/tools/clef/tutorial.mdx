...
Apart from listing accounts, a *request* can be submitted to create a new account, signing transactions and data or recovering signatures. The available methods are documented in the Clef [External API Spec](https://github.com/ParallaxProtocol/parallax/tree/main/cmd/clef#external-api-1) and the [External API Changelog](https://github.com/ParallaxProtocol/parallax/blob/main/cmd/clef/extapi_changelog.md).

<Note>
*Note, the number of things that can be done from the External API is deliberately small to limit the power of remote calls as much as possible! Clef has an* [*Internal API*](https://github.com/ParallaxProtocol/parallax/tree/main/cmd/clef#ui-api-1) *too for the UI (User Interface) which is much richer and can support custom interfaces on top. But that's out of scope here.*
</Note>
...
```sh
parallaxd --testnet --datadir testnet --signer=testnet/clef/clef.ipc
```

With the Parallax client running, open a new terminal and attach a Javascript console:

```sh
parallaxd attach testnet/parallax.ipc
```
...
More examples, including a ruleset for a rate-limited window, are available on the [Clef GitHub](https://github.com/ParallaxProtocol/parallax/blob/main/cmd/clef/rules.md#example-1-ruleset-for-a-rate-limited-window) and on the [Rules page](./rules).
