...
The Parallax client has a built-in command to check whether it is affected by any publically disclosed vulnerability, using the command `parallaxd version-check`. This command will fetch the latest json file (and the accompanying [signature-file](https://geth.ethereum.org/docs/vulnerabilities/vulnerabilities.json.minisig), and cross-check the data against its own version number.
...
* `uid`
  * Unique identifier of the vulnerability. Format `Parallax-<year>-<sequential id>`
* `summary`
...
Advisories published via GitHub can be accessed [here](https://github.com/ParallaxProtocol/parallax/security/advisories?state=published).
