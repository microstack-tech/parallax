...
The client can now be used to handle requests to the Parallax client using the full JSON-RPC API. For example, the function `BlockNumber()` wraps a call to the `eth_blockNumber` endpoint. The function `SendTransaction` wraps a call to `eth_sendTransaction`. The full list of client methods can be found [here](https://pkg.go.dev/github.com/ParallaxProtocol/parallax/rpc/client#Client).
...
import (
 ...
 "github.com/ParallaxProtocol/parallax/rpc/client"
...
Details relating to these endpoints can be found at [pkg.go.dev](https://pkg.go.dev/github.com/ParallaxProtocol/parallax/rpc/client) or the Parallax client [GitHub](https://github.com/ParallaxProtocol/parallax/tree/main/rpc/client).
