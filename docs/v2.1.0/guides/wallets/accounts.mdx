...
<Steps>
  ...
  <Step title="Standalone keyfiles (advanced)">
    Advanced users can produce a single, portable keyfile (outside the keystore) with:

    ```bash
    ./parallax-wallet generate my-key.json
    ```

    Use this for air-gapped cold-signing workflows. Regular users should prefer `parallax-wallet new`.
  </Step>
</Steps>
