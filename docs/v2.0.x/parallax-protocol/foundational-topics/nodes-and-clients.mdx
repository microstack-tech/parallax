...
Note: the Parallax client does not currently include a light sync mode. Light client support may be added in a future release.
...
* [GitHub](https://github.com/ParallaxProtocol/parallax)
...
Light client mode downloads all block headers, block data, and verifies some randomly. Only syncs tip of the chain from the trusted checkpoint. The Parallax client does not implement this mode.
...
**Setting up sync mode in** [**Parallax**](https://github.com/ParallaxProtocol/parallax)

`parallaxd --syncmode "full"`

For further details, check out the guide on [sync modes](../../parallax-client/fundamentals/sync-modes).
