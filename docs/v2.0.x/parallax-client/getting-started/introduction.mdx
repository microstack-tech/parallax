...
If the Parallax client was installed from source on Linux, `make parallax` drops the four suite binaries — `parallaxd` (the full-node daemon), `parallax-cli` (the JSON-RPC client), `parallax-wallet` (the offline wallet tool) and `parallax` (a multi-call wrapper that dispatches `parallax node …` → `parallaxd`, `parallax rpc …` → `parallax-cli` and `parallax wallet …` → `parallax-wallet`) — into `./build/bin/`. Either move them to somewhere on `$PATH`, add `./build/bin` to `$PATH`, or prefix each invocation with `./build/bin/` in the snippets below.
...
```sh
parallaxd --testnet --datadir parallax-tutorial --http --http.api eth,net --signer=parallax-tutorial/clef/clef.ipc --http
```
...
```sh
parallaxd attach http://127.0.0.1:8545
```
