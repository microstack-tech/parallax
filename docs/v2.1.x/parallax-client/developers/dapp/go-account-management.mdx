...
The encrypted keystore is implemented by the [wallet.Manager](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/wallet#Manager) struct from the [wallet](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/wallet) package, which also contains the configuration constants for the *standard* or *light* security modes described above. Hence client side account management simply requires importing the wallet package into the application code.

```go
import "github.com/ParallaxProtocol/parallax/v2/wallet"
import "github.com/ParallaxProtocol/parallax/v2/wallet/keystore"
...
am := wallet.NewManager(&wallet.Config{InsecureUnlockAllowed: false}, ks)
```
...
The last two arguments of [keystore.NewKeyStore](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/wallet/keystore#NewKeyStore) are the crypto parameters defining how resource-intensive the keystore encryption should be. The options are [wallet.StandardScryptN, wallet.StandardScryptP, wallet.LightScryptN, wallet.LightScryptP](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/wallet#pkg-constants) or custom values (requiring understanding of the underlying cryptography). The *standard* version is recommended.
...
A Parallax account is implemented by the [wallet.Account](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/wallet#Account) struct from the Parallax client [wallet](https://pkg.go.dev/github.com/ParallaxProtocol/parallax/v2/wallet) package. Assuming an instance of an [wallet.Manager](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/wallet#Manager) called am exists, all of the described lifecycle operations can be executed with a handful of function calls (error handling omitted).
...
*Although instances of* [*wallet.Account*](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/wallet#Account) *can be used to access various information about specific Parallax accounts, they do not contain any sensitive data (such as passphrases or private keys), rather they act solely as identifiers for client code and the keystore.*
...
Assuming an instance of an [wallet.Manager](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/wallet#Manager) called am exists, a new account can be created to sign transactions using [NewAccount](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/wallet#Manager.NewAccount). Creating transactions is out of scope for this page so instead a random [util.Hash](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/util#Hash) will be signed instead.
...
txHash := util.HexToHash("0x0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef")
```
...
Note that [SignWithPassphrase](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/wallet#Manager.SignWithPassphrase) takes an [wallet.Account](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/wallet#Account) as the signer, whereas [Sign](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/wallet#Manager.Sign) takes only a [util.Address](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/util#Address). The reason for this is that an [wallet.Account](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/wallet#Account) object may also contain a custom key-path, allowing [SignWithPassphrase](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/wallet#Manager.SignWithPassphrase) to sign using accounts outside of the keystore; however [Sign](https://godoc.org/github.com/ParallaxProtocol/parallax/v2/wallet#Manager.Sign) relies on accounts already unlocked within the keystore, so it cannot specify custom paths.
