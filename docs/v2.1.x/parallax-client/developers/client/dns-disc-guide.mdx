...
```sh
go get github.com/ParallaxProtocol/parallax/v2/cmd/devp2p
```

To create a signing key, the `parallax-wallet` utility is needed. It ships
with the release package and replaces the older standalone `parallaxkey`
tool.

```sh
go get github.com/ParallaxProtocol/parallax/v2/cmd/parallax-wallet
```
...
To turn a node list into a DNS node tree, the list needs to be signed. To do this, a key pair is required. To create the key file in the correct format, the `cmd/parallax-wallet` utility should be used. Choose a strong password to encrypt the key on disk!

```sh
parallax-wallet generate dnskey.json
```
...
```sh
parallaxd --discovery.dns "enrtree://AMBMWDM3J6UY3M32TMMROUNLX6Y3YTLVC3DC6HN2AVG5NHNSAXDW6@mainnet.nodes.example.org"
```
